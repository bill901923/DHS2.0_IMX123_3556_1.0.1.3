...
prepare: sdk_prepare middleware_prepare
	...
	@cd $(SDK_ROOT)/build && if [ -f config_mpp_$(CFG_CHIP_TYPE).sh ];then ./config_mpp_$(CFG_CHIP_TYPE).sh;fi;cd -
...
drv: prepare
	...
	@if [ -f $(DRV_INTERDRV_PATH_LITEOS)/mipi_rx/Makefile ];then make -C $(DRV_INTERDRV_PATH_LITEOS)/mipi_rx||exit $?;fi
