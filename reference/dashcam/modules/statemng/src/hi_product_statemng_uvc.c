...
static HI_S32 PDT_STATEMNG_ResetUVCMediaCfg(HI_PDT_MEDIA_CFG_S* pstMediaCfg)
{
    ...
    for (i = 0; i < HI_PDT_MEDIA_VPSS_MAX_CNT; ++i)
    {
        ...
        for (j = 0; j < HI_PDT_MEDIA_VPSS_PORT_MAX_CNT; ++j)
        {
            ...
            if (pstMediaCfg->stViVpssMode.astMode[0][0].enMode != VI_ONLINE_VPSS_ONLINE)
            {
                pstMediaCfg->stVideoCfg.stVprocCfg.astVpssAttr[i].astVportAttr[j].bSupportBufferShare = HI_TRUE;
            }
#endif
            bVpssFound = HI_TRUE;
