...
static HI_S32 PDT_STATEMNG_SetAllCamAntifliker(const HI_PDT_MEDIA_CFG_S *pstMediaCfg)
{
    HI_S32 s32Ret = HI_SUCCESS;
    PDT_STATEMNG_CONTEXT* pstStateMngCtx = PDT_STATEMNG_GetCtx();
    ...
    for (i = 0; i < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT; i++)
    {
        HI_PDT_SCENE_ANTIFLICKER_E enAntiflicker = 0;
        s32Ret = HI_PDT_PARAM_GetCamParam(pstStateMngCtx->enCurrentWorkMode, i,
                                          HI_PDT_PARAM_TYPE_ANTIFLICKER,
                                          (HI_PDT_SCENE_ANTIFLICKER_E*)&enAntiflicker);
        PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINTER, "get Antiflicker param");

        const HI_PDT_MEDIA_VCAP_DEV_ATTR_S* pstVcapDevAttr = &pstMediaCfg->stVideoCfg.stVcapCfg.astVcapDevAttr[i];
        ...
        for (j = 0; j < HI_PDT_MEDIA_VCAPDEV_PIPE_MAX_CNT; j++)
        {
            if (HI_TRUE != pstVcapDevAttr->astVcapPipeAttr[j].bEnable ||
                HI_TRUE == pstVcapDevAttr->astVcapPipeAttr[j].bIspBypass ||
                (WDR_MODE_2To1_LINE == pstVcapDevAttr->enWdrMode && j > 0))
            {
                continue;
            }

            const HI_PDT_MEDIA_VCAP_PIPE_ATTR_S* pstVcapPipeAttr = &pstVcapDevAttr->astVcapPipeAttr[j];
            VcapPipeHdl = pstVcapPipeAttr->VcapPipeHdl;

            s32Ret = HI_PDT_SCENE_SetAntiflicker(VcapPipeHdl, enAntiflicker);
            PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINTER, "set Antiflicker");
            MLOGD("Set Antifliker VcapHdl[%d] Antiflicker[%d].\n", VcapPipeHdl, enAntiflicker);
        }
...
HI_S32 PDT_STATEMNG_SetSceneMode(const HI_PDT_MEDIA_CFG_S *mediaCfg)
{
    HI_S32 ret = HI_SUCCESS;
    
    if (s_stSTATEMNGContext.enCurrentWorkMode != HI_PDT_WORKMODE_USB_STORAGE
        && s_stSTATEMNGContext.enCurrentWorkMode != HI_PDT_WORKMODE_UPGRADE) {
        HI_PDT_SCENE_MODE_S sceneMode = {0};
        ret = HI_PDT_PARAM_GetSceneCfg(s_stSTATEMNGContext.enCurrentWorkMode, s_stSTATEMNGContext.astCamCtx, &sceneMode);
        PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "get scene configure");
        ret = HI_PDT_SCENE_SetSceneMode(&sceneMode);
        
        PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "set scene mode");

        ret = PDT_STATEMNG_SetAllCamAntifliker(mediaCfg);
        PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "set scene enAntifliker");
        MLOGD("HI_PDT_SCENE_Pause 3.\n");
        ret = HI_PDT_SCENE_Pause(HI_FALSE);
        PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "scene pause, false");
    }
    return HI_SUCCESS;
}


HI_S32 PDT_STATEMNG_RebuildMedia(const HI_PDT_MEDIA_CFG_S* mediaCfg)
{
    HI_S32 ret = HI_SUCCESS;
    MLOGI("rebuild media!\n");
    ret = HI_PDT_SCENE_Pause(HI_TRUE);
    PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "scene pause, true");

    HI_PDT_MEDIA_VideoOutStop();

    HI_PDT_MEDIA_AudioDeinit();

    ret = HI_PDT_MEDIA_AudioOutStop();
    PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "audio out stop");

    ret = HI_PDT_MEDIA_VideoRebuild(mediaCfg);
    PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "video rebuild");

    ret = PDT_STATEMNG_SetSceneMode(mediaCfg);
    PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "set scenemode");

    if(s_stSTATEMNGContext.bScreenDormantStatus != HI_TRUE)
    {
        ret = HI_PDT_MEDIA_VideoOutStart(HI_NULL,NULL);
        PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "video out start");
    }

    ret = HI_PDT_MEDIA_AudioInit(&mediaCfg->stAudioCfg);
    PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "audio init");

    ret = HI_PDT_MEDIA_AudioOutStart(&mediaCfg->stAudioOutCfg);
    PDT_STATEMNG_CHECK_RET(ret, HI_PDT_STATEMNG_EINTER, "audio out start");

    return HI_SUCCESS;
}


HI_S32 PDT_STATEMNG_ResetMedia(HI_PDT_MEDIA_CFG_S *pstMediaCfg)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    s32Ret = HI_PDT_MEDIA_IsAudioNeedReset(pstMediaCfg, &audioNeedReset, &audiooutNeedReset);
    PDT_STATEMNG_CHECK_RET(s32Ret,HI_PDT_STATEMNG_EINTER,"HI_PDT_MEDIA_IsAudioNeedReset");

    if (HI_TRUE == bFirst) {
        MLOGI("fastboot ignore first vo and ao stop\n");
    }
    else
    {
        /** Video out stop */
        ...
        if (audioNeedReset || pstStateMngCtx->bForceReBuid) {
            HI_PDT_MEDIA_AudioDeinit();
        }

        if (audiooutNeedReset || pstStateMngCtx->bForceReBuid) {
    ...
    {
        s32Ret = HI_PDT_MEDIA_VideoReset(pstMediaCfg, compress);
    }
    if(HI_PDT_MEDIA_ENOTINIT == s32Ret)
    {
    ...
    s32Ret = PDT_STATEMNG_SetSceneMode(pstMediaCfg);
    PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINTER, "set scenemode");


    s32Ret = HI_PDT_MEDIA_AudioInit(&pstMediaCfg->stAudioCfg);
    PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINTER, "audio init");

    s32Ret = HI_PDT_MEDIA_AudioOutStart(&pstMediaCfg->stAudioOutCfg);
    PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINTER, "audio out start");
    ...
    else if(pstStateMngCtx->bScreenDormantStatus != HI_TRUE)
    {
        /** video out start */
        s32Ret = HI_PDT_MEDIA_VideoOutStart(NULL,NULL);
        if(HI_SUCCESS != s32Ret)
        {
            MLOGD("PDT_STATEMNG_RebuildMedia 2n\n");
            PDT_STATEMNG_RebuildMedia(pstMediaCfg);
    ...
    /** add all stream */
            ...
            MLOGD("PDT_STATEMNG_AddRtspStreamsByType\n");
    s32Ret = PDT_STATEMNG_AddRtspStreamsByType(pstMediaCfg, HI_PDT_MEDIA_VENC_TYPE_LIVE);
...
HI_S32 PDT_STATEMNG_ChangeRecType(HI_REC_TYPE_E enRecType)
{
...
#ifdef CONFIG_RAWCAP_ON
    if((HI_TRUE == pstStateMngCtx->stRawCapCtx.bDebug))
    {
        bCompress = HI_FALSE;
        HI_S32 i,j = 0;
        for(i=0;i<HI_PDT_MEDIA_VCAP_DEV_MAX_CNT;i++)
    ...
    /** set Media config */
    s32Ret = HI_PDT_MEDIA_VideoReset(&stEnterMediaCfg, bCompress);
    if(HI_PDT_MEDIA_ENOTINIT == s32Ret)
    {
        MLOGD(YELLOW"Media not init\n\n"NONE);
    }
...
#endif
    /** add all stream */
    s32Ret = PDT_STATEMNG_AddRtspStreamsByType(&stEnterMediaCfg, HI_PDT_MEDIA_VENC_TYPE_LIVE);
    PDT_STATEMNG_CHECK_RET_WITH_UNRETURN(s32Ret, "add rtsp streams");
...
HI_S32 PDT_STATEMNG_SetAudio(HI_MESSAGE_S* pstMsg)
{
        ...
        MLOGE(YELLOW"PDT_STATEMNG_SetAudio\n\n"NONE);
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 i = 0;
    PDT_STATEMNG_CONTEXT *pstStateMngCtx = PDT_STATEMNG_GetCtx();
    HI_PDT_MEDIA_CFG_S stMediaCfg;
    ...
    PDT_STATEMNG_CHECK_RET(s32Ret, s32Ret, "HI_PDT_MEDIA_AudioInit");
...
HI_S32 PDT_STATEMNG_SetFlip(HI_MESSAGE_S* pstMsg)
{
    ...
    for (i = 0; i < HI_PDT_MEDIA_VCAPDEV_PIPE_MAX_CNT; i++)
    {
        ...
        if (HI_TRUE == pstVcapDevAttr->astVcapPipeAttr[i].bIspBypass
            || stMediaCfg.stViVpssMode.astMode[pstStateMngCtx->s32PreviewCamID][i].enMode == VI_OFFLINE_VPSS_ONLINE
...
HI_S32 PDT_STATEMNG_SetAntiflicker(HI_MESSAGE_S* pstMsg)
{
    HI_S32 s32Ret = HI_SUCCESS;
    PDT_STATEMNG_CONTEXT* pstStateMngCtx = PDT_STATEMNG_GetCtx();
    /** get setting information */
    HI_PDT_WORKMODE_E enSettingWorkMode = pstMsg->arg2;
    HI_PDT_SCENE_ANTIFLICKER_E enAntiflicker = 0;
    memcpy(&enAntiflicker, pstMsg->aszPayload, sizeof(HI_PDT_SCENE_ANTIFLICKER_E));

    /** get param to Param module */
    HI_PDT_SCENE_ANTIFLICKER_E enOldAntiflicker = 0;
    s32Ret = HI_PDT_PARAM_GetCamParam(enSettingWorkMode,
                                      pstStateMngCtx->s32PreviewCamID,
                                      HI_PDT_PARAM_TYPE_ANTIFLICKER,
                                      (HI_VOID*)&enOldAntiflicker);
    PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINVAL, "get Antiflicker");

    /** filter out the same parameter */
    if (enOldAntiflicker == enAntiflicker)
    {
        MLOGE(YELLOW"the setting item value as same as current setting value\n\n"NONE);
        return HI_SUCCESS;
    }
    ...
    HI_S32 i = 0;
    /** get media config from Param module */
    HI_PDT_MEDIA_CFG_S stMediaCfg = {0};
    ...
    {
        if (HI_TRUE != pstVcapDevAttr->astVcapPipeAttr[i].bEnable ||
                HI_TRUE == pstVcapDevAttr->astVcapPipeAttr[i].bIspBypass)
        {
            continue;
        }
    ...
    s32Ret = HI_PDT_PARAM_SetCamParam(enSettingWorkMode,
                                      pstStateMngCtx->s32PreviewCamID,
                                      HI_PDT_PARAM_TYPE_ANTIFLICKER,
                                      (HI_VOID*)&enAntiflicker);
    PDT_STATEMNG_CHECK_RET(s32Ret, HI_PDT_STATEMNG_EINVAL, "save Antiflicker");
    return s32Ret;
}
...
#ifdef CONFIG_MOTIONDETECT_ON
    s32Ret = PDT_STATEMNG_StartMd();
    PDT_STATEMNG_CHECK_RET(s32Ret, s32Ret, "PDT_STATEMNG_StartMd");
#endif
...
HI_S32 PDT_STATEMNG_SetCrop(HI_MESSAGE_S* pstMsg)
{
    ...
    MLOGI(YELLOW"PDT_STATEMNG_SetCrop \n\n"NONE);
    HI_S32 s32Ret = HI_SUCCESS;
    PDT_STATEMNG_CONTEXT* pstStateMngCtx = PDT_STATEMNG_GetCtx();
    /** get setting information */
    HI_PDT_WORKMODE_E enSettingWorkMode = pstMsg->arg2;
    ...
    MLOGI(YELLOW"s32Direction %d %d  %d\n\n"NONE,s32Direction,stCropCfg.u32CurH,stCropCfg.u32MaxH );
    if (0 < s32Direction)
    {
    ...
    MLOGI(YELLOW"stCropCfg.u32CurH %d \n\n"NONE,stCropCfg.u32CurH);
    /** set crop to take effect */
        MLOGI(YELLOW"1. HI_PDT_MEDIA_DispWindowCrop \n\n"NONE);
...
HI_S32 PDT_STATEMNG_SetUsbMode(HI_VOID)
{
    ...
    s32Ret = HI_PDT_USBCTRL_SetMode(enUsbMode);
    ...
    for (s32Idx = 0; s32Idx < HI_PDT_RECTASK_MAX_CNT; s32Idx++)
    {
        if ((pstStateMngCtx->astCamCtx[s32Idx].bEnable) && (pstStateMngCtx->stRecCtx.astRecTaskCtx[s32Idx].bEnable))
        {
            ...
            else
            {
                ...
                if (HI_SUCCESS != s32Ret)
                {
                    HI_RECMNG_StopTask(pstStateMngCtx->stRecCtx.astRecTaskCtx[s32Idx].RecMngTaskHdl,
                ...
                else
                {
                    ...
                    HI_LEDMNG_LED_STATUS_S stLedStatus = {
                        .enLedStatus = HI_LEDMNG_LED_STATUS_FLASH,
                        .s32LightOnTime_us = 1000 * 1000,
                        .s32LightOffTime_us = 1000 * 1000,
                    };
                    HI_LEDMNG_SetStatus(HI_LEDMNG_LED_IDX_0,&stLedStatus);
#endif
                    pstStateMngCtx->stRecCtx.astRecTaskCtx[s32Idx].bFileEndRecv = HI_FALSE;
                }
            }
...
#ifdef CONFIG_LED_ON
            HI_LEDMNG_LED_STATUS_S stLedStatus = {
                .enLedStatus = HI_LEDMNG_LED_STATUS_ON,
                .s32LightOnTime_us = 0,
                .s32LightOffTime_us = 0,
            };
            HI_LEDMNG_SetStatus(HI_LEDMNG_LED_IDX_0,&stLedStatus);
#endif
        }
...
HI_S32 PDT_STATEMNG_RemoveRtspStreamsByType(const HI_PDT_MEDIA_CFG_S* pstMediaCfg, HI_PDT_MEDIA_VENC_TYPE_E vencType)
{
    /** remove streams */
    HI_S32 s32Ret = HI_SUCCESS;
    const HI_PDT_MEDIA_VENC_CFG_S* pstVencCfg =  NULL;

    for (HI_S32 i = 0; i < HI_PDT_MEDIA_VENC_MAX_CNT; i++) {

        pstVencCfg = &(pstMediaCfg->stVideoCfg.astVencCfg[i]);
        if (pstVencCfg->bEnable && pstVencCfg->enVencType == vencType) {

            HI_CHAR szStreamName[HI_APPCOMM_COMM_STR_LEN];
            snprintf(szStreamName, HI_APPCOMM_COMM_STR_LEN, "%d", pstVencCfg->VencHdl);
            MLOGI(GREEN"remove [ %s ]"NONE"\n", szStreamName);
            s32Ret = HI_LIVESVR_RemoveStream(szStreamName);

            if (HI_LIVESVR_ELOST == s32Ret) {
                MLOGI(YELLOW" stream venchdl[%d] is not in RtspStreams \n\n"NONE, pstVencCfg->VencHdl);
            }

            if (HI_SUCCESS != s32Ret && HI_LIVESVR_ELOST != s32Ret) {
                MLOGE(RED"remove stream venchdl[%d] failed \n\n"NONE, pstVencCfg->VencHdl);
                return HI_PDT_STATEMNG_EINTER;
            }
        }
    }
...
HI_S32 PDT_STATEMNG_AddRtspStreamsByType(const HI_PDT_MEDIA_CFG_S* pstMediaCfg, HI_PDT_MEDIA_VENC_TYPE_E vencType)
{
    ...
    for (i = 0; i < HI_PDT_MEDIA_VENC_MAX_CNT; i++)
    {
        ...
        if (pstVencCfg->bEnable && pstVencCfg->enVencType == vencType)
        {
            ...
            {                
                MLOGI(GREEN"PDT_STATEMNG_AddRtspStreamsByType Pass"NONE"\n");
            }
        }
    }
    return HI_SUCCESS;
...
static HI_VOID PDT_STATEMNG_PowerSupplyCheckTaskCB(HI_VOID *privData, struct timeval *now)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_BOOL bIsOn = HI_TRUE;
    s32Ret = PDT_STATEMNG_GetPowerSupplyStatue(&bIsOn);
    PDT_STATEMNG_CHECK_RET_WITH_UNRETURN(s32Ret, "PowerSupplyCheck callback");
    if (HI_FALSE == bIsOn) {
        HI_EVENT_S stEvent;
        memset(&stEvent, 0 , sizeof(HI_EVENT_S));
...
HI_S32 PDT_STATEMNG_CreatePowerSupplyCheckTask()
{
    ...
    stTimeTsdkAttr.stAttr.periodic = HI_FALSE;
    stTimeTsdkAttr.timerProc = PDT_STATEMNG_PowerSupplyCheckTaskCB;
...
HI_S32 PDT_STATEMNG_GetPhotoFileName(HI_S32 s32FileCnt,HI_PHOTOMNG_FILENAME_S* pstFileList,HI_VOID* pvPrivateData)
{
    ...
    MLOGI("PDT_STATEMNG_GetPhotoFileName \n");
    HI_S32 s32Ret = HI_FAILURE;
   //HI_DTCF_DIR_E enDtcfDir = DTCF_DIR_BUTT;
    ...
    MLOGI("pstPhotoTaskCtx->s32CamID %d \n",pstPhotoTaskCtx->s32CamID);
 ...
 /*  s32Ret = HI_FILEMNG_GenerateFileName(HI_DTCF_FILE_TYPE_JPG, enDtcfDir, HI_TRUE, &stFileName);
   ...
   s32Ret=HI_SUCCESS;
    ...
    {
        sprintf(stFileName.szFileName[i],"POTOTEST_%d.jpg",i);
        ...
        MLOGE("pstFileList[%d].szFileName =%s \n", i,pstFileList[i].szFileName);        
        MLOGE("pstFileList[%d].u32PreAllocLen =%d\n",i, pstFileList[i].u32PreAllocLen);
    }
