...
HI_S32 PDT_STATEMNG_PhotoStatesSetMediaMode(HI_MESSAGE_S* pstMsg)
{
    ...
    MLOGI(YELLOW"setting MediaMode(%d) for workmode(%d)\n\n"NONE, enSettingMediaMode, enSettingWorkMode);
...
HI_S32 PDT_STATEMNG_PhotoStatesSettingMsgProc(HI_MESSAGE_S* pstMsg)
{
    ...
    switch (pstMsg->arg1)
    {
        ...
        case HI_PDT_PARAM_TYPE_ANTIFLICKER:
            s32Ret = PDT_STATEMNG_SetAntiflicker(pstMsg);
            break;
...
//#ifdef CONFIG_PQT_SUPPORT_ON
    (HI_VOID)PDT_STATEMNG_AddPQDebugStreams();
    (HI_VOID)HI_PDT_SCENE_Pause(PDT_STATEMNG_GetCtx()->sceneStatus);
//#endif
...
HI_S32 PDT_STATEMNG_PhotoStatesEnter(HI_VOID* pvArg)
{
    ...
    {
        MLOGI(YELLOW"publish HI_EVENT_STATEMNG_SWITCH_WORKMODE(%x), arg2(%s) succeed\n\n"NONE,
...
HI_S32 PDT_STATEMNG_PhotoStatesExit(HI_VOID* pvArg)
{
    ...
    PDT_STATEMNG_STATE_ATTR_S* pstStateAttr = (PDT_STATEMNG_STATE_ATTR_S*)pvArg;
    MLOGI(YELLOW"exit state(%s), bRunning(%d)\n\n"NONE,
...
HI_S32 PDT_STATEMNG_PhotoStatesMsgProc(HI_MESSAGE_S* pstMsg, HI_VOID* pvArg, HI_STATE_ID* pStateID)
{
    ...
    PDT_STATEMNG_STATE_ATTR_S* pstStateAttr = (PDT_STATEMNG_STATE_ATTR_S*)pvArg;
    MLOGI("current state(%s), bRunning(%d)\n\n", pstStateAttr->stState.name, pstStateAttr->bRunning);
    ...
    MLOGI(YELLOW" will process message what(%x) \n\n"NONE, pstMsg->what);
    MLOGI(YELLOW"PDT_STATEMNG_PhotoStatesMsgProc\n\n"NONE);
    switch (pstMsg->what)
    {
        case HI_EVENT_STATEMNG_TRIGGER:
        {
         ...
         //   {
                for(s32Idx = 0;s32Idx < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT; s32Idx++)
                {
                    ...
                    MLOGI("Do %d  HI_PHOTOMNG_TaskStart\n\n",s32Idx);
                ...
                }else
                {
                    ...
                    MLOGI("HI_PHOTOMNG_TaskStart Pass\n");
        ...
        {
            MLOGI(YELLOW"no need to process message HI_EVENT_PHOTOMNG_TASK_END(%x)\n\n"NONE, pstMsg->what);
            return HI_PROCESS_MSG_RESULTE_OK;
        }
        ...
        {
            MLOGI(YELLOW"process msg(0x%08X)\n"NONE, pstMsg->what);
            return HI_PROCESS_MSG_UNHANDLER;
        }
...
//#ifdef CONFIG_PQT_SUPPORT_ON
        case HI_EVENT_STATEMNG_PQDEBUG:
            {
                if (pstStateAttr->bRunning) {
                    PDT_STATEMNG_StopRec(HI_FALSE);
                    MUTEX_LOCK(pstStateMngCtx->Mutex);
                ...
                s32Ret = PDT_STATEMNG_HandlePQDebugMsg(pstMsg);
                if (s32Ret != HI_SUCCESS) {
                    (HI_VOID)PDT_STATEMNG_SetPQDebug(pstStateMngCtx->pqDebugStatus);
                }
                return HI_PROCESS_MSG_RESULTE_OK;
            }

        case HI_EVENT_STATEMNG_SCENE_AUTO:
            {
                s32Ret = HI_PDT_SCENE_Pause(*(HI_BOOL *)pstMsg->aszPayload);
                if (s32Ret == HI_SUCCESS) {
                    pstStateMngCtx->sceneStatus = *(HI_BOOL *)pstMsg->aszPayload;
                }
...
//#endif
        ...
        default:
        {
            ...
            MLOGI(YELLOW"can not process this message, upload it to Base state\n\n"NONE);
            return HI_PROCESS_MSG_UNHANDLER;
        }
    }
...
HI_S32 PDT_STATEMNG_PhotoStatesInit(const PDT_STATEMNG_STATE_ATTR_S* pstBase)
{
    ...
    MLOGI(RED"PDT_STATEMNG_PhotoStatesInit\n\n"NONE);
