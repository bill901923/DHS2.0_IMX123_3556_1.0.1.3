        ...
        PDT_STATEMNG_RAWCAPTASK_CONTEXT_S* pstRawCapCtx)
{
    ...
    if (WDR_MODE_NONE == stVcapAttr.stVcapDevAttr.stWDRAttr.enWDRMode)
    {
...
HI_S32 PDT_STATEMNG_RecStatesSetPlayloadType(HI_MESSAGE_S* pstMsg)
{
    ...
    {
        PDT_STATEMNG_RebuildMedia(&stMediaCfg);
    }
    else
#endif
    {
    ...
    /** add all stream */
    s32Ret = PDT_STATEMNG_AddRtspStreamsByType(&stMediaCfg, HI_PDT_MEDIA_VENC_TYPE_LIVE);
...
static HI_S32 PDT_STATEMNG_SetLapseInterval(HI_MESSAGE_S* pstMsg)
{
    ...
    for (s32Idx = 0; s32Idx < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT; s32Idx++)
    {
        ...
        if (pstRecMngAttr->enRecType != HI_REC_TYPE_LAPSE)
        {
             MLOGI(YELLOW"normal record type  unsupport change lapse_interval\n"NONE);
             return HI_SUCCESS;
        }
...
//#ifdef CONFIG_PQT_SUPPORT_ON
            if (HI_TRUE == pstStateMngCtx->pqDebugStatus) {
                MLOGI(YELLOW" PQDebug is running, can not start the rec\n\n"NONE);
...
//#ifdef CONFIG_PQT_SUPPORT_ON
        case HI_EVENT_STATEMNG_PQDEBUG:
            {
                if (pstStateAttr->bRunning == HI_TRUE) {
