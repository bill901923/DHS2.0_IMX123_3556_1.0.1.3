...
{
    HI_S32 s32Ret = HI_SUCCESS;
    MLOGD(YELLOW"process message [%d] file name(%s)"NONE"\n", pstMsg->what, pstMsg->aszPayload);
    if(HI_EVENT_RECMNG_FILE_END == pstMsg->what ||
       HI_EVENT_PHOTOMNG_FILE_END == pstMsg->what ||
       HI_EVENT_RECMNG_MANUAL_SPLIT_FILE_END == pstMsg->what) {
        s32Ret =  HI_FILEMNG_AddFile(pstMsg->aszPayload);
    ...
    if (HI_EVENT_RECMNG_MANUAL_SPLIT_FILE_END == pstMsg->what) {
        s32Ret = HI_FILEMNG_RenameEmrFile(pstMsg->aszPayload);
        if (HI_FILEMNG_ENOTMAIN == s32Ret) {
            MLOGD("NotMainFileName[%s]\n", pstMsg->aszPayload);
        } else {
            PDT_STATEMNG_CHECK_RET_WITH_UNRETURN(s32Ret,"filemng rename emr file");
        }
    }
    /** no need publish result event and reset g_bStateMngInProgress to HI_FALSE */
    return HI_PROCESS_MSG_RESULTE_OK;
...
static HI_S32 PDT_STATEMNG_BaseStatePreviewPIPProc(HI_MESSAGE_S *pstMsg)
{
    ...
    s32Ret = HI_PDT_MEDIA_VideoReset(&stMediaCfg, bCompress);
    if(pstStateMngCtx->bScreenDormantStatus != HI_TRUE)
    {
        s32Ret |= HI_PDT_MEDIA_VideoOutStart(NULL,NULL);
    }
    if (s32Ret)
...
{
    HI_S32 ret;
    HI_SYSTEM_STARTUP_SRC_E startupSrc = HI_SYSTEM_STARTUP_SRC_STARTUP;
    HI_SYSTEM_GetStartupWakeupSource(&startupSrc);
    if(startupSrc != HI_SYSTEM_STARTUP_SRC_WAKEYP) {
        /** init usb ctrl module*/
        ret = HI_PDT_USBCTRL_Init();
        PDT_STATEMNG_CHECK_RET_WITH_UNRETURN(ret,"init usb ctrl module");
    }
    return HI_SUCCESS;
}
...
HI_S32 PDT_STATEMNG_BaseStateExit(HI_VOID *pvArg)
{
    ...
    HI_SYSTEM_GetStartupWakeupSource(&startupSrc);
    if(startupSrc != HI_SYSTEM_STARTUP_SRC_WAKEYP) {
        ret = HI_PDT_USBCTRL_Deinit();
        PDT_STATEMNG_CHECK_RET_WITH_UNRETURN(ret,"deinit usb ctrl module");
    }
    return HI_SUCCESS;
}
