...
HI_S32 HI_PDT_INIPARAM_LoadDevMngConf(PDT_PARAM_DEVMNG_CFG_S *pstParam)
{
    ...
    snprintf(szIniNodeName, PDT_INIPARAM_NODE_NAME_LEN, "pqtest:enable");
    s32Ret = HI_CONFACCESS_GetInt(PDT_INIPARAM, szIniModuleName, szIniNodeName, 0, (HI_S32 *)&pstParam->u8pqenb);
    PDT_INIPARAM_CHECK_LOAD_RESULT(s32Ret, szIniNodeName);
    printf("pqtest u8pqenb[%d]\n", pstParam->u8pqenb);
