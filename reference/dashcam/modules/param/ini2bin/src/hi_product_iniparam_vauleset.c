...
HI_S32 HI_PDT_INIPARAM_LoadValueSetConf(PDT_PARAM_CONFITEM_VALUESET_S *pstParam)
{
    ...
    for(i = 0;i < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT;i++)
    {
        ...
        /* Load cam antiflicker mode */
        snprintf(szIniNodePrefix, PDT_INIPARAM_NODE_NAME_LEN, "cam.%d.antiflicker.mode",i);
        s32Ret = PDT_INIPARAM_VALUESET_LoadItemValues(szIniModuleName, szIniNodePrefix, &pstParam->astAntiflickerValue[i]);
        PDT_INIPARAM_CHECK_LOAD_RESULT(s32Ret, szIniNodePrefix);
    }
