...
#define FFMPEG_DEMUXER  (HI_APP_LIB_PATH"/libffmpeg_demuxer.so")
    HI_VOID* pLibhandle = dlopen(FFMPEG_DEMUXER,RTLD_LAZY|RTLD_LOCAL);
    if(pLibhandle==NULL)
    {
        MLOGE("dlopen(%s) fail.\n",FFMPEG_DEMUXER);
        return;
    ...
    s_stNETCTRLFFmpegDemuxerOps.stFmtFun.fmt_open = dlsym(pLibhandle, "HI_FFMPEG_Open");
    ...
    s_stNETCTRLFFmpegDemuxerOps.stFmtFun.fmt_close = dlsym(pLibhandle, "HI_FFMPEG_Close");
    ...
    s_stNETCTRLFFmpegDemuxerOps.stFmtFun.fmt_probe = dlsym(pLibhandle, "HI_FFMPEG_Probe");
    ...
    s_stNETCTRLFFmpegDemuxerOps.stFmtFun.fmt_getinfo = dlsym(pLibhandle, "HI_FFMPEG_Getinfo");
...
//#ifdef CONFIG_PQT_SUPPORT_ON
        {"STATEMNG_PQ_DEBUG",         HI_EVENT_STATEMNG_PQDEBUG},
        {"STATEMNG_SCENE_AUTO",       HI_EVENT_STATEMNG_SCENE_AUTO},
//#endif
    };
...
HI_S32 PDT_NETCTRL_GetParamTypeValueByDesc(HI_CHAR* pDesc, HI_S32* ps32ParamType)
{
    ...
    PDT_NETCTRL_ITEM_VALUE_S astParamTypeValueSet[ ] = {
        ...
        {PDT_NETCTRL_PARAM_TYPE_ANTIFLICKER,        HI_PDT_PARAM_TYPE_ANTIFLICKER},
        {PDT_NETCTRL_PARAM_TYPE_PQTOOL,            DHS_PARAM_TYPE_PQTOOL},
    };
...
HI_S32 PDT_NETCTRL_GetSimulationEventEnum(const HI_CHAR* enumStr, HI_S32* simulationEvent)
{
    if (!HI_APPCOMM_STRCMP_ENUM(enumStr, HI_EVENT_VIDEODETECT_MD_MOTIONSTATE_CHANGE)) {
        *simulationEvent = HI_EVENT_VIDEODETECT_MD_MOTIONSTATE_CHANGE;
    } else if (!HI_APPCOMM_STRCMP_ENUM(enumStr, HI_EVENT_GSENSORMNG_COLLISION)) {
        *simulationEvent = HI_EVENT_GSENSORMNG_COLLISION;
    } else if (!HI_APPCOMM_STRCMP_ENUM(enumStr, HI_EVENT_AHDMNG_PLUG_STATUS)) {
        *simulationEvent = HI_EVENT_AHDMNG_PLUG_STATUS;
    } else {
        MLOGE("Invalid simulationEvent[%s]\n", enumStr);
        return HI_FAILURE;
    }

    MLOGD("simulationEvent[%s] Enum[%d]\n", enumStr, *simulationEvent);
    return HI_SUCCESS;
}
