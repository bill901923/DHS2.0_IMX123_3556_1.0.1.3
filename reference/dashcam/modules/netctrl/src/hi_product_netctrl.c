...
#define PDT_NETCTRL_ARG_OPT(opt, type, valRegion, helpMsg, value, valueBufSize) \
    {\
        (HI_CHAR*)opt, type, (HI_CHAR*)valRegion, HI_FALSE, {0}, \
        (HI_CHAR*)(helpMsg), (HI_VOID*)(value), valueBufSize \
    }
...
static HI_S32 PDT_NETCTRL_EnableUsb(HI_VOID)
{
    ...
    ret = HI_PDT_USBCTRL_Pause(HI_FALSE);
    return ret;
}
...
static HI_S32 PDT_NETCTRL_DisableUsb(HI_VOID)
{
    ...
    HI_PDT_STATEMNG_WORKMODE_STATE_S workModeState = {};
    HI_S32 ret = HI_PDT_STATEMNG_GetState(&workModeState);
    HI_APPCOMM_CHECK_RETURN_WITH_ERRINFO(ret, ret,"Get Statemng State");

    if((workModeState.enWorkMode==HI_PDT_WORKMODE_UVC)||(workModeState.enWorkMode==HI_PDT_WORKMODE_USB_STORAGE)) {
        MLOGE("usb pause failed!\n");
        return HI_FAILURE;
    }

    s32Ret = HI_PDT_USBCTRL_Pause(HI_TRUE);
...
HI_S32 PDT_NETCTRL_EventToString(HI_EVENT_S* pstEvent, HI_CHAR* pszBuffer, HI_U32 u32BufLen)
{
    ...
    switch (pstEvent->EventID)
    {
        ...
        case HI_EVENT_STORAGEMNG_DEV_ERROR:
        {
            ...
            if (stEvent.arg1 >= PDT_NETCTRL_STORAGE_MAX_ERRCNT){
...
static HI_VOID PDT_NETCTRL_CheckClientConnect(HI_VOID *privData, struct timeval *now)
{
    HI_S32 s32Retry = 0;
    HI_S32 i = 0;
    HI_S32 s32SockFd = -1;
...
//#ifdef CONFIG_PQT_SUPPORT_ON
        HI_EVENT_STATEMNG_PQDEBUG,
        HI_EVENT_STATEMNG_SCENE_AUTO,
//#endif
    };
...
static HI_S32 PDT_NETCTRL_UnRegisterClient(HI_CHAR* pszIpAddr)
{
    ...
    for (i = 0; i < NETCTRL_MAX_WEBSERVER_CLIENT_NUM; i++)
    {
        if (0 == strncmp(s_stNETCTRLClient[i].szIPv4Addr, pszIpAddr, sizeof(s_stNETCTRLClient[i].szIPv4Addr)))
        {
            ...
            PDT_NETCTRL_CheckClientConnect(NULL,NULL);
            stEvent.EventID = HI_EVENT_NETCTRL_CLIENT_DISCONNECTED;
            PDT_NETCTRL_PublishEvent(&stEvent);
        }
    }
...
HI_S32 PDT_NETCTRL_SetPQDebug(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    HI_S32  result = HI_SUCCESS;
    HI_S32  respResult = HI_FAILURE;
    HI_CHAR act[HISNET_MAX_ACT_LEN] = {0};
    HI_CHAR value[PDT_NETCTRL_ITEM_VALUE_DESCRIPTION_LEN] = {0};
    ...
    ARG_OPT_S opts[] = {
        ...
        {
            (HI_CHAR*)"act",     ARG_TYPE_MUST | ARG_TYPE_NO_OPT | ARG_TYPE_STRING, (HI_CHAR*)"get|set", HI_FALSE, {0},
            (HI_CHAR*)ARG_S4"print help msg\r\n", (HI_VOID*)(act), HISNET_MAX_ACT_LEN
        },
        {
            (HI_CHAR*)"pqdebug",   ARG_TYPE_MUST | ARG_TYPE_NO_OPT | ARG_TYPE_STRING, HI_NULL, HI_FALSE, {0},
            (HI_CHAR*)ARG_S4"print help msg\r\n", (HI_VOID*)(value), sizeof(value)
        },
        {
            (HI_CHAR*)"END",     ARG_TYPE_END, NULL, HI_FALSE, {0},
            (HI_CHAR*)ARG_S4"END\n", NULL, 0
    ...
    if ( Hisnet_Server_ARG_Parser(argc, pargv, opts) != HI_SUCCESS) {
        pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_ARGPARSER_ERROR);
        return HI_FAILURE;
    }

    if (Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts) == HI_SUCCESS) {
        if (strncmp(act, "set", sizeof("set")) == 0) {
            HI_PDT_STATEMNG_WORKMODE_STATE_S workModeState = {};
            result = HI_PDT_STATEMNG_GetState(&workModeState);
            if (workModeState.bStateMngInProgress) {
                pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                return HI_FAILURE;
            }

            HI_BOOL state = HI_FALSE;
            if (strncmp(PDT_NETCTRL_OFF, value, strnlen(value, sizeof(value))) == 0) {
                state = HI_FALSE;
            }
            else if (strncmp(PDT_NETCTRL_ON, value, strnlen(value, sizeof(value))) == 0) {
                state = HI_TRUE;
            } else {
                pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                return HI_FAILURE;
            ...
            if (workModeState.bRunning) {
                message.what = HI_EVENT_STATEMNG_STOP;
                message.arg1 = HI_TRUE;
                message.arg2 = workModeState.enWorkMode;
                result = PDT_NETCTRL_SendSyncMsg(&message, &respResult);
                if (HI_SUCCESS != result || HI_SUCCESS != respResult) {
                    pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                    return HI_FAILURE;
                }
            }

            memset(&message, 0, sizeof(HI_MESSAGE_S));
            message.what = HI_EVENT_STATEMNG_PQDEBUG;
            message.arg2 = workModeState.enWorkMode;
            HI_CHAR* payload = (message.aszPayload);
            *(HI_BOOL*)payload = (HI_BOOL)state;

            result = PDT_NETCTRL_SendSyncMsg(&message, &respResult);
            if (respResult != HI_SUCCESS || result != HI_SUCCESS) {
                pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                return HI_FAILURE;
            }
            pfnStringProc(pvData, "Success\r\n");
...
HI_VOID PDT_NETCTRL_SetPQDebugReg(HI_VOID)
{
    cgi_cmd_trans_s cgiCmd;
    snprintf(cgiCmd.cgi,MAX_CGI_INTER,"setpqdebug.cgi");
    snprintf(cgiCmd.cmd,MAX_CMD_INTER,"setpqdebug&-act=set&");
    snprintf(cgiCmd.method,METHOD_LEN, CGI_METHOD_SET);
    cgiCmd.cmdProc = PDT_NETCTRL_SetPQDebug;
    HISNET_RegisterCmd(&cgiCmd);
}

HI_S32 PDT_NETCTRL_SetSceneAuto(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    HI_S32  result     = HI_SUCCESS;
    HI_S32  respResult = HI_FAILURE;
    HI_CHAR act[HISNET_MAX_ACT_LEN] = {0};
    ...
    ARG_OPT_S opts[] = {
        ...
        {
            (HI_CHAR*)"sceneauto",   ARG_TYPE_MUST | ARG_TYPE_NO_OPT | ARG_TYPE_STRING, HI_NULL, HI_FALSE, {0},
            (HI_CHAR*)ARG_S4"print help msg\r\n", (HI_VOID*)(value), sizeof(value)
        },
        {
            (HI_CHAR*)"END",     ARG_TYPE_END, NULL, HI_FALSE, {0},
    ...
    if (Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts) == HI_SUCCESS) {
        if (strncmp(act, "set", sizeof("set")) == 0) {
            ...
            memset(&message, 0, sizeof(HI_MESSAGE_S));
            message.what = HI_EVENT_STATEMNG_SCENE_AUTO;
            message.arg2 = workModeState.enWorkMode;
            HI_CHAR* payload = (message.aszPayload);
            *(HI_BOOL*)payload = (HI_BOOL)state;
...
HI_VOID PDT_NETCTRL_SetSceneAutoReg(HI_VOID)
{
    cgi_cmd_trans_s cgiCmd;
    snprintf(cgiCmd.cgi,MAX_CGI_INTER,"setsceneauto.cgi");
    snprintf(cgiCmd.cmd,MAX_CMD_INTER,"setsceneauto&-act=set&");
    snprintf(cgiCmd.method,METHOD_LEN, CGI_METHOD_SET);
    cgiCmd.cmdProc = PDT_NETCTRL_SetSceneAuto;
    HISNET_RegisterCmd(&cgiCmd);
}
...
HI_S32 PDT_NETCTRL_GetCamNum(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    ...
    ret = Hisnet_Server_ARG_Parser(argc, pargv, opts);
...
#else
            HI_BOOL camEnable = HI_FALSE;
            HI_PDT_STATEMNG_WORKMODE_STATE_S workModeState;

            ret = HI_PDT_STATEMNG_GetState(&workModeState);
            if (ret != HI_SUCCESS) {
                pfnStringProc(pvData, (HI_CHAR*)"var camnum=\"1\";\r\n");
                return HI_FAILURE;
            }

            ret = HI_PDT_PARAM_GetCamParam(workModeState.enWorkMode, BACK_CAM_ID, HI_PDT_PARAM_TYPE_CAM_STATUS, &camEnable);
            if (ret != HI_SUCCESS) {
                pfnStringProc(pvData, (HI_CHAR*)"var camnum=\"1\";\r\n");
                return HI_FAILURE;
            }
...
HI_S32 PDT_NETCTRL_GetCamChnl(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    ...
    if (HI_SUCCESS == Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts))
    {
        if (0 == strncmp(_act, "get", sizeof("get")))
        {
            ...
            s32Ret = HI_PDT_PARAM_GetCamParam(stWorkModeState.enWorkMode, camid, HI_PDT_PARAM_TYPE_APK_VENCID, &s32Value);
            if (HI_SUCCESS != s32Ret)
            {
                pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                return HI_FAILURE;
...
HI_S32 PDT_NETCTRL_SetCommonParam(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    ...
    if (HI_SUCCESS == Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts))
    {
        if (0 == strncmp(_act, "set", sizeof("set")))
        {
            ...
            HI_PDT_STATEMNG_WORKMODE_STATE_S stWorkModeState;
            s32Ret = HI_PDT_STATEMNG_GetState(&stWorkModeState);
            if ((((HI_PDT_PARAM_TYPE_AUDIO == enParamType) || (HI_PDT_PARAM_RECORD_TYPE == enParamType) ||
                (HI_PDT_PARAM_TYPE_LAPSE_INTERVAL == enParamType)) && (HI_PDT_WORKMODE_NORM_REC == stWorkModeState.enWorkMode)) ||
                (HI_PDT_PARAM_TYPE_PREVIEWPIP == enParamType))
            {
                if (stWorkModeState.bStateMngInProgress)
                {
                    pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
...
HI_S32 PDT_NETCTRL_SendClickKey(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    ...
    if (HI_SUCCESS == Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts))
    {
        if (0 == strncmp(_act, "set", sizeof("set")))
        {
            ...
            else if (0 == strncasecmp(PDT_NETCTRL_KEY_EXPAND, szKeyType, PDT_NETCTRL_ITEM_VALUE_DESCRIPTION_LEN))
            {
                stEvent.arg1 = HI_KEYMNG_KEY_IDX_3;
            }
...
/* for auto test */
static HI_S32 PDT_NETCTRL_PublishSimulationEvent(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    HI_S32  ret = 0;
    HI_S32  simulationEvevt;
    HI_S32  arg[PDT_NETCTRL_STR_LEN] = {0};
    HI_CHAR act[HISNET_MAX_ACT_LEN] = {0};
    HI_CHAR paramType[EVENT_ID_MAX_LEN] = {0};
    HI_EVENT_S event = {0};

    ARG_OPT_S opts[] = {
        PDT_NETCTRL_ARG_OPT("?",    ARG_TYPE_NO_PARA | ARG_TYPE_SINGLE, HI_NULL, "print help msg\r\n", NULL, 0),
        PDT_NETCTRL_ARG_OPT("act",  ARG_TYPE_MUST | ARG_TYPE_NO_OPT | ARG_TYPE_STRING, "get|set", HI_NULL, act, HISNET_MAX_ACT_LEN),
        PDT_NETCTRL_ARG_OPT("type", ARG_TYPE_MUST | ARG_TYPE_NO_OPT | ARG_TYPE_STRING, HI_NULL, HI_NULL, paramType, sizeof(paramType)),
        PDT_NETCTRL_ARG_OPT("arg1", ARG_TYPE_NO_OPT | ARG_TYPE_INT, HI_NULL, HI_NULL, &arg[0], sizeof(arg[0])),
        PDT_NETCTRL_ARG_OPT("arg2", ARG_TYPE_NO_OPT | ARG_TYPE_INT, HI_NULL, HI_NULL, &arg[1], sizeof(arg[1])),
        PDT_NETCTRL_ARG_OPT("END",  ARG_TYPE_END, NULL, "END\n", HI_NULL, 0),
    };
    ...
    if (Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts) == HI_SUCCESS) {
        if (0 == strncmp(act, "set", sizeof("set"))) {

            ret = PDT_NETCTRL_GetSimulationEventEnum(paramType, &simulationEvevt);
            if (ret != HI_SUCCESS) {
                pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_ARGPARSER_ERROR);
                return HI_FAILURE;
            }

            event.EventID = simulationEvevt;
            event.arg1 = arg[0];
            event.arg2 = arg[1];
            PDT_NETCTRL_PublishEvent(&event);
            pfnStringProc(pvData, "Success\r\n");
        } else {
            pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_CMD_INVALID);
            return HI_FAILURE;
...
static HI_VOID PDT_NETCTRL_PublishSimulationEventReg(HI_VOID)
{
    cgi_cmd_trans_s stCgiCmd;
    snprintf(stCgiCmd.cgi,MAX_CGI_INTER, "publishsimulationevent.cgi");
    snprintf(stCgiCmd.cmd,MAX_CMD_INTER, "publishsimulationevent&-act=set&");
    snprintf(stCgiCmd.method,METHOD_LEN, CGI_METHOD_GET);
    stCgiCmd.cmdProc = PDT_NETCTRL_PublishSimulationEvent;
    HISNET_RegisterCmd(&stCgiCmd);
}
...
static HI_S32 PDT_NETCTRL_DeletTmpUpgradePkt(HI_VOID)
{
    /*  if have old upgrade pkt*/
    HI_CHAR* nameSuffix = NULL;
    HI_S32 ret;
    DIR* dp;
    struct dirent* file;
    struct stat sb;
    ...
    /* search upgrad file */
    HI_STORAGEMNG_CFG_S storageCfg = {0};
    ret = HI_PDT_PARAM_GetStorageCfg(&storageCfg);
    HI_APPCOMM_CHECK_RETURN_WITH_ERRINFO(ret, HI_FAILURE, "HI_PDT_PARAM_GetStorageCfg");

    if (!(dp = opendir(storageCfg.szMntPath))) {
        MLOGE("error opendir %s!!!\n", storageCfg.szMntPath);
        return HI_FAILURE;
    }

    while ((file = readdir(dp)) != NULL) {
        HI_CHAR filePath[HI_UPGRADE_PKT_PATH_MAX_LEN] = {0};
        snprintf(filePath, sizeof(filePath), "%s/%s", storageCfg.szMntPath, file->d_name);
        stat(filePath, &sb);
        ...
        if (S_ISREG(sb.st_mode)) {
            /* file head is upgrade */
            ret = strncmp(file->d_name, HI_UPGRADE_PKT_PREFIX, prefixStrLen);

            if (ret == HI_SUCCESS) {
                nameSuffix = strrchr(file->d_name, '.');

                if (NULL == nameSuffix) {
                    continue;
                }

                /* file rear is ".sw01" || ".appsw01" in sd card */
                if ((strncasecmp(nameSuffix + 1, "appsw01", strnlen("appsw01", HI_UPGRADE_PKT_PATH_MAX_LEN)) == 0) ||
                    (strncasecmp(nameSuffix + 1, "sw01", strnlen("sw01", HI_UPGRADE_PKT_PATH_MAX_LEN)) == 0)) {
                    MLOGI("file->d_name:%s\n", filePath);
                    ret = remove(filePath);

                    if (ret != HI_SUCCESS) {
                        MLOGW("remove[%s] failed\n", filePath);
                    }
                }
            }
        }
...
static HI_S32 PDT_NETCTRL_UpgradeReceivePktInfo(const HI_CHAR* pszPath, HI_U32 u32Offset, HI_U32 u32UnitLen, FILE** pPktFP)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 s32Count = 0;
    MLOGD("\n");
...
static HI_S32 PDT_NETCTRL_UpgradeReceivePkt(const HI_U8* pszRecvBuf, HI_U32 u32BufLen, FILE* pPktFP)
{
    HI_S32 s32Ret;
    ...
    event.EventID = HI_EVENT_NETCTRL_RECEIVE_PKT;
    event.arg1 = (s_stReceiveInfo[s32Count].u32OffSet * 100 / s_stReceiveInfo[s32Count].u32PktLen);
...
static HI_S32 PDT_NETCTRL_SendSyncSwitchWorkModeMessage(HI_PDT_WORKMODE_E mode)
{
    HI_S32 ret = HI_FAILURE;
    HI_MESSAGE_S message = {0};

    memset(&message, 0, sizeof(HI_MESSAGE_S));
    message.what = HI_EVENT_STATEMNG_SWITCH_WORKMODE;
    message.arg2 = mode;
    ...
    HI_S32 retryTime = 20;
    HI_PDT_STATEMNG_WORKMODE_STATE_S workModeState = {0};
    HI_usleep(500 * 1000);
    while (retryTime--) {
        ret = HI_PDT_STATEMNG_GetState(&workModeState);
        if (ret == HI_SUCCESS && workModeState.enWorkMode == mode) {
            if(mode == HI_PDT_WORKMODE_UPGRADE) {
                MLOGI(RED"---------------- UPGRADE workmode Set\n"NONE);
            } else if(mode == HI_PDT_WORKMODE_NORM_REC) {
                MLOGI(RED"---------------- NORM REC workmode Set\n"NONE);
            } else {
                MLOGI(RED"---------------- %d workmode Set\n"NONE, mode);
            }
            break;
        } else {
            HI_usleep(500 * 1000);
    ...
    if(retryTime == 0) {
        MLOGE("switch workmode failed\n");
        return HI_FAILURE;
    } else {
        MLOGI("switch workmode success\n");
        return HI_SUCCESS;
    }
}
...
HI_VOID* PDT_NETCTRL_TIMEOUT_CHECK(HI_VOID* pArg)
{
    ...
    while (s_stReceiveInfo[s32Count].bEnable)
    {
        if (s_stReceiveInfo[s32Count].u32PktLen <= s_stReceiveInfo[s32Count].u32OffSet)
        {
            s_stReceiveInfo[s32Count].u32OffSet = 0;
        ...
        if (PDT_UPGRADE_MAX_WAITCOUNT == s32TimeoutFlag)
        {
            ...
            s32Ret = HI_PDT_STATEMNG_SendMessage(&stMessage);
            if(s32Ret != HI_SUCCESS) {
                MLOGE("HI_PDT_STATEMNG_SendMessage\n");
            }
            s32Ret = PDT_NETCTRL_SendSyncSwitchWorkModeMessage(HI_PDT_WORKMODE_NORM_REC);
            if(s32Ret != HI_SUCCESS) {
                MLOGW("Switch to NORM_REC failed\n");
            }
            break;
        }
    }
    s_stReceiveInfo[s32Count].pthreadFlag = HI_FALSE;
...
HI_S32 PDT_NETCTRL_CheckUpgradePktInfo(HI_S32 argc, const HI_CHAR* pargv[], PfnCgiStringProc pfnStringProc, HI_VOID* pvData)
{
    ...
    if (HI_SUCCESS == Hisnet_Server_ARG_OptIsSet((HI_CHAR*)"act", opts))
    {
        if (0 == strncmp(_act, "get", sizeof("get")))
        {
            ...
            if(s_stReceiveInfo[0].u32OffSet == 0){
                 PDT_NETCTRL_DeletTmpUpgradePkt();
            }else if(strcasecmp(szSoftVersion, s_stReceiveInfo[0].stDevInfo.szSoftVersion) != 0){
                PDT_NETCTRL_DeletTmpUpgradePkt();
            }

            HI_THTTPD_RegisterUploadFileProc(PDT_NETCTRL_UpgradeReceivePktInfo, PDT_NETCTRL_UpgradeReceivePkt);
            ...
            if (HI_PDT_WORKMODE_UPGRADE != stWorkModeState.enWorkMode)
            {
                ...
                if(stWorkModeState.enWorkMode == HI_PDT_WORKMODE_NORM_REC)
                {
                ...
                s32Ret = PDT_NETCTRL_SendSyncSwitchWorkModeMessage(HI_PDT_WORKMODE_UPGRADE);
                if(s32Ret != HI_SUCCESS) {
                    MLOGE("switch to upgrade failed\n");
                    pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                    return HI_FAILURE;
                }

                s32Ret = pthread_create(&s_stReceiveInfo[s32Count].timeOutTaskID, NULL, PDT_NETCTRL_TIMEOUT_CHECK, (HI_VOID *)&s32Count);
                if(s32Ret == HI_SUCCESS) {
                    MLOGI("Timeout task created success\n");
                    s_stReceiveInfo[s32Count].pthreadFlag = HI_TRUE;
                } else {
                    MLOGI("Timeout task created failed\n");
                    pfnStringProc(pvData, (HI_CHAR*)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_RTN_ERROR);
                    s_stReceiveInfo[s32Count].pthreadFlag = HI_FALSE;
                }
                pfnStringProc(pvData, (HI_CHAR*)"var offset=\"%d\";\r\n"
                                                "var unitsize=\"%d\";\r\n",
                              s_stReceiveInfo[s32Count].u32OffSet, s_stReceiveInfo[s32Count].u32UnitSize);
            }
        } else {
            pfnStringProc(pvData, (HI_CHAR *)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_CMD_INVALID);
            return HI_FAILURE;
...
static HI_S32 PDT_NETCTRL_CancelUpgrade(HI_S32 argc, const HI_CHAR* argv[], PfnCgiStringProc cgiStringProcFn, HI_VOID *data)
{
    HI_CHAR act[HISNET_MAX_ACT_LEN] = {0};
    ...
    HI_S32 ret = Hisnet_Server_ARG_Parser(argc, argv, opts);
    if (ret != HI_SUCCESS) {
        cgiStringProcFn(data, (HI_CHAR *)"SvrFuncResult=\"%d\"\r\n", HISNET_SVR_ARGPARSER_ERROR);
        return HI_FAILURE;
    }

    if (Hisnet_Server_ARG_OptIsSet((HI_CHAR *) "act", opts) == HI_SUCCESS) {
        if (strncmp(act, "set", HISNET_MAX_ACT_LEN) == 0) {
            HI_S32 count = 0;
            if(s_stReceiveInfo[count].bEnable == HI_FALSE) {
                MLOGI(GREEN"no need cancel upgrade\n"NONE);
                cgiStringProcFn(data, "Success\r\n");
                return HI_SUCCESS;
            }
            for(count=0; count<NETCTRL_MAX_RECEIVE_COUNT && s_stReceiveInfo[count].bEnable; count++) {
                s_stReceiveInfo[count].pFileFP = NULL;
                s_stReceiveInfo[count].u32OffSet = 0;
                s_stReceiveInfo[count].bEnable = HI_FALSE;
                s_stReceiveInfo[count].u32RecvBufLen = 0;
                if (s_stReceiveInfo[count].pthreadFlag == HI_TRUE) {
                    pthread_join(s_stReceiveInfo[count].timeOutTaskID, NULL);
                    s_stReceiveInfo[count].pthreadFlag = HI_FALSE;
                }
                HI_EVENT_S event;
                memset(&event, 0, sizeof(event));
                event.EventID = HI_EVENT_NETCTRL_RECEIVE_PKT;
                event.arg1 = -1;
                HI_EVTHUB_Publish(&event);
                MLOGI(GREEN"cancel upgrade\n"NONE);

                ret = PDT_NETCTRL_SendSyncSwitchWorkModeMessage(HI_PDT_WORKMODE_NORM_REC);
                if (ret != HI_SUCCESS) {
                    cgiStringProcFn(data, (HI_CHAR *) "SvrFuncResult=\"%d\"\r\n", HISNET_SVR_CMD_INVALID);
                    return HI_FAILURE;
                }
...
static HI_VOID PDT_NETCTRL_CancelUpgradeReg(HI_VOID)
{
    cgi_cmd_trans_s cgiCmd;
    snprintf(cgiCmd.cgi, MAX_CGI_INTER, "cancelupgrade.cgi");
    snprintf(cgiCmd.cmd, MAX_CMD_INTER, "cancelupgrade&-act=set&");
    snprintf(cgiCmd.method, METHOD_LEN, CGI_METHOD_GET);
    cgiCmd.cmdProc = PDT_NETCTRL_CancelUpgrade;
    HISNET_RegisterCmd(&cgiCmd);
}
...
static HI_VOID PDT_NETCTRL_CmdReg(HI_VOID)
{
    ...
    PDT_NETCTRL_PublishSimulationEventReg();
...
//#ifdef CONFIG_PQT_SUPPORT_ON
    PDT_NETCTRL_SetPQDebugReg();
    PDT_NETCTRL_SetSceneAutoReg();
//#endif
}
