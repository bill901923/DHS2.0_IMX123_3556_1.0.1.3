...
#define LOGO_SIZE_S           (86)
    ...
    {
        HI_PDT_RES_GetData(HI_PDT_RES_PURP_OSDLOGO_MAIN, &osdLogo, NULL);
    }
    else if (pstBitmap->u32Width >= LOGO_SIZE_S)
    {
        HI_PDT_RES_GetData(HI_PDT_RES_PURP_OSDLOGO_SUB, &osdLogo, NULL);
    }
    else
    {
    ...
    memcpy(&stBitmap.enPixelFormat, osdLogo, sizeof(PIXEL_FORMAT_E));
    osdLogo += sizeof(PIXEL_FORMAT_E);

    memcpy(&stBitmap.u32Width, osdLogo, sizeof(HI_U32));
    osdLogo += sizeof(HI_U32);

    memcpy(&stBitmap.u32Height, osdLogo, sizeof(HI_U32));
    osdLogo += sizeof(HI_U32);
    ...
    pstBitmap->pvData = (HI_VOID*)osdLogo;
    pstBitmap->u64PhyAddr = (HI_U64)(HI_UL)osdLogo;
    return HI_SUCCESS;
}
#endif
        ...
        switch (pstDispAttr->enBindedMod)
        {
            ...
            case HI_OSD_BINDMOD_VENC:
            {
                HI_MAPI_VENC_ATTR_S stStreamAttr;
                 MLOGI("HI_OSD_BINDMOD_VENC  - HI_MAPI_VENC_GetAttr\n");
                s32Ret = HI_MAPI_VENC_GetAttr(pstDispAttr->ChnHdl, &stStreamAttr);
                if (s32Ret != HI_SUCCESS)
                {
                    pstOsd->astDispAttr[s32DispIdx].bShow = HI_FALSE;
...
#else
    HI_MSG_SendSync(0,HI_MSG_MEDIA_INFO_OSD_STOP, NULL, 0, NULL, NULL, 0);
#endif
#endif
}
