...
#define HI_PDT_MEDIA_AO_CHN_MAX_CNT          (2)/**<audio out chnl maximum count */


#define HI_PDT_MEDIA_VB_MAX_POOLS            (8) /**<vb maximum pools count */
#define UI_FILELIST_WINDOW_LEFT_LEN          (100)/**<irreguar screen */

#define HI_PDT_MEDIA_DISP_VGS_BUFFERLEN      (3)/**<disp video layer buffer len when byPassVgs is FAUSE */
...
typedef struct hiPDT_MEDIA_VCAP_DEV_ATTR_S
{
    ...
    VI_INTF_MODE_E    interfaceMode; /* <Interface mode */
    HI_U32            componentMask[VI_COMPMASK_NUM]; /* <Component mask */
    VI_YUV_DATA_SEQ_E dataSeq; /* <Input data sequence (only the YUV format is supported) */
...
typedef struct hiPDT_MEDIA_DISP_WND_CROP_CFG_S
{
    ...
    HI_HANDLE   E_ModHdl;       /**<binded module     handle: vcap-pipe    | vproc-vpss */
    HI_HANDLE   E_ChnHdl;       /**<binded module chn handle: vcap-pipechn | vproc-vpssport */ 
...
typedef struct hiPDT_MEDIA_AO_CFG_S
{
    HI_BOOL   bEnable;
    HI_HANDLE AoHdl;
    HI_PDT_MEDIA_AoChnCfg aoChnCfg[HI_PDT_MEDIA_AO_CHN_MAX_CNT];
