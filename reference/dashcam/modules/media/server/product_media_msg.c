...
static HI_S32 PDT_MSG_MEDIA_byeason_SETCROP_Callback(HI_S32 s32MsgID, const HI_VOID* pvRequest, HI_U32 u32ReqLen, HI_MSG_PRIV_DATA_S* pstPrivData,
        HI_VOID** ppvResponse, HI_U32* pu32RespLen, HI_VOID* pvUserData)
{
    
    return HI_PDT_MEDIA_Crop_byeason((HI_PDT_MEDIA_DISP_WND_CROP_CFG_S*)pvRequest);
}
...
HI_S32 PDT_MEDIA_MSG_RegisterCallback(HI_VOID)
{
    ...
    s32Ret |= HI_MSG_RegisterMsgProc(HI_MSG_MEDIA_byeason_SETCROP, PDT_MSG_MEDIA_byeason_SETCROP_Callback, NULL);
    HI_APPCOMM_CHECK_RETURN(s32Ret, HI_FAILURE);
