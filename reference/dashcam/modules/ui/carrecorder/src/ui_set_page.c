...
#endif
    {PDT_UI_ACTION_SET_ANTIFLICKER,        HI_PDT_PARAM_TYPE_ANTIFLICKER},
};
...
static const PDT_UI_SET_DATA_S s_astRecordMediaCamItems[] = {
    ...
    {PDT_UI_ACTION_SHOW_ANTIFLICKER,      HI_PDT_PARAM_TYPE_ANTIFLICKER,      STRING_ANTIFLICKER,  HI_TRUE},
...
static HI_VOID UI_SET_ShowList(HI_VOID)
{
    ...
    switch(enAction)
    {
        ...
        case PDT_UI_ACTION_SHOW_ANTIFLICKER:
#ifdef CONFIG_MOTIONDETECT_ON
...
static HI_VOID UI_SET_Back(HI_VOID)
{
    ...
    if( 0 == s_u32CurLevel  ||  s_u32CurLevel >= MAX_SET_LEVELS)
    {
        ...
        if(((HI_SUCCESS != s32Ret) ||(-1 == stEvent.arg1)) && PDT_UI_COMM_isMdReady())
        {
           ...
           //by Eason      MLOGE(" PDT_UI_COMM_CheckSd by carrecorder \n");
        ...
        {
            if (s32Ret == HI_SUCCESS && stEvent.arg1 != -1) {
                MLOGI("HI_EVENT_RECMNG_MUXER_CNT_CHANGE arg1[%d] is not -1 , can not start rec!\n", stEvent.arg1);
            } else {
                MLOGI("md is not ready, no need to start rec\n");
            }
        }
        s_u32CurLevel = 0;
...
static HI_VOID UI_SET_OnClickRow(HI_VOID)
{
    ...
    switch(stSetRow.u32Action)
    {
        ...
        case PDT_UI_ACTION_SET_ANTIFLICKER:
#ifdef CONFIG_MOTIONDETECT_ON
        ...
        case PDT_UI_ACTION_SET_PARKING:
            {
                if (stSetRow.u32Action == PDT_UI_ACTION_SET_USB_MODE)
                {
                    ...
                    s32Ret = HI_PDT_USBCTRL_SetMode((HI_USB_MODE_E)stSetRow.s32Value);
...
HI_S32 PDT_UI_SET_OnKeyDown(HIGV_HANDLE WidgetHdl, HI_PARAM wParam, HI_PARAM lParam)
{
    ...
    else if (PDT_UI_KEY_DOWN == wParam)
    {
