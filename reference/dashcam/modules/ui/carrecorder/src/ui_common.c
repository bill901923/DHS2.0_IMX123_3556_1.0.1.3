...
HI_VOID PDT_UI_COMM_ShowAlarm(HI_U32 u32StringID)
{
    ...
    MLOGD("PDT_UI_COMM_ShowAlarm-1. %x \n", u32StringID);
   ...
   // HI_MESSAGE_S stMessage = {};
    ...
    switch (pstEvent->EventID)
    {
        case HI_EVENT_KEYMNG_SHORT_CLICK:
        {
            ...
            switch (pstEvent->arg1)
            {
                case HI_KEYMNG_KEY_IDX_0:
                   // stInputEvent.value = PDT_UI_KEY_OK ;
                    break;                
                case HI_KEYMNG_KEY_IDX_4:
                {
                  MLOGI("HI_EVENT_KEYMNG_SHORT_CLICK!\n");  
                  MLOGI("WorkMode: %u, !\n", stWorkModeState.enWorkMode);
                  MLOGI("stWorkModeState.enWorkMode: %d !\n", stWorkModeState.enWorkMode);
                          ...
                          case 5:
                            {
                                ...
                                if(HI_PDT_WORKMODE_PHOTO == stWorkModeState.enWorkMode)
                                {
                                    ...
                                    /*  	DHS_AWB_SET_S Set_AWB;
                                        ...
                                        Set_AWB.item=1;        
                                        Set_AWB.item=1;
                                        DHS_Set_config_for_AWB(&Set_AWB);
            ...
            MLOGI("stInputEvent.msg %d stInputEvent.value: %d !\n",stInputEvent.msg, stInputEvent.value);
...
*/
                ...
                case HI_KEYMNG_KEY_IDX_2:
                 ...
                 case HI_KEYMNG_KEY_IDX_4:
                    ...
                    MLOGI("HI_EVENT_KEYMNG_LONG_CLICK!\n");
                    MLOGI("WorkMode: %u, !\n", stWorkModeState.enWorkMode);
                    MLOGI("pstEvent->arg2: %d !\n", pstEvent->arg2);
                   switch (pstEvent->arg2)
                    {
                        case 2:    
                                            
                            MLOGI("long click shutdown\n");
...
#define PDT_UI_STORAGE_MAX_ERRCNT 5
            /* not PowerAction  */
            if (pstEvent->arg1 < PDT_UI_STORAGE_MAX_ERRCNT)
            {
                MLOGD("UI receive SD DEV_ERROR count[%d]\n", pstEvent->arg1);
            }
            else
            {
        ...
        case HI_EVENT_STATEMNG_SD_AVAILABLE:
        {
            ...
            if(PDT_UI_WINMNG_WindowIsShow(FILELIST_PAGE)) {
                ...
                HI_APPCOMM_CHECK_EXPR_WITHOUT_RETURN((s32Ret == HI_SUCCESS), "HI_GV_ADM_Sync");
            }
        }
        break;
...
static HI_S32 PDT_UI_UpgradeShowCancel()
{
    HI_S32 ret = PDT_UI_WINMNG_FinishWindow(SD_UPGRADE_WINDOW);
    MLOGI("========================ret=%d\n", ret);
    ret |= PDT_UI_WINMNG_StartWindow(ALARM_WINDOW, HI_TRUE);
    PDT_UI_COMM_ShowAlarm(STRING_UPGRADE_CANCEL);
    return ret;
}

static HI_S32 PDT_UI_UpgradeShowProgress(HI_U8 percent)
{
    if(percent > PDT_UI_UPGRADING_RANGE) {
        percent = PDT_UI_UPGRADING_RANGE;
    }
    HI_CHAR percentString[32] = {};
    snprintf(percentString, sizeof(percentString), "%2d%%", percent);
    MLOGI("upgrade progressing, percent=%d, percentString=%s\n", percent, percentString);
    HI_S32 ret = HI_GV_Widget_SetText(SD_UPGRADE_WINDOW_LABEL_ASK, percentString);
    ret |= HI_GV_Widget_Paint(SD_UPGRADE_WINDOW_LABEL_ASK, NULL);
    if(ret) {
        MLOGE("Hi gv widget Paint failed\n");
        HI_APPCOMM_CHECK_RETURN(ret, ret);
    }

    ret = HI_GV_ProgressBar_SetPos(UPGRADE_PROGRESS, percent);
    ret |= HI_GV_Widget_Paint(UPGRADE_PROGRESS, NULL);

    return ret;
}

static HI_S32 PDT_UI_UpgradeShowReceivingBar(HI_U8 percent)
{
    HI_S32 ret = HI_FAILURE;

    MLOGI("percent=%d\n", percent);
    if (HI_GV_Widget_IsShow(UPGRADE_PROGRESS) == HI_FALSE) {
        if(HI_GV_Widget_IsShow(ALARM_WINDOW) == HI_TRUE) {
            MLOGD("To finish ALARM WINDOW \n");
            ret = HI_GV_Widget_Hide(ALARM_WINDOW);
            HI_APPCOMM_CHECK_RETURN(ret, ret);
        }
        MLOGI("--------------------------Show Receiving progress---------------------\n");
        if(HI_GV_Widget_IsShow(SD_UPGRADE_WINDOW) == HI_FALSE) {
            ret = PDT_UI_WINMNG_StartWindow(SD_UPGRADE_WINDOW, HI_TRUE);
        }
        ret |= HI_GV_Widget_Hide(SD_UPGRADE_CANCEL);
        ret |= HI_GV_Widget_Hide(SD_UPGRADE_CONFIRM);
        ret |= HI_GV_Widget_Hide(SD_UPGRADE_GROUPBOX_TITLE_BELOW);
        HI_APPCOMM_CHECK_RETURN(ret, ret);

        ret = HI_GV_Widget_SetTextByID(SD_UPGRADE_WINDOW_LABEL_TITLE, STRING_UPGRADE_RECEIVING);
        ret |= HI_GV_Widget_Show(SD_UPGRADE_WINDOW_LABEL_TITLE);
        ret |= HI_GV_Widget_Paint(SD_UPGRADE_WINDOW_LABEL_TITLE, NULL);
        HI_APPCOMM_CHECK_RETURN(ret, ret);

        ret = HI_GV_ProgressBar_SetRange(UPGRADE_PROGRESS, 0, PDT_UI_UPGRADING_RANGE);
        HI_APPCOMM_CHECK_RETURN(ret, ret);
        ret = HI_GV_Widget_Show(UPGRADE_PROGRESS);
        HI_APPCOMM_CHECK_RETURN(ret, ret);
    }

    ret = PDT_UI_UpgradeShowProgress(percent);

    return ret;
}

static HI_S32 PDT_UI_UpgradeShowUpgradingBar(HI_U8 percent, HI_BOOL showTitileForce)
{
    HI_S32 ret = 0;

    if(showTitileForce) {
        ret = HI_GV_Widget_SetTextByID(SD_UPGRADE_WINDOW_LABEL_TITLE, STRING_UPGRADING);
        ret |= HI_GV_Widget_Show(SD_UPGRADE_WINDOW_LABEL_TITLE);
        ret |= HI_GV_Widget_Paint(SD_UPGRADE_WINDOW_LABEL_TITLE, NULL);
        HI_APPCOMM_CHECK_RETURN(ret, ret);
    } else if(HI_GV_Widget_IsShow(UPGRADE_PROGRESS) == HI_FALSE) {
        if(HI_GV_Widget_IsShow(ALARM_WINDOW) == HI_TRUE) {
            MLOGD("To finish ALARM WINDOW \n");
            ret = HI_GV_Widget_Hide(ALARM_WINDOW);
            HI_APPCOMM_CHECK_RETURN(ret, ret);
        ...
        if(HI_GV_Widget_IsShow(SD_UPGRADE_WINDOW) == HI_FALSE) {
            MLOGI("--------------------------Show UPGRADE_WINDOW---------------------\n");
            ret = PDT_UI_WINMNG_StartWindow(SD_UPGRADE_WINDOW, HI_TRUE);
            HI_APPCOMM_CHECK_RETURN(ret, ret);
        }
        MLOGI("--------------------------Show UPGRADE PROGRESS---------------------\n");
        ret = HI_GV_Widget_Hide(SD_UPGRADE_CANCEL);
        ret |= HI_GV_Widget_Hide(SD_UPGRADE_CONFIRM);
        ret |= HI_GV_Widget_Hide(SD_UPGRADE_GROUPBOX_TITLE_BELOW);
        HI_APPCOMM_CHECK_RETURN(ret, ret);
...
HI_S32 PDT_UI_COMM_EventProc(HI_EVENT_S* pstEvent)
{
    ...
    switch (pstEvent->EventID)
    {
        ...
        case HI_EVENT_STATEMNG_SD_AVAILABLE:
        case HI_EVENT_STATEMNG_SD_UNAVAILABLE:
                    MLOGE("PDT_UI_COMM_SDEven \n");
            //PDT_UI_COMM_SDEventProc(pstEvent); /*by Eason*/
            break;
...
*/
        ...
        case HI_EVENT_UPGRADE_NEWPKT:
            ...
            if (HI_PDT_WORKMODE_UPGRADE != stWorkModeState.enWorkMode)
            {
                ...
                if (s32Ret == HI_SUCCESS)
                {
                    ...
                    {
                        MLOGD("not support SD upgrade,because have PARKING_COLLISION record\n");
                        return HI_SUCCESS;
                    }
                }
                else
                ...
                {
                    MLOGD("not support SD upgrade in ohter PowerAction mode\n");
                }
            }
            else
            {
        ...
        case HI_EVENT_UPGRADE_PROGRESS:{
            return PDT_UI_UpgradeShowUpgradingBar((HI_U8)pstEvent->arg1, HI_FALSE);
        }/* always return, so no break here */

        case HI_EVENT_STATEMNG_UPGRADE_ABNORMAL:
            MLOGI("ABNORMAL: arg1=%d, arg2=%d\n", pstEvent->arg1, pstEvent->arg2);
        ...
        case HI_EVENT_NETCTRL_RECEIVE_PKT:{
            MLOGI("arg1=%d, arg2=%d\n", pstEvent->arg1, pstEvent->arg2);
            if(pstEvent->arg1 == -1) {
                return PDT_UI_UpgradeShowCancel();
            } else {
                s32Ret = PDT_UI_UpgradeShowReceivingBar((HI_U8)pstEvent->arg1);
                if(pstEvent->arg1 == PDT_UI_UPGRADING_RANGE) {
                    s32Ret = PDT_UI_UpgradeShowUpgradingBar(0, HI_TRUE);
                }
        ...
        case HI_EVENT_NETCTRL_CLIENT_CONNECTED:
        {
            ...
            if(HI_PDT_WORKMODE_NORM_REC == stWorkModeState.enWorkMode)
            {
                if ( (!stWorkModeState.bRunning) && PDT_UI_COMM_isMdReady())
                {
                    ...
                    {
                        HI_S32 nSndCnt = 0;
                        while((HI_PDT_STATEMNG_EINPROGRESS == s32Ret) && (nSndCnt < 1000) &&(PDT_UI_COMM_isMdReady()))
                        {
                            nSndCnt++;
                            hi_usleep(60*1000);
                            s32Ret = HI_PDT_STATEMNG_SendMessage(&stMessage);
