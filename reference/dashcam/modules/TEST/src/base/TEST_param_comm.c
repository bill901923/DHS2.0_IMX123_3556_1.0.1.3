...
#endif
#include "TEST_param_inner.h"
#include "TEST_param.h"
#include "TEST_param_debug.h"
    ...
    HI_PDT_PARAM_SCENE_CAM_VCAP_ATTR_S *pstSceneAttr)
{
    ...
    memcpy(&pstVcapDevAttr->stSnsAttr,&pstVcapSpecAttr->stSnsAttr,sizeof(HI_MAPI_SENSOR_ATTR_S));
    ...
    pstVcapDevAttr->stSnsAttr.stInputDevAttr.inputMode = pstVcapCommCfg->inputMode;
    pstVcapDevAttr->interfaceMode = pstVcapCommCfg->interfaceMode;
    pstVcapDevAttr->dataSeq = pstVcapCommCfg->dataSeq;
    pstVcapDevAttr->inputDataType = pstVcapCommCfg->inputDataType;

    for (s32Idx = 0; s32Idx < VI_COMPMASK_NUM; s32Idx++) {
        pstVcapDevAttr->componentMask[s32Idx] = pstVcapSpecAttr->stVcapDevAttr.au32ComponentMask[s32Idx];
    }
    ...
    HI_PDT_MEDIA_VPSS_ATTR_S *pstVpssAttr)
{
    ...
    pstVpssAttr->stVpssAttr.stNrAttr.enNrType = pstVpssCommAttr->stNrAttr.enNrType;
    ...
    HI_MEDIA_VENC_ATTR_S *pstVencAttr)
{
    ...
    {
        MLOGE("unsupport payload type [%d] \n",pstVencCommAttr->enPayload);
        return HI_PDT_PARAM_EUNSUPPORT;
    }
    return HI_SUCCESS;
}
    ...
    for(i = 0; i < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT;i++)
    {
        if(s32CamID == PDT_PARAM_GetCtx()->pstCfg->astCamMediaCfg[i].s32CamID)
        {
            ...
            if (HI_FALSE == pstSpecMediaCfg->stVcapAttr.astVcapPipeAttr[0].bIspBypass
                && pstSpecMediaCfg->astMode[0].enMode != VI_OFFLINE_VPSS_ONLINE
...
static HI_S32 PDT_PARAM_GetAntiflickerValue(HI_S32 s32CamID, PDT_PARAM_MEDIAMODE_TYPE_E enType,
    HI_PDT_SCENE_ANTIFLICKER_E *penAntiflicker)
{
    if (enType >= PDT_PARAM_MEDIAMODE_TYPE_BUTT)
    {
        MLOGE("Medie Mode type is invalid, get CAMID[%d] With Param Type[Anfilicker] error\n",s32CamID);
        return HI_PDT_PARAM_EINVAL;
    }
    ...
    for(i = 0; i < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT;i++)
    {
        ...
        {
            *penAntiflicker =
                PDT_PARAM_GetCtx()->pstCfg->astCamMediaCfg[i].stMediaCommCfg[enType].stVcapDev.enAnfilicker;
            break;
        }
    }
    if(i >= HI_PDT_MEDIA_VCAP_DEV_MAX_CNT)
    {
        MLOGE("get CAMID[%d] With Param Type[Anfilicker] error \n",s32CamID);
        return HI_PDT_PARAM_EINVAL;
    }
    MLOGD("get CAMID[%d] enAnfilicker[%d] \n",s32CamID,*penAntiflicker);
    return HI_SUCCESS;
}

static HI_S32 PDT_PARAM_SetAntiflickerValue(HI_S32 s32CamID, PDT_PARAM_MEDIAMODE_TYPE_E enType, HI_PDT_SCENE_ANTIFLICKER_E *penAntiflicker)
{
    if (enType >= PDT_PARAM_MEDIAMODE_TYPE_BUTT)
    {
        MLOGE("Medie Mode type is invalid, set CAMID[%d] With Param Type[VideoMode] error\n",s32CamID);
    ...
    if(i >= HI_PDT_MEDIA_VCAP_DEV_MAX_CNT)
    {
        MLOGE("set CAMID[%d] With Param Type[Anfilicker] error \n",s32CamID);
        return HI_PDT_PARAM_EINVAL;
    }
    MLOGD("set CAMID[%d] enAnfilicker[%d] \n",s32CamID, *penAntiflicker);
    return HI_SUCCESS;
}
...
static HI_S32 PDT_PARAM_SetCrop(HI_S32 s32CamID, PDT_PARAM_MEDIAMODE_TYPE_E enType, const HI_PDT_MEDIA_DISP_WND_CROP_CFG_S *pstCropCfg)
{
    ...
    MLOGI("set crop CAMID[%d] Enable[%d] u32CurH[%d] u32MaxH[%d]\n",s32CamID,pstCropCfg->bEnable,pstCropCfg->u32CurH,pstCropCfg->u32MaxH);
    return HI_SUCCESS;
}
                ...
                if(pstParamCfg->astCamMediaCfg[i].astMediaModeSpecCfg[j].enMediaMode == pstMediaModeCfg->enMediaMode)
                {
                    ...
                    for(k = 0;k < pstCamCfg->u32VpssCnt && k < HI_PDT_VCAPDEV_VPSS_MAX_CNT;k++)
                    {
                           ...
                           && pstParamCfg->astCamMediaCfg[i].stMediaCommCfg[enType].stVprocCfg.astVpssAttr[k].bEnable)
                        {
                            ...
                            MLOGD("scene param: VcapPipe[%d], VcapPipeChn[%d] VpssHdl[%d] Vport[%d]\n",pstSceneAttr->VcapPipeHdl,
                    ...
                    for(k = 0;k < pstCamCfg->u32VencCnt && k < HI_PDT_VCAPDEV_VENC_MAX_CNT;k++)
                    {
                            ...
                            && !bFirst && pstParamCfg->astCamMediaCfg[i].stMediaCommCfg[enType].stVencCfg.astVencAttr[k].bEnable)
                        {
                            ...
                            bFirst = HI_TRUE;
                            MLOGD("scene param: VcapPipe[%d], VcapPipeChn[%d] Venc[%d]\n",pstSceneAttr->VcapPipeHdl,
...
HI_S32 HI_PDT_PARAM_GetMediaCommCfg(HI_PDT_PARAM_MEDIA_COMM_CFG_S *pstMediaCommCfg)
{
    ...
    for(i = 0;i < HI_PDT_MEDIA_DISP_MAX_CNT;i++)
    {
        ...
        //pstMediaCommCfg->stVideoOutCfg.astDispCfg[0].astWndCfg[0].enRotate=0;
    /*load acap configure*/
...
HI_S32 HI_PDT_PARAM_GetCamParam(HI_PDT_WORKMODE_E enWorkMode,HI_S32 s32CamID, HI_PDT_PARAM_TYPE_E enType, HI_VOID *pvParam)
{
    ...
    switch(enType)
    {
        ...
        case HI_PDT_PARAM_MD_SENSITIVITY:
            ...
            break;
#endif
        case HI_PDT_PARAM_TYPE_ANTIFLICKER:
            s32Ret = PDT_PARAM_GetAntiflickerValue(s32CamID, enMediaModeType,
                (HI_PDT_SCENE_ANTIFLICKER_E*)pvParam);
            break;
        default:
            HI_MUTEX_UNLOCK(PDT_PARAM_GetCtx()->mutexLock);
...
#endif
        case HI_PDT_PARAM_TYPE_ANTIFLICKER:
            s32Ret = PDT_PARAM_SetAntiflickerValue(s32CamID,enMediaModeType,
                (HI_PDT_SCENE_ANTIFLICKER_E*)pvParam);
            PDT_PARAM_CHECK_RETURN_WITH_UNLOCK(s32Ret, s32Ret);
            break;
        default:
    ...
    switch(enType)
    {
        ...
        case HI_PDT_PARAM_TYPE_BOOTSOUND:
            *(HI_BOOL *)pvParam = PDT_PARAM_GetCtx()->pstCfg->stDevMngCfg.bBootSoundEnable;
            ...
            break;
        case DHS_PARAM_TYPE_PQTOOL:
            *(HI_BOOL *)pvParam = PDT_PARAM_GetCtx()->pstCfg->stDevMngCfg.u8pqenb;
            MLOGI("pqtest Enable[%d]\n\n", *(HI_BOOL*)pvParam);
            break;
...
HI_S32 HI_PDT_PARAM_SetCommParam(HI_PDT_PARAM_TYPE_E enType, const HI_VOID *pvParam)
{
    ...
    switch(enType)
    {
        ...
        case HI_PDT_PARAM_TYPE_BOOTSOUND:
            ...
            break;
        case DHS_PARAM_TYPE_PQTOOL:
            PDT_PARAM_GetCtx()->pstCfg->stDevMngCfg.u8pqenb = *(HI_BOOL *)pvParam;
            MLOGI("pqtest Enable[%d]\n\n", *(HI_BOOL*)pvParam);
            break;
        case HI_PDT_PARAM_TYPE_GSENSOR_SENSITIVITY:
...
HI_S32 HI_PDT_PARAM_GetCamItemValues(HI_PDT_WORKMODE_E enWorkMode, HI_S32 s32CamID, HI_PDT_PARAM_TYPE_E enType, HI_PDT_ITEM_VALUESET_S *pstValueSet)
{
    ...
    switch(enType)
    {
        ...
        case HI_PDT_PARAM_TYPE_ANTIFLICKER:
            memcpy(pstValueSet, &PDT_PARAM_GetCtx()->pstCfg->stItemValues.astAntiflickerValue[s32CamID], sizeof(HI_PDT_ITEM_VALUESET_S));
            break;
        default:
    ...
    for(i = 0;i < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT;i++)
    {
        if(pstCamCtx[i].bEnable)
        {
            ...
            if(j < HI_PDT_SCENE_PIPE_MAX_NUM)
            {
                ...
                if(pstSceneMode->astPipeAttr[j].bBypassIsp) {
                    pstSceneMode->astPipeAttr[j].bBypassVenc = HI_TRUE;
                } else {
                    pstSceneMode->astPipeAttr[j].bBypassVenc = HI_FALSE;
                }
...
static HI_S32 PDT_PARAM_GetVpssIndex(HI_PDT_MEDIA_VPROC_CFG_S* vprocCfg,
...
static HI_S32 PDT_PARAM_GetVcapIndex(HI_PDT_MEDIA_VCAP_CFG_S* vcapCfg,
                                     HI_HANDLE vcapPipeHdl, HI_U32* vcapDevIndex, HI_U32* vcapPipeIndex)
{
    HI_S32 i = 0;
    HI_S32 j = 0;
    for (i = 0; i < HI_PDT_MEDIA_VCAP_DEV_MAX_CNT; ++i) {
        if (!vcapCfg->astVcapDevAttr[i].bEnable) {
            continue;
        }
        for (j = 0; j < HI_PDT_MEDIA_VCAPDEV_PIPE_MAX_CNT; ++j) {
            if ((vcapCfg->astVcapDevAttr[i].astVcapPipeAttr[j].bEnable)
                && (vcapCfg->astVcapDevAttr[i].astVcapPipeAttr[j].VcapPipeHdl == vcapPipeHdl)) {
                *vcapDevIndex = i;
                *vcapPipeIndex = j;
                return HI_SUCCESS;
            }
    ...
    MLOGE(" vcapPipeHdl[%d]is not existed\n", vcapPipeHdl);
    return HI_FAILURE;
}


static HI_BOOL PDT_PARAM_IsVcapDevBindedVenc(HI_PDT_MEDIA_CFG_S *pstMediaCfg, HI_S32 vencIndex, HI_S32 vcapDevIndex)
{
    if (!pstMediaCfg->stVideoCfg.stVcapCfg.astVcapDevAttr[vcapDevIndex].bEnable ||
        !pstMediaCfg->stVideoCfg.astVencCfg[vencIndex].bEnable) {
        return HI_FALSE;
    }

    if (pstMediaCfg->stVideoCfg.astVencCfg[vencIndex].enBindedMod == HI_PDT_MEDIA_VIDEOMOD_VCAP) {
        if (pstMediaCfg->stVideoCfg.stVcapCfg.astVcapDevAttr[vcapDevIndex].VcapDevHdl ==
            pstMediaCfg->stVideoCfg.astVencCfg[vencIndex].ModHdl) {
            return HI_TRUE;
        } else {
            return HI_FALSE;
        }
    ...
    /**Venc Binded Vpss*/
    HI_HANDLE vpssHdl = pstMediaCfg->stVideoCfg.astVencCfg[vencIndex].ModHdl;
    HI_HANDLE vportHdl = pstMediaCfg->stVideoCfg.astVencCfg[vencIndex].ChnHdl;
    HI_U32 vpssIndex  = 0;
    HI_U32 vportIndex = 0;
    PDT_PARAM_GetVpssIndex(&pstMediaCfg->stVideoCfg.stVprocCfg, vpssHdl, vportHdl, &vpssIndex, &vportIndex);

    HI_HANDLE vcapPipeHdl = pstMediaCfg->stVideoCfg.stVprocCfg.astVpssAttr[vpssIndex].VcapPipeHdl;
    HI_U32 currentVcapDevIndex  = 0;
    HI_U32 currentVcapPipeIndex = 0;
    PDT_PARAM_GetVcapIndex(&pstMediaCfg->stVideoCfg.stVcapCfg, vcapPipeHdl, &currentVcapDevIndex, &currentVcapPipeIndex);
    if (currentVcapDevIndex != vcapDevIndex) {
        return HI_FALSE;
    } else {
        return HI_TRUE;
    }
...
HI_S32 HI_PDT_PARAM_RecordUpdateMediaCfg(HI_PDT_MEDIA_CFG_S *pstMediaCfg)
{
    ...
    /** Suppose RecTask index is same as VcapDev Index*/
   for (i = 0; i < HI_PDT_RECTASK_MAX_CNT; i++)
   {
        ...
        if (HI_REC_TYPE_LAPSE == pstRecMngAttr->enRecType)
        {
            if (pstRecMngAttr->u32Interval_ms < 64 * 100)
            {
                for (j = 0; j < HI_PDT_MEDIA_VENC_MAX_CNT; j++)
                {
                    if (!PDT_PARAM_IsVcapDevBindedVenc(pstMediaCfg, j, i)) {
                        continue;
                    }
