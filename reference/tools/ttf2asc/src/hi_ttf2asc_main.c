...
{
    printf("usage: ./ttf2asc xxx.ttf w h output\n");
}
...
static HI_S32 SaveHexFile(const HI_U8* pu8DataBuf,HI_U32 u32DataSize,const HI_CHAR* pszFilePath)
{
    FILE *fp = NULL;
    ...
    if(fwrite(pu8DataBuf, u32DataSize, 1, fp) != 1) {
        fclose(fp);
        MLOGE("fwrite [%s] failed %s\n", pszFilePath,strerror(errno));
        return HI_FAILURE;
    }
    fflush(fp);
    fclose(fp);
...
HI_S32 main(HI_S32 argc, HI_CHAR** argv)
{
    ...
    /*step 4.save hex to file*/
    SaveHexFile(pu8HexData, u32BitmapSize/8, Output);
