...
#define UPGRADE_PKT_APP_SUFFIX "appsw"
...
{
    HI_S32 s32Idx = 0;
    HI_U8 pktType = UPGRADE_IMAGES_BY_UBOOT;
    ...
    pktType = (HI_U8)strtoul(argv[1], NULL, 10);
    if(pktType != UPGRADE_IMAGES_BY_UBOOT && pktType != UPGRADE_IMAGES_BY_APP) {
        MLOGE("Invalid pkt type %d\n", pktType);
        exit(1);
    }
    HI_CHAR szModel[HI_APPCOMM_COMM_STR_LEN];
    HI_CHAR szSoftVersion[HI_APPCOMM_COMM_STR_LEN];
    snprintf(szModel, HI_APPCOMM_COMM_STR_LEN, "%s", argv[2]);
    snprintf(szSoftVersion, HI_APPCOMM_COMM_STR_LEN, "%s", argv[3]);
    ...
    s32Ret = UPGRADE_ParseConfig(argv[4], pstPktHead, &stConfigFileInfo);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    pstPktHead->compress = HI_FALSE;
    pstPktHead->pktType = pktType;
    ...
    if(pktType == UPGRADE_IMAGES_BY_UBOOT) {
        snprintf(szReleasePacketName, HI_UPGRADE_PKT_PATH_MAX_LEN, "./"HI_UPGRADE_PKT_PREFIX"_%s_%s."UPGRADE_PKT_SUFFIX,
                 szModel, szSoftVersion);
    } else {
        snprintf(szReleasePacketName, HI_UPGRADE_PKT_PATH_MAX_LEN, "./"HI_UPGRADE_PKT_PREFIX"_%s_%s."UPGRADE_PKT_APP_SUFFIX,
                 szModel, szSoftVersion);
    }
