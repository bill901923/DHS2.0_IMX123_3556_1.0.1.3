...
static HI_BOOL s_bMSGServerRun2 = HI_FALSE;
static pthread_t s_MSGServerTid2 = 0;
...
static HI_VOID* MSG_SERVER_Run_DHS(HI_VOID* arg)
{
    prctl(PR_SET_NAME, __func__, 0, 0, 0);
    HI_S32 s32Ret = 0;

    while (s_bMSGServerRun2)
    {
...
{
    HI_S32 s32Ret = 0;
    HI_APPCOMM_CHECK_EXPR(0 == s_MSGServerTid2, HI_EINITIALIZED);
     HI_IPCMSG_CONNECT_S  stConnectAttr = {0, DHS_MSG_SRVPORT, 1};
    ...
    pthread_attr_init(&threadattr);
        ...
        threadattr.schedparam.sched_priority = 5;
    #endif
    s_bMSGServerRun2 = HI_TRUE;
    s32Ret = pthread_create(&s_MSGServerTid2, &threadattr, MSG_SERVER_Run_DHS, NULL);
    pthread_attr_destroy(&threadattr);

    if (HI_SUCCESS != s32Ret)
    {
        HI_IPCMSG_DelService(DHS_MSG_SRVNAME);
        s_bMSGServerRun2 = HI_FALSE;
        MLOGE("pthread_create fail:%s\n", strerror(errno));
        return HI_ENORES;
    }
...
HI_S32 DHS_MSG_SERVER_Deinit(HI_VOID)
{
    ...
    pthread_join(s_MSGServerTid2, NULL);
