 ...
 * @date      2017/12/22
 */
HI_S32 HI_MSG_SendASync(HI_S32 setport,HI_S32 s32MsgID, const HI_VOID* pvMsg, HI_U32 u32MsgLen, HI_MSG_PRIV_DATA_S* pstPrivData)
{
    ...
    if(setport)
    {
       lg_s32MsgFd=g_s32MsgFd2;
    }
    else
    {
       lg_s32MsgFd=g_s32MsgFd;
    }
    HI_APPCOMM_CHECK_EXPR(-1 != lg_s32MsgFd, HI_ENOINIT);
    ...
    if (pstReq)
    {
        ...
        s32Ret = HI_IPCMSG_SendAsync(lg_s32MsgFd, pstReq, MSG_RespProc);
 ...
 * @date      2017/12/22
 */
HI_S32 HI_MSG_SendSync(HI_S32 setport,HI_S32 s32MsgID, const HI_VOID* pvRequest, HI_U32 u32ReqLen, HI_MSG_PRIV_DATA_S* pstPrivData, HI_VOID* pvResponse, HI_U32 u32RespLen)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_IPCMSG_MESSAGE_S* pstReq = NULL;
   ...
   //MLOGI("setport %d in HI_MSG_SendSync %d ->%d\n",setport, s32MsgID,lg_s32MsgFd);
    HI_APPCOMM_CHECK_EXPR(-1 != lg_s32MsgFd, HI_ENOINIT);
    ...
    if (pstReq)
    {
        ...
        s32Ret = HI_IPCMSG_SendSync(lg_s32MsgFd, pstReq, &pstResp, MSG_SEND_SYNC_TIMEOUT);
        HI_IPCMSG_DestroyMessage(pstReq);
        ...
        if (HI_SUCCESS != s32Ret || NULL == pstResp)
        {
            ...
            MLOGI("setport %d in HI_MSG_SendSync %d ->%d\n",setport, s32MsgID,lg_s32MsgFd);
            MLOGI("HI_EINTER 3 HI_MSG_SendSync %d ->% d\n", s32MsgID,lg_s32MsgFd);
            return HI_EINTER;
        }
        else
        {
            ...
            //MLOGI("Pass HI_MSG_SendSync %d ->% d\n", s32MsgID,lg_s32MsgFd);
            HI_IPCMSG_DestroyMessage(pstResp);
            return s32Ret;
        }
    }

    MLOGI("HI_EINTER 4 HI_MSG_SendSync %d ->% d\n", s32MsgID,lg_s32MsgFd);
    return HI_EINTER;
}
