...
static const HI_CHAR *s_apszLOGLevel[HI_LOG_LEVEL_BUTT] = {
    LIGHT_PURPLE "FATAL" NONE,
    "",
    LIGHT_RED "ERROR" NONE,
    YELLOW "WARN" NONE,
    LIGHT_GREEN "INFO" NONE,
    LIGHT_BLUE "DEBUG" NONE,
};
  ...
  //  if (s_bLogOn)
    {
        ...
        if (s_bLogTime)
        {
            ...
            dprintf(NONE"[%02d:%02d:%02d:%03ld %s-%s]:%s[%d]:", tm.tm_hour, tm.tm_min, tm.tm_sec, tv.tv_usec / 1000, s_apszLOGLevel[enLevel], pszModule, pszFunc, u32Line);
#else
            fprintf(stdout, NONE"[%02d:%02d:%02d:%03ld %s-%s]:%s[%d]:", tm.tm_hour, tm.tm_min, tm.tm_sec, tv.tv_usec / 1000, s_apszLOGLevel[enLevel], pszModule, pszFunc, u32Line);
#endif
        }
        else
        {
#ifdef __HuaweiLite__
            dprintf(NONE"[%s-%s]:%s[%d]:", s_apszLOGLevel[enLevel], pszModule, pszFunc, u32Line);
#else
            fprintf(stdout, NONE"[%s-%s]:%s[%d]:", s_apszLOGLevel[enLevel], pszModule, pszFunc, u32Line);
#endif
        }
