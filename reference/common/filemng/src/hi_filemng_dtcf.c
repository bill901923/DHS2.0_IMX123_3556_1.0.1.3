...
static HI_S32 FILEMNG_AddRecord(const HI_CHAR *pszFilePath, HI_DTCF_DIR_E enDir)
{
    ...
    if (HI_SUCCESS != s32Ret) {
        ...
        if (DTCF_DIR_EMR_FRONT == enDir || DTCF_DIR_EMR_REAR == enDir) {
            MLOGE("please call HI_FILEMNG_RenameEmrFile to add emr record file name.");
            return HI_FILEMNG_EINVAL;
        }
...
static HI_VOID FILEMNG_RenameSubEmrFile(const HI_CHAR *filePath, HI_DTCF_DIR_E dir)
{
    HI_CHAR subFilePath[HI_APPCOMM_MAX_PATH_LEN];
    HI_CHAR subDstFilePath[HI_APPCOMM_MAX_PATH_LEN];
    if (dir == DTCF_DIR_NORM_FRONT &&
            strnlen(s_stFILEMNGDTCF_Cfg.aszDirNames[DTCF_DIR_NORM_FRONT_SUB], HI_DIR_LEN_MAX) > 0 &&
            strnlen(s_stFILEMNGDTCF_Cfg.aszDirNames[DTCF_DIR_EMR_FRONT_SUB], HI_DIR_LEN_MAX) > 0) {
        HI_DTCF_GetRelatedFilePath(filePath, DTCF_DIR_NORM_FRONT_SUB, subFilePath, HI_APPCOMM_MAX_PATH_LEN);
        HI_DTCF_GetEmrFilePath(subFilePath, subDstFilePath, HI_APPCOMM_MAX_PATH_LEN);
        rename(subFilePath, subDstFilePath);
    }
    if (dir == DTCF_DIR_NORM_REAR &&
            strnlen(s_stFILEMNGDTCF_Cfg.aszDirNames[DTCF_DIR_NORM_REAR_SUB], HI_DIR_LEN_MAX) > 0 &&
            strnlen(s_stFILEMNGDTCF_Cfg.aszDirNames[DTCF_DIR_EMR_REAR_SUB], HI_DIR_LEN_MAX) > 0) {
        HI_DTCF_GetRelatedFilePath(filePath, DTCF_DIR_NORM_REAR_SUB, subFilePath, HI_APPCOMM_MAX_PATH_LEN);
        HI_DTCF_GetEmrFilePath(subFilePath, subDstFilePath, HI_APPCOMM_MAX_PATH_LEN);
        rename(subFilePath, subDstFilePath);
    }
}

static HI_S32 FILEMNG_RenameMainEmrFile(const HI_CHAR *filePath, HI_DTCF_DIR_E dir)
{
    HI_S32 ret = 0;
    HI_CHAR srcFileName[HI_APPCOMM_MAX_PATH_LEN];
    HI_DTCF_DIR_E srcDir = DTCF_DIR_BUTT;
    HI_U32 fileAmount = s_u32FILEMNGAllObjCnt;
    for (HI_S32 i = 0; i < s_u32FILEMNGAllObjCnt; i++) {
        ret = HI_DTCF_GetFileByIndex(i, srcFileName, HI_APPCOMM_MAX_PATH_LEN, &srcDir);
        if (ret != HI_SUCCESS) {
            HI_LOG_PrintFuncErr(HI_DTCF_GetFileByIndex, ret);
            return HI_FILEMNG_EINTER;
        }
        if (strncmp(filePath, srcFileName, HI_APPCOMM_MAX_PATH_LEN) == 0 && dir == srcDir) {
            HI_DTCF_DelFileByIndex(i, &fileAmount);
            HI_CHAR dstFileName[HI_APPCOMM_MAX_PATH_LEN];
            HI_DTCF_GetEmrFilePath(filePath, dstFileName, HI_APPCOMM_MAX_PATH_LEN);
            rename(filePath, dstFileName);
            if (dir == DTCF_DIR_NORM_FRONT) {
                HI_DTCF_AddFile(dstFileName, DTCF_DIR_EMR_FRONT);
            } else if (dir == DTCF_DIR_NORM_REAR) {
                HI_DTCF_AddFile(dstFileName, DTCF_DIR_EMR_REAR);
            } else {
                HI_MUTEX_UNLOCK(s_FILEMNGMutex);
                MLOGE("file:%s Dir:%d is not norm record path.\n", filePath, dir);
                return HI_FILEMNG_EINTER;
            }
...
/**
 * @brief    move a file object from norm directory to emr directory.
 * @param[in] filePath:the source file absolute path.
 * @return 0 success,non-zero error code.
 * @exception    None
 * @author    HiMobileCam Reference Develop Team
 * @date      2017/12/12
 */
HI_S32 HI_FILEMNG_RenameEmrFile(const HI_CHAR *filePath)
{
    HI_S32 ret = 0;
    HI_APPCOMM_CHECK_POINTER(filePath, HI_FILEMNG_EINVAL);
    HI_MUTEX_LOCK(s_FILEMNGMutex);
    if (s_bFILEMNGInit == HI_FALSE) {
        HI_MUTEX_UNLOCK(s_FILEMNGMutex);
        MLOGE("%s\n", FILEMNG_Strerror(HI_FILEMNG_ENOTINIT));
        return HI_FILEMNG_ENOTINIT;
    }
    ret = HI_PathIsDirectory(filePath);
    if (ret == 0) {
        HI_DTCF_DIR_E dir = DTCF_DIR_BUTT;
        ret = HI_DTCF_GetFileDirType(filePath, &dir);
        if (ret != HI_SUCCESS) {
            HI_MUTEX_UNLOCK(s_FILEMNGMutex);
            HI_LOG_PrintFuncErr(HI_DTCF_GetFileDirType, ret);
            return HI_FILEMNG_EINVAL;
        }
        if (dir == DTCF_DIR_NORM_FRONT || dir == DTCF_DIR_NORM_REAR) {
            if (s_u32FILEMNGRecObjCnt > 0) {
                FILEMNG_RenameSubEmrFile(filePath, dir);
                ret = FILEMNG_RenameMainEmrFile(filePath, dir);
                HI_MUTEX_UNLOCK(s_FILEMNGMutex);
                return ret;
            } else {
                HI_MUTEX_UNLOCK(s_FILEMNGMutex);
                MLOGE("file:%s was not added.\n", filePath);
                return HI_FILEMNG_EINTER;
            }
        } else if (dir == DTCF_DIR_NORM_FRONT_SUB || dir == DTCF_DIR_NORM_REAR_SUB) {
            HI_MUTEX_UNLOCK(s_FILEMNGMutex);
            return HI_FILEMNG_ENOTMAIN;
        } else {
            HI_MUTEX_UNLOCK(s_FILEMNGMutex);
            MLOGE("Only support move file from norm to emr.\n");
            return HI_FILEMNG_EINTER;
        }
    }
    HI_MUTEX_UNLOCK(s_FILEMNGMutex);
    MLOGE("%s:%s\n", FILEMNG_Strerror(HI_FILEMNG_ELOST), filePath);
    return HI_FILEMNG_ELOST;
}
                                   ...
                                   HI_FILEMNG_OBJ_FILENAME_S *pstFileName)
{
    ...
    MLOGI("Do HI_FILEMNG_GenerateFileName\n");
    MLOGI("---enType= %d\n",enType);
    MLOGI("---enDir= %d\n",enDir);
    HI_S32 s32Ret = 0;
    ...
    if (HI_DTCF_FILE_TYPE_MP4 == enType || HI_DTCF_FILE_TYPE_TS == enType) {
        MLOGI("Do FILEMNG_GenerateRecordName\n");
     ...
     MLOGI("Do FILEMNG_GeneratePhotoName\n");
...
HI_S32 HI_FILEMNG_SetSearchScope(HI_DTCF_DIR_E aenDirs[DTCF_DIR_BUTT], HI_U32 u32DirCount, HI_U32 *pu32FileObjCnt)
{
    ...
    HI_U32 ScopeDirCount = 0;
    HI_DTCF_DIR_E ScopeDirs[DTCF_DIR_BUTT];
    HI_S32 i = 0;
    for (i = 0; i < u32DirCount ; i++) {
        if (aenDirs[i] == DTCF_DIR_EMR_FRONT_SUB ||
            ...
            MLOGD("Ignore substream directories.\n");
        } else {
            ScopeDirs[ScopeDirCount] = aenDirs[i];
            ScopeDirCount++;
        }
    }
    s32Ret = HI_DTCF_Scan(ScopeDirs, ScopeDirCount, pu32FileObjCnt);
    if (HI_SUCCESS != s32Ret) {
        HI_MUTEX_UNLOCK(s_FILEMNGMutex);
        HI_LOG_PrintFuncErr(HI_DTCF_Scan, s32Ret);
        return HI_FILEMNG_EINTER;
    ...
    if (ScopeDirCount <= s_u32MaxDirCount) {
        ...
        memcpy(s_aenDirs, ScopeDirs, DTCF_DIR_BUTT * sizeof(HI_DTCF_DIR_E));
    } else {
        HI_MUTEX_UNLOCK(s_FILEMNGMutex);
        MLOGE("%s\n", FILEMNG_Strerror(HI_FILEMNG_EINVAL));
        return HI_FILEMNG_EINVAL;
