                              ...
                              DNG_IMAGE_STATIC_INFO_S* pstStaticInfo)
{
    ...
    /**blacklevel is change with bitsPersSample, value is 200 when Bits is 12*/
    /**if Bits is smaller then 12, then the value must right shift (12-Bits)*/
    /**else  the value must left shift (Bits - 12)*/
    if (pstStaticInfo->stDngRawFormat.u8BitsPerSample >= 12){
        for (i = 0; i < DNG_MUXER_ISP_BAYER_CHN; i++){
            pstRawIFD->stRawIFDInfo.stRawFormat.af32BlackLevel[i] = 200 << (pstStaticInfo->stDngRawFormat.u8BitsPerSample - 12);
        }
    }
    else{
        for (i = 0; i < DNG_MUXER_ISP_BAYER_CHN; i++){
            pstRawIFD->stRawIFDInfo.stRawFormat.af32BlackLevel[i] = 200 >> (12 - pstStaticInfo->stDngRawFormat.u8BitsPerSample);
        }
    }
                            ...
                            HI_PHOTOMNG_DNG_USERINFO_S* pstDNGUserInfo)
{
    ...
    if (pstStaticInfo->stDngRawFormat.u8BitsPerSample >= 12){
        for (i = 0; i < DNG_MUXER_ISP_BAYER_CHN; i++){
            pstIFD->stIFD0Info.stRawFormat.af32BlackLevel[i] = 200 << (pstStaticInfo->stDngRawFormat.u8BitsPerSample - 12);
        }
    }
    else{
        for (i = 0; i < DNG_MUXER_ISP_BAYER_CHN; i++){
            pstIFD->stIFD0Info.stRawFormat.af32BlackLevel[i] = 200 >> (12 - pstStaticInfo->stDngRawFormat.u8BitsPerSample);
        }
    }
