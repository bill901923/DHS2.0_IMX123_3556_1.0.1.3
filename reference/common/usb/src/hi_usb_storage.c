...
{
    if (status == 1) {
        MLOGI("usb device(%s) connect.\n",(HI_CHAR*)conext);
        fmass_partition_startup((HI_CHAR*)conext);
    } else {
...
HI_S32 USB_STORAGE_Init(const HI_CHAR *pszDevPath)
{
    MUTEX_LOCK(g_Mutex);
...
#ifdef __HuaweiLite__
    fmass_register_notify(fmass_notify, (HI_CHAR*)pszDevPath);
#endif
    g_bInited = HI_TRUE;
    MUTEX_UNLOCK(g_Mutex);
    return HI_SUCCESS;
