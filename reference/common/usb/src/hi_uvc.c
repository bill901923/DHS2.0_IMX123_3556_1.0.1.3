...
static HI_S32 UVC_STREAM_VPortDataProc(HI_HANDLE VprocHdl, HI_HANDLE VportHdl, HI_MAPI_FRAME_DATA_S* pstVportYUV, HI_VOID* pvPrivData)
{
    ...
    //420sp, not change format
    if(PIXEL_FORMAT_YVU_SEMIPLANAR_420 == pstVportYUV->enPixelFormat)
    {
        memcpy(pstNode->pu8Mem, pCVBufVirt_Y,pstVportYUV->u32Height*pstVportYUV->u32Width*3/2);
        pstNode->u32used+=pstVportYUV->u32Height*pstVportYUV->u32Width*3/2;
        pUserPageAddr = HI_NULL;
    }
    pUserPageAddr = HI_NULL;

    UVC_FRAMEBUF_PutNodeToQueue(&UVC_FRAMEBUF_GetQueues()->stReady, pstNode);
    MLOGD("FrameSize[%u], UVC Height[%u] u32Width[%u]\n", pstNode->u32used,pstVportYUV->u32Height,pstVportYUV->u32Width);
    return HI_SUCCESS;
}
...
static HI_VOID* YUV_DumpTask(HI_VOID *pvArg)
{
    ...
    s32Ret = HI_MAPI_VPROC_StartVpssDumpYUV(pstSrc->VprocHdl,pstSrc->VportHdl,-1, &stDumpYuvCb);
...
static HI_S32 UVC_LoadMod(HI_VOID)
{
    ...
    HI_insmod(HI_APPFS_KOMOD_PATH"/usb_f_uvc.ko", NULL);
    HI_insmod(HI_APPFS_KOMOD_PATH"/g_webcam.ko", "streaming_interval=1");
#else
