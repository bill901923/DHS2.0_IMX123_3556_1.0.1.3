...
{
    if (0 == strncmp(pszBootSector + STG_FSTOOL_FSTYPE_ID_OFFSET_EXFAT, STG_FSTOOL_FSTYPE_ID_EXFAT,
                     STG_FSTOOL_FSTYPE_ID_LENGTH)) {
        return HI_SUCCESS;
    } else {
        return HI_FAILURE;
    }
...
static HI_S32 STG_FSTOOL_GetFSType(const HI_CHAR *pszPartition, HI_CHAR *pszFSType)
{
    ...
    if (s32Ret == HI_SUCCESS) {
        s32Ret = snprintf_s(pszFSType, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                            STG_FSTOOL_FSTYPE_EXFAT);
        if (s32Ret < 0) {
            STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  snprintf failed,and ret:%x!\n", __FUNCTION__, __LINE__,
                           s32Ret);
            return HI_ERR_STORAGE_SYSTEM_CALL_FAILURE;
...
static HI_S32 STG_FSTOOL_GetNode(const HI_CHAR *pszFSType, STG_FSTOOL_NODE_S **ppstNode)
{
    ...
    if (HI_NULL == pstTmpNode) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  no tool of fs(%s) , and ret:%x!\n", __FUNCTION__,
                       __LINE__, pszFSType, HI_ERR_STORAGE_ILLEGAL_PARAM);
...
static HI_S32 STG_FSTOOL_CheckMMCBlkPath(char *partionPath)
{
    HI_CHAR aszSrcPath[PATH_MAX + 1] = { 0 };
    HI_CHAR aszBlkPath[PATH_MAX + 1] = { 0 };
    HI_S32 s32Ret;
    HI_U32 i = 0;
    size_t partionLen = strlen(partionPath);
    for (i = 0; i < (partionLen - 1); i++) {
        // /dev/mmcblk0p1
        if (*(partionPath + i) == 'p' && (*(partionPath + i + 1) > '0' && *(partionPath + i + 1) <= '9')) {
            break;
        }
    }

    if ((i <= 0) || (i >= (partionLen - 1))) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "cannot parse blk path\n");
        return HI_FAILURE;
    }
    s32Ret = strncpy_s(aszSrcPath, PATH_MAX + 1, partionPath, i);
    if (s32Ret != 0) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "cpy blk path faild\n");
        return HI_FAILURE;
    }

    if (!realpath(aszSrcPath, aszBlkPath)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "dev path (%s) is not exist\n", aszSrcPath);
        return HI_FAILURE;
    }
...
static HI_S32 STG_FSTOOL_DetectPartition(HI_CHAR *pszPartition)
{
    ...
    if (pRealPath != HI_NULL) {
        return HI_SUCCESS;
    }
    if (STG_FSTOOL_CheckMMCBlkPath(pszPartition) == HI_SUCCESS) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s %d dev block exist, partion %s not exist!\n", __FUNCTION__,
                       __LINE__, pszPartition);
        return HI_STORAGE_FSTOOL_ERR_FSTYPE_UNSUPPORT;
    } else {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s %d partion %s and dev block not exist!\n", __FUNCTION__,
                       __LINE__, pszPartition);
...
static HI_S32 STG_FSTOOL_TestPartition(HI_MW_PTR pFSTool, HI_MW_PTR pStorage)
{
    ...
    if (HI_SUCCESS != s32Ret) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR,
                       "%s  %d  device(%s) is exception, system errno(%d), and ret:%x!\n", __FUNCTION__, __LINE__,
                       pstFSTool->aszPartitionPath, errno, HI_STORAGE_FSTOOL_ERR_DEVICE_EXCEPTION);
        return s32Ret;
    }

    s32Ret = snprintf_s(aszTestFile, HI_STORAGE_PATH_LEN_MAX + 16, HI_STORAGE_PATH_LEN_MAX + 16 - 1, "%s/%s",
                        pstFSTool->aszMountPath, STG_FSTOOL_TEST_PARTITION_FILE_NAME);
    if (s32Ret < 0) {
    ...
    if (0 > fd) {
        (HI_VOID)STG_FSTOOL_Umount(pFSTool, HI_FALSE);
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR,
                       "%s  %d  open test file(%s) fail, system errno(%d), and ret:%x!\n", __FUNCTION__, __LINE__, aszTestFile, errno,
                       HI_STORAGE_FSTOOL_ERR_DEVICE_EXCEPTION);
        return HI_STORAGE_FSTOOL_ERR_DEVICE_EXCEPTION;
    }
    ...
    if (0 > write(fd, aszTestData, STG_FSTOOL_TEST_PARTITION_DATA_LEN)) {
        if (errno != ENOSPC) {
            STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR,
                           "%s  %d  write test file(%s) fail, system errno(%d), and ret:%x!\n", __FUNCTION__, __LINE__,
                           pstFSTool->aszPartitionPath, errno, HI_STORAGE_FSTOOL_ERR_DEVICE_EXCEPTION);
            s32Ret = HI_STORAGE_FSTOOL_ERR_DEVICE_EXCEPTION;
        } else {
            // space is full not return erron
            STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_WARN,
                           "%s  %d  write test file(%s) fail, system errno(%d) for space is full!\n", __FUNCTION__, __LINE__,
...
HI_S32 STG_FSTOOL_Register(const HI_STORAGE_FS_TOOL_S *pstFSTool)
{
    ...
    s32Ret = snprintf_s(pstCurNode->stTool.aszFSType, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pstFSTool->aszFSType);
    if (s32Ret < 0) {
        free(pstCurNode);
        pstCurNode = NULL;
...
HI_S32 STG_FSTOOL_Create(const STG_FSTOOL_CFG_S *pstCfg, HI_MW_PTR *ppFSTool)
{
    ...
    s32Ret = snprintf_s(pstTool->aszPartitionPath, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pstCfg->aszPartitionPath);
    if (s32Ret < 0) {
        free(pstTool);
        pstTool = NULL;
        return HI_ERR_STORAGE_SYSTEM_CALL_FAILURE;
    }
    s32Ret = snprintf_s(pstTool->aszMountPath, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pstCfg->aszMountPath);
    if (s32Ret < 0) {
        free(pstTool);
        pstTool = NULL;
...
HI_S32 STG_FSTOOL_GetPartitionPath(HI_MW_PTR pFSTool, HI_CHAR *pszPartitionPath)
{
    HI_S32 s32Ret = 0;
    STG_FSTOOL_S *pstFSTool = (STG_FSTOOL_S *)pFSTool;
    s32Ret = snprintf_s(pszPartitionPath, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pstFSTool->aszPartitionPath);
    if (s32Ret < 0) {
        return HI_ERR_STORAGE_SYSTEM_CALL_FAILURE;
    }
...
HI_STORAGE_FSTOOL_ERR_E STG_FSTOOL_Check(HI_MW_PTR pFSTool, HI_MW_PTR pStorage)
{
    ...
    if (HI_SUCCESS != s32Ret) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  do fsck(%s) failed.\n", __FUNCTION__,
                       __LINE__, pstFSTool->aszPartitionPath);
        return (HI_STORAGE_FSTOOL_ERR_E)s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret) {
        ...
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  do fsck(%s) failed, and ret:%x!\n", __FUNCTION__,
                       __LINE__, pstFSTool->aszPartitionPath, enRet);
        return enRet;
    }

    if (HI_SUCCESS != STG_FSTOOL_GetNode(aszFSType, &pstToolNode)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  do fsck(%s) failed, and ret:%x!\n", __FUNCTION__,
                       __LINE__, pstFSTool->aszPartitionPath, HI_STORAGE_FSTOOL_ERR_FSTYPE_UNSUPPORT);
        return HI_STORAGE_FSTOOL_ERR_FSTYPE_UNSUPPORT;
    }
    ...
    if (HI_SUCCESS != s32Ret) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  test partition(%s) failed, and ret:%x!\n", __FUNCTION__,
                       __LINE__, pstFSTool->aszPartitionPath, s32Ret);
        return HI_STORAGE_FSTOOL_ERR_DEVICE_EXCEPTION;
    }
...
HI_S32 STG_FSTOOL_Mount(HI_MW_PTR pFSTool, HI_MW_PTR pStorage)
{
    ...
    if (HI_SUCCESS != s32Ret) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  do mount(%s) failed, and ret:%x!\n", __FUNCTION__,
                       __LINE__, pstFSTool->aszPartitionPath, HI_ERR_STORAGE_SYSTEM_CALL_FAILURE);
        return HI_ERR_STORAGE_SYSTEM_CALL_FAILURE;
    }
    ...
    if (HI_SUCCESS != mount(pstFSTool->aszPartitionPath, pstFSTool->aszMountPath, aszFSType,
                            MS_NOEXEC | MS_NOATIME | MS_NODIRATIME, 0)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  mount %s on %s failure, and ret:%x, errno(%d)!\n",
                       __FUNCTION__, __LINE__, pstFSTool->aszPartitionPath, pstFSTool->aszMountPath, HI_ERR_STORAGE_SYSTEM_CALL_FAILURE,
                       errno);
        return HI_ERR_STORAGE_SYSTEM_CALL_FAILURE;
    }
