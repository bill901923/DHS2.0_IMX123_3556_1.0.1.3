...
static HI_VOID *STG_DEVMONITOR_Routine(HI_VOID *pArg)
{
    ...
    while (!pMonitor->bDestroy) {
        ...
        if (!bStateChangeDurHandle) {
            bStateChangeDurHandle = (enLastState != enCurState) ? HI_TRUE : HI_FALSE;
        }
        if (pMonitor->bStopNotify
            || pMonitor->bPauseNotify || !bStateChangeDurHandle
            ...
            STG_DEV_EVENT_INFO_S stStateEventInfo;
            stStateEventInfo.enState = stEventInfo.enState;
            stStateEventInfo.u32ErrorCnt = 0;
            s32Ret = STG_DEVFSM_SendEvent(pMonitor->pDevFSM, &stStateEventInfo);
            if (HI_SUCCESS != s32Ret) {
                STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  SendAsync failure, and ret:%x!\n", __FUNCTION__,
                               __LINE__, s32Ret);
            }
        }
        if(stEventInfo.u32ErrorCnt != 0){
            //error happened,then send error event add.
            (HI_VOID)STG_DEVFSM_SendEvent(pMonitor->pDevFSM, &stEventInfo);
        }
