...
HI_S32 HI_STORAGE_Create(const HI_STORAGE_CFG_S *pstCfg, const HI_STORAGE_ON_STATE_CHANGE_PFN pfnOnStateChange,
                         HI_MW_PTR *ppStorage)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    if (0 == pstCfg->u8DevErrCountThr) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR,
                       "%s  %d  param pstCfg->u8DevErrCountThr should be over 0, and ret:%x!\n", __FUNCTION__, __LINE__,
                       HI_ERR_STORAGE_ILLEGAL_PARAM);
        (HI_VOID)pthread_mutex_unlock(&s_mModuleLock);
        return HI_ERR_STORAGE_ILLEGAL_PARAM;
    }
    ...
    if (0 == strlen(pstCfg->aszPartitionPath)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR,
                       "%s  %d  param pstCfg->aszPartitionPath length is 0, and ret:%x!\n", __FUNCTION__, __LINE__,
                       HI_ERR_STORAGE_ILLEGAL_PARAM);
        (HI_VOID)pthread_mutex_unlock(&s_mModuleLock);
        return HI_ERR_STORAGE_ILLEGAL_PARAM;
    }
    ...
    if (HI_NULL == realpath(pstCfg->aszMountPath, aszMntPath)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR,
                       "%s  %d  param pstCfg->aszMountPath is illegal(system errno %d), and ret:%x!\n", __FUNCTION__, __LINE__, errno,
                       HI_ERR_STORAGE_ILLEGAL_PARAM);
        (HI_VOID)pthread_mutex_unlock(&s_mModuleLock);
        return HI_ERR_STORAGE_ILLEGAL_PARAM;
    }
    ...
    if (HI_NULL == pstInst) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  has no free inst, and ret:%x!\n", __FUNCTION__,
                       __LINE__, HI_ERR_STORAGE_INST_CNT_UPPER_LIMIT);
        (HI_VOID)pthread_mutex_unlock(&s_mModuleLock);
        return HI_ERR_STORAGE_INST_CNT_UPPER_LIMIT;
    }

    s32Ret = snprintf_s(stFSToolCfg.aszPartitionPath, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
    ...
    s32Ret = snprintf_s(stFSToolCfg.aszMountPath, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pstCfg->aszMountPath);
    if (s32Ret < 0) {
        STG_FreeInst(pstInst);
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "snprintf_s stFSToolCfg.aszPartitionPath failed!\n");
    ...
    s32Ret = snprintf_s(stDevCfg.aszPartitionPath, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pstCfg->aszPartitionPath);
    if (s32Ret < 0) {
        STG_FreeInst(pstInst);
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "snprintf_s stFSToolCfg.aszPartitionPath failed!\n");
...
HI_S32 HI_STORAGE_GetState(HI_MW_PTR pStorage, HI_STORAGE_STATE_E *penState)
{
    ...
    if (STG_DEV_IsPaused(pstInst->pDev)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  storage is paused, and ret:%x!\n", __FUNCTION__,
                       __LINE__, HI_ERR_STORAGE_PAUSED);
        STG_UnlockInst(pstInst);
        (HI_VOID)pthread_mutex_unlock(&s_mModuleLock);
...
HI_S32 HI_STORAGE_Format(HI_MW_PTR pStorage, const HI_CHAR *pszFSType, HI_U64 u64ClusterSize)
{
    ...
    if ((STG_FS_STATE_CHECKING == enFSState) || (STG_FS_STATE_IDEL == enFSState)) {
        STG_LOG_Printf(MODULE_STORAGE, STG_LOG_LEVEL_ERR, "%s  %d  fs is checking now, and ret:%x!\n", __FUNCTION__,
                       __LINE__, HI_ERR_STORAGE_FS_CHECKING);
        STG_UnlockInst(pstInst);
        (HI_VOID)pthread_mutex_unlock(&s_mModuleLock);
    ...
    if (STG_FS_STATE_MOUNTED == enFSState) {
        ...
        (HI_VOID)STG_FSTOOL_Umount(pstInst->pFSTool,
