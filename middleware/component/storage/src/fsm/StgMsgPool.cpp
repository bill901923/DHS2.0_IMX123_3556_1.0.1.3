...
HI_S32 StgMsgPool::init(HI_U32 u32MaxMsgNum, HI_U32 u32MaxMsgPayloadLen)
{
    ...
    memset_s(mMsgPrivList, sizeof(MSG_PRIVATE_INFO_S) * u32MaxMsgNum, 0x00,
             (sizeof(MSG_PRIVATE_INFO_S) * u32MaxMsgNum));

    for (i = 0; i < u32MaxMsgNum; i++) {
        mMsgMemList[i].pMsgData = (u32MaxMsgPayloadLen > 0) ? (HI_VOID *)&mMsgPayloadList[i * u32MaxMsgPayloadLen] :
                                  HI_NULL;
