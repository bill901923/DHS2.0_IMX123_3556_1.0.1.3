...
HI_VOID STG_PROC_EnableInst(HI_MW_PTR hInst, const HI_CHAR *pszPartition)
{
    ...
    pstInst->hInst = hInst;
    s32Ret = snprintf_s(pstInst->aszPartition, HI_STORAGE_PATH_LEN_MAX, HI_STORAGE_PATH_LEN_MAX - 1, "%s",
                        pszPartition);
    if (s32Ret < 0) {
...
HI_VOID STG_PROC_RefDuration(HI_MW_PTR hInst, STG_PROC_DURATION_TYPE_E enState)
{
    ...
    if (STG_PROC_DURATION_TYPE_FSCHECK == enState) {
        ...
        pstInst->u64FSCheckDuration = (HI_U64)(stEndTime.tv_sec - pstStartTime->tv_sec) * 1000 * 1000 +
                                      (stEndTime.tv_usec - pstStartTime->tv_usec);
    } else if (STG_PROC_DURATION_TYPE_FORMAT == enState) {
        pstStartTime = &pstInst->stFmtStartTime;
        pstInst->u64FormatDuration = (HI_U64)(stEndTime.tv_sec - pstStartTime->tv_sec) * 1000 * 1000 +
                                     (stEndTime.tv_usec - pstStartTime->tv_usec);
    } else {
        pstStartTime = &pstInst->stMntStartTime;
        pstInst->u64MountDuration = (HI_U64)(stEndTime.tv_sec - pstStartTime->tv_sec) * 1000 * 1000 +
                                    (stEndTime.tv_usec - pstStartTime->tv_usec);
    }
#endif
