...
typedef struct hi_STORAGE_FS_TOOL_S {
    HI_STORAGE_FSTOOL_ERR_E(*pfnCheck)(const HI_CHAR
                            *pszPartitionPath);                          // ps32FSSitutaion such as FS Exception, FS fragment serious and so on
    HI_STORAGE_FSTOOL_ERR_E(*pfnFormat)(const HI_CHAR *pszPartitionPath,
                            HI_U64 u64ClusterSize);  // u64ClusterSize unit is bytes, should be 512*2^n, such as 1024B
    HI_CHAR aszFSType[HI_STORAGE_PATH_LEN_MAX];                                                         /**such as vfat/exfat */
} HI_STORAGE_FS_TOOL_S;

typedef HI_VOID (*HI_STORAGE_ON_STATE_CHANGE_PFN)(HI_MW_PTR pStorage, const HI_CHAR *pszPartitionPath,
