...
static HI_S32 COMMON_GET_Frame(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer)
{
    ...
    HI_S32 s32Ret = av_read_frame(pstDemuxer->pstFormatContext, &pstDemuxer->stPkt);
...
static HI_S32 COMMON_GET_FileSize(HI_FFDEMUXER_FORMAT_MEMBER_S *pstFormatMember, HI_S64 *ps64FileSize)
{
    ...
    HI_S32 s32Ret = fseeko(pMediaFd, 0, SEEK_END);
...
static HI_S32 COMMON_ProbeVideo(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer, HI_BOOL *pbBsInited)
{
    ...
    if ((pstDemuxer->enFormat != HI_FFDEMUX_FORMAT_TS) && (pstDemuxer->enFormat != HI_FFDEMUX_FORMAT_MP4)) {
    ...
    HI_U32 i = 0;
    HI_S32 s32Ret = av_find_best_stream(pstDemuxer->pstFormatContext, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    if (s32Ret < 0) {
        ...
        if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_MP4) {
            if (AV_CODEC_ID_H264 == pstDemuxer->enVidCodecId
                || pstDemuxer->enVidCodecId == AV_CODEC_ID_HEVC) {
                const HI_CHAR *pszFilterName = (pstDemuxer->enVidCodecId == AV_CODEC_ID_H264) ? "h264_mp4toannexb" :
                                               "hevc_mp4toannexb";
            ...
            for (i = 0; i < u32StreamNum; i++) {
                if (pstDemuxer->pstFormatContext->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
                    ...
                    } else if (pstDemuxer->pstFormatContext->streams[i]->codecpar->codec_id == AV_CODEC_ID_HEVC) {
        ...
        if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_TS) {
            HI_U32 i = 0;
...
static HI_S32 COMMON_ProbeAudio(HI_FFDEMUXER_FORMAT_MEMBER_S *pstFormatMember)
{
    ...
    if (pstFormatMember->enFormat == HI_FFDEMUX_FORMAT_TS) {
        ...
        if (s32Ret < 0) {
            ...
            if (pstFormatMember->enFormat == HI_FFDEMUX_FORMAT_WAV) {
...
static HI_S32 COMMON_ProbeMeta(HI_FFDEMUXER_FORMAT_MEMBER_S *pstFormatMember)
{
    ...
    if (pstFormatMember->enFormat == HI_FFDEMUX_FORMAT_MP4) {
...
static HI_S32 COMMON_Seekfile(HI_FFDEMUXER_FORMAT_MEMBER_S *pstFormatMember, HI_S32 s32StreamIndex, HI_S64 s64MSec,
                              HI_S32 s32Flag)
{
    ...
    if (s32StreamIndex == pstFormatMember->s32UsedVideoStreamIndex && pstFormatMember->pstUsedVideoStream) {
        s64VideoDuration = pstFormatMember->pstUsedVideoStream->duration * FFMPEG_TIME_UNIT *
                           pstFormatMember->pstUsedVideoStream->time_base.num / pstFormatMember->pstUsedVideoStream->time_base.den;
        if (s64MSec > s64VideoDuration) {
            FFDEMUX_Printf(MODULE_NAME_DEMUX, HI_FFDEMUX_LOG_LEVEL_DEBUG,
                           "seek input time beyond total video time seektime: %lld s64VideoDuration %lld \n", s64MSec, s64VideoDuration);
    ...
    if (s32StreamIndex != -1 && s32StreamIndex < (HI_S32)pstFormatMember->pstFormatContext->nb_streams) {
        s64SeekTarget = av_rescale(s64SeekTarget,
                                   pstFormatMember->pstFormatContext->streams[s32StreamIndex]->time_base.den,
    ...
    HI_S32 s32Ret = avformat_seek_file(pstFormatMember->pstFormatContext, s32StreamIndex, s64SeekMin, s64SeekTarget,
                                s64SeekMax, s32SeekFlag);
    if (s32Ret < 0) {
        FFDEMUX_Printf(MODULE_NAME_DEMUX, HI_FFDEMUX_LOG_LEVEL_DEBUG,
                       "error while seeking %lld ,streamIndex:%d, s32Ret:%d\n", s64MSec, s32StreamIndex, s32Ret);
...
static HI_S32 COMMON_GetVideoInfo(HI_FFDEMUXER_FORMAT_MEMBER_S *pstFormatMember, HI_FORMAT_FILE_INFO_S *pstFmtInfo)
{
    ...
    if (pstFormatMember->enFormat == HI_FFDEMUX_FORMAT_TS) {
        ...
        for (s32Index = 0; s32Index < HI_DEMUXER_RESOLUTION_CNT; s32Index++) {
            ...
            pstFmtInfo->stSteamResolution[s32Index].s32VideoStreamIndex =
                pstFormatMember->stSteamResolution[s32Index].s32VideoStreamIndex;
    ...
    if (pstFormatMember->enFormat == HI_FFDEMUX_FORMAT_MP4) {
        ...
        pstFmtInfo->fFrameRate = (stFrameRate.den &&
                                  stFrameRate.num)
                                 ? (HI_FLOAT)stFrameRate.num / (HI_FLOAT)stFrameRate.den
                                 : FFMPEG_DEFAULT_FPS;
...
static HI_S32 COMMON_GetMetaInfo(HI_FFDEMUXER_FORMAT_MEMBER_S *pstFormatMember, HI_FORMAT_FILE_INFO_S *pstFmtInfo)
{
    ...
    if (!pstFormatMember->pstMetaStream && !pstFormatMember->pstVideoStream) {
        pstFmtInfo->stSteamResolution[pstFormatMember->s32VideoStreamIndex].s32VideoStreamIndex =
            pstFormatMember->s32VideoStreamIndex;
    ...
    pstFmtInfo->stSteamResolution[pstFormatMember->s32MetaStreamIndex].u32Width =
        pstFormatMember->pstMetaStream->codecpar->width;
    pstFmtInfo->stSteamResolution[pstFormatMember->s32MetaStreamIndex].u32Height =
        pstFormatMember->pstMetaStream->codecpar->height;
    pstFmtInfo->stSteamResolution[pstFormatMember->s32MetaStreamIndex].s32VideoStreamIndex =
        pstFormatMember->s32MetaStreamIndex;
...
HI_S32 COMMON_Read(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer, HI_FORMAT_PACKET_S *pstFmtFrame)
{
    ...
    while (bGetPacket != HI_TRUE) {
        HI_S32 s32Ret = COMMON_GET_Frame(pstDemuxer);
        ...
        if (pstDemuxer->stPkt.stream_index == pstDemuxer->s32AudioStreamIndex) {
            ...
            } else if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_TS) {
                s32Ret = TS_SET_AudioInfo(pstDemuxer, pstFmtFrame);
            } else if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_WAV) {
...
HI_S32 COMMON_Free(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer, HI_FORMAT_PACKET_S *pstFmtFrame)
{
    ...
    if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_MP4) {
        if (!(pstDemuxer->bHasAdts && pstDemuxer->stPkt.stream_index == pstDemuxer->s32AudioStreamIndex) == HI_FALSE) {
...
HI_S32 COMMON_Probe(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer)
{
    ...
    HI_S32 s32Ret = COMMON_ProbeVideo(pstDemuxer, &bBsfInited);
    if (s32Ret != HI_SUCCESS) {
    ...
    if (COMMON_GET_FileSize(pstDemuxer, &s64FileSize) != HI_SUCCESS) {
        ...
        if (bBsfInited && pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_MP4) {
...
HI_S32 COMMON_Getinfo(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer, HI_FORMAT_FILE_INFO_S *pstFmtInfo)
{
    ...
    if ((pstDemuxer->s32AudioStreamIndex == HI_DEMUXER_NO_MEDIA_STREAM)
        && (pstDemuxer->s32VideoStreamIndex == HI_DEMUXER_NO_MEDIA_STREAM)) {
        FFDEMUX_Printf(MODULE_NAME_DEMUX, HI_FFDEMUX_LOG_LEVEL_ERR, "should call fmt_probe first \n");
        return HI_ERR_DEMUXER_ACTION;
    }

    if (memset_s(pstFmtInfo, sizeof(HI_FORMAT_FILE_INFO_S), 0x00, sizeof(HI_FORMAT_FILE_INFO_S)) != HI_SUCCESS) {
    ...
    // get video info
    HI_S32 s32Ret = COMMON_GetVideoInfo(pstDemuxer, pstFmtInfo);
    if (s32Ret != HI_SUCCESS) {
    ...
    if (pstDemuxer->pstUsedVideoStream && pstDemuxer->pstAudioStream) {
        if (pstDemuxer->pstUsedVideoStream->time_base.den) {
            s64VideoDuration = pstDemuxer->pstUsedVideoStream->duration * FFMPEG_TIME_UNIT *
                               pstDemuxer->pstUsedVideoStream->time_base.num / pstDemuxer->pstUsedVideoStream->time_base.den;
        }
        if (pstDemuxer->pstAudioStream->time_base.den) {
            s64AudioDuration = pstDemuxer->pstAudioStream->duration * FFMPEG_TIME_UNIT *
                               pstDemuxer->pstAudioStream->time_base.num / pstDemuxer->pstAudioStream->time_base.den;
        }
...
HI_S32 COMMON_Setattr(HI_FFDEMUXER_FORMAT_MEMBER_S *pstDemuxer, HI_FORMAT_MEDIA_ATTR_S *pstFmtInfo)
{
    ...
    if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_WAV) {  // when wav just set audio
    ...
    if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_MP4) {
        ...
        if ((pstFmtInfo->s32VidStreamId != pstDemuxer->s32VideoStreamIndex) &&
            (pstFmtInfo->s32VidStreamId != pstDemuxer->s32MetaStreamIndex) && 
            (bFound == HI_FALSE)) {
            FFDEMUX_Printf(MODULE_NAME_DEMUX, HI_FFDEMUX_LOG_LEVEL_ERR, "set MP4 video index:%d not support error \n",
                           pstFmtInfo->s32VidStreamId);
            return HI_ERR_DEMUXER_NOT_SUPPORT;
        ...
        if (pstFmtInfo->s32VidStreamId == pstDemuxer->s32VideoStreamIndex) {
            ...
            const HI_CHAR *pszFilterName = (AV_CODEC_ID_H264 == pstDemuxer->enUsedVidCodecId) ? "h264_mp4toannexb" :
                                           "hevc_mp4toannexb";
    ...
    if (pstDemuxer->enFormat == HI_FFDEMUX_FORMAT_TS) {
        if ((HI_U32)pstFmtInfo->s32VidStreamId >= pstDemuxer->pstFormatContext->nb_streams ||
            pstFmtInfo->s32VidStreamId < 0) {
