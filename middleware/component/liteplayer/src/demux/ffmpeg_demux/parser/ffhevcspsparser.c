...
{
    if ((pstSpsInfo == NULL) || (pu8Sps == NULL) || (u32SpsLen == 0)) {
        return HI_FALSE;
    }

    HI_MW_PTR bitReader = 0;
    HI_S32 s32Ret = FFBitsReader_create(&bitReader, pu8Sps, u32SpsLen);
    ...
    if (memcpy_s(pstSpsInfo, sizeof(mSpsInfo), &mSpsInfo, sizeof(mSpsInfo)) != HI_SUCCESS) {
...
HI_BOOL FFHEVCParser_parseStRefPicSet(HI_MW_PTR bitReader, HI_U32 u32StRpsIdx, HI_U32 u32NumShortTermRefPicSets,
                                      HI_U32 au32NumDeltaPocs[64])
{
