...
typedef struct hiAAC_ADTS_HEADER_S {
    ...
    /* variable */
    HI_U8 u8CopyBit;          /* 1 bit of the 72-bit copyright ID (
transmitted as 1 bit per frame) */
    HI_U8 u8CopyStart;        /* 1 : this bit starts the 72-bit ID, 0 : it
does not */
    HI_U32 u32FrameLength;    /* length of frame */
    HI_U32 u32BufferFull;     /* number of 32-bit words left in enc buffer,
0x7FF : VBR */
...
} HI_AAC_ADTS_HEADER_S;

HI_S32 FFAACParser_parseAdtsHeader(HI_AAC_ADTS_HEADER_S *pstHeader, HI_U8 *pu8Sps, HI_U32 u32SpsLen,
