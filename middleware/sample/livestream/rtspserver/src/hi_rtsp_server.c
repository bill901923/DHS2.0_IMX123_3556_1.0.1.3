...
static HI_S32 RTSPSVR_OnClientConnection(HI_MW_PTR obj, HI_S32 s32SocketFd, HI_CHAR* pClientReq, HI_U32 u32ReqLen)
{
    ...
    HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "HI_RTSPSVR_MSGParser_CheckValidRequest Pass\n");
    ...
    HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "HI_RTSPSVR_MSGParser_GetStreamName Pass %d  %s\n",bGetStreamName,aszStream);
    ...
    HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "HI_RTSPSVR_MSGParser_GetCSeq Pass %d\n",s32Cseq);
    ...
    HI_List_For_Each(pstPosNode, &pstServerCtx->pStreamlist)
    {
        pstStreamNode = HI_LIST_ENTRY(pstPosNode, HI_LIVE_MEDIA_STREAM_NODE_S, listPtr);
         ...
         HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "pstStreamNode->name =%s\n",pstStreamNode->name);
         HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "aszStream =%s\n",aszStream);
        if (!strcasecmp(pstStreamNode->name, aszStream))
        {
            HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "!strcasecmp(pstStreamNode->name, aszStream)\n");
            bFound = HI_TRUE;
            break;
        }
    }
    ...
    {
        HI_SERVER_LOG_Printf(MODULE_NAME_RTSPSERVER, HI_SERVER_LOG_LEVEL_ERR, "invalid rtsp request, could not find correspond stream A bFound %d\n",bFound);
        StatCode = HIRTSP_STATUS_BAD_REQUEST;
...
HI_S32 HI_RTSPSVR_Start(HI_MW_PTR handle)
{
    ...
    s32Ret = HI_NETWORK_Listener_Start(pstServerCtx->hNetListener);
    if (s32Ret != HI_SUCCESS)
    {
