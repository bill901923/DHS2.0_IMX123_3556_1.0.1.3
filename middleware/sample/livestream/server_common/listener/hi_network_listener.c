...
static HI_S32 NETWORK_Listener_SetNonblock(HI_S32 sockfd)
{
    ...
    s32Ret = fcntl(sockfd, F_SETFL, flags | O_NONBLOCK);   //设置成非阻塞模式；
    if( s32Ret < 0 )
    {
        return s32Ret;
    }
...
/*
keepalive: 1:开启keepalive属性, 0:关闭
keepidle: 如该连接在keepidle秒内没有任何数据往来,则进行探测
keepinterval: 探测时发包的时间间隔为keepinterval 秒
keepcount:  探测尝试的次数.如果第1次探测包就收到响应了,则后2次的不再发.
*/
...
static HI_VOID* NETWORK_Listener_Process(void* args)
{
    ...
    {
        HI_SERVER_LOG_Printf(MODULE_NAME_LIVESTREAM, HI_SERVER_LOG_LEVEL_ERR, "set lisnTimeoutVal fail \n");
        return HI_NULL;
    }
