...
static HI_VOID MAPI_VCAP_ClientResetStatus(HI_VOID)
{
    HI_S32 s32Ret;
    HI_S32 i = 0;
    HI_S32 j = 0;
    ...
    for (i = 0; i < HI_VCAP_DUMP_SCENE_NUM; i++) {
        for (j = 0; j < HI_MAPI_VCAP_MAX_PIPE_NUM; j++) {
            ...
            s32Ret = memset_s(&g_astVcapDumpRawInfo[i][j].stVcapDumpAttr, sizeof(HI_MAPI_DUMP_ATTR_S), 0,
...
HI_S32 MAPI_VCAP_ClientInit(HI_VOID)
{
    HI_S32 i = 0;
    HI_S32 j = 0;

    MAPI_VCAP_ClientResetStatus();
...
HI_S32 MAPI_VCAP_ClientDeinit(HI_VOID)
{
    HI_S32 i = 0;
    HI_S32 j = 0;
...
static HI_VOID *MAPI_DumpRawProc(HI_VOID *pVoid)
{
    ...
    for (i = 0; i < pstRawDataInfo->u32GroupCount; i++) {
        ...
        u32RawWriteBuffFreeLen = sizeof(HI_MAPI_FRAME_DATA_S) *
            ((HI_VCAP_RAWGROUP_MAX_NUM * HI_VCAP_RAW_EACHGROUP_MAX_NUM) - (s32RetDataNum * i));
        s32Ret = memcpy_s((&astRawWriteBuff[0] + s32RetDataNum * i), u32RawWriteBuffFreeLen,
            &astRawTempBuff[0], sizeof(HI_MAPI_FRAME_DATA_S) * s32RetDataNum);
        if (s32Ret != HI_SUCCESS)
        {
            MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "memcpy_s fail,ret:%#x\n", s32Ret);
            goto VCAP_DUMP_FAIL;
        }
    }
...
HI_S32 HI_MAPI_VCAP_InitSensor(HI_HANDLE VcapDevHdl, HI_MAPI_SENSOR_ATTR_S *pstSensorAttr, HI_BOOL bResetSensor)
{
    ...
    CHECK_MAPI_VCAP_TRUE_RET(MAPI_GetMediaInitStatus(), "media not inited yet", HI_MAPI_VCAP_ENOT_INITED);
    stPrivData.as32PrivData[0] = bResetSensor;

    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapDevHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_SENSOR_INIT, pstSensorAttr, sizeof(HI_MAPI_SENSOR_ATTR_S), &stPrivData);
...
HI_S32 HI_MAPI_VCAP_SetPipeVCNumber(HI_HANDLE VcapPipeHdl, HI_U32 u32VCNumber)
{
    ...
    stPrivData.as32PrivData[0] = u32VCNumber;
    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapPipeHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VCAP_SET_PIPE_VCNUMBER, HI_NULL, 0, &stPrivData);

    CHECK_MAPI_VCAP_RET(s32Ret, "MSG_CMD_VCAP_SET_PIPE_VCNUMBER fail\n");
...
HI_S32 HI_MAPI_VCAP_GetPipeVCNumber(HI_HANDLE VcapPipeHdl, HI_U32 *pu32VCNumber)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapPipeHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VCAP_GET_PIPE_VCNUMBER, pu32VCNumber, sizeof(HI_U32), HI_NULL);

    CHECK_MAPI_VCAP_RET(s32Ret, "MSG_CMD_VCAP_GET_PIPE_VCNUMBER fail\n");
...
HI_S32 HI_MAPI_VCAP_StartPipe(HI_HANDLE VcapPipeHdl)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapPipeHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VCAP_START_PIPE, HI_NULL, 0, HI_NULL);

    CHECK_MAPI_VCAP_RET(s32Ret, "MSG_CMD_VCAP_START_PIPE fail\n");
...
HI_S32 HI_MAPI_VCAP_StopPipe(HI_HANDLE VcapPipeHdl)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapPipeHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VCAP_STOP_PIPE, HI_NULL, 0, HI_NULL);

    CHECK_MAPI_VCAP_RET(s32Ret, "MSG_CMD_VCAP_STOP_PIPE fail\n");
...
HI_S32 HI_MAPI_VCAP_InitISP(HI_HANDLE VcapPipeHdl)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapPipeHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VCAP_INIT_ISP, HI_NULL, 0, HI_NULL);

    CHECK_MAPI_VCAP_RET(s32Ret, "MSG_CMD_VCAP_INIT_ISP fail\n");
...
HI_S32 HI_MAPI_VCAP_DeInitISP(HI_HANDLE VcapPipeHdl)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VCAP, VcapPipeHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VCAP_DEINIT_ISP, HI_NULL, 0, HI_NULL);

    CHECK_MAPI_VCAP_RET(s32Ret, "MSG_CMD_VCAP_DEINIT_ISP fail\n");
...
HI_S32 HI_MAPI_VCAP_GetSnapAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_SNAP_ATTR_S *pstSnapAttr)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSnapAttr);
    CHECK_MAPI_VCAP_TRUE_RET(MAPI_GetMediaInitStatus(), "media not inited yet", HI_MAPI_VCAP_ENOT_INITED);
...
HI_S32 HI_MAPI_VCAP_SetDumpRawAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_DUMP_ATTR_S *pstDumpAttr)
{
    ...
    if (pstDumpAttr->stDumpAttr.bEnable) {
        s32Ret = memcpy_s(&g_astVcapDumpRawInfo[NORMAL_RAW_INFO_ID][VcapPipeHdl].stVcapDumpAttr,
    ...
    if (pstDumpAttr->stDumpBNRAttr.bEnable) {
        s32Ret = memcpy_s(&g_astVcapDumpRawInfo[BNR_RAW_INFO_ID][VcapPipeHdl].stVcapDumpAttr,
            sizeof(HI_MAPI_DUMP_ATTR_S), pstDumpAttr, sizeof(HI_MAPI_DUMP_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    }
    ...
    HI_MAPI_VCAP_RAW_DATA_S *pstVCapRawData)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_MAX_VAL_RET("u32Count", u32Count, HI_VCAP_RAWGROUP_MAX_NUM);
    CHECK_MAPI_VCAP_MIN_VAL_RET("u32Count", u32Count, 1);
