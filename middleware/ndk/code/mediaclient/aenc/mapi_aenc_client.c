...
static HI_S32 MAPI_AENC_GetAData(HI_HANDLE AencHdl)
{
    ...
    while (g_stAencContext.astAencChn[AencHdl].enAencChnState == MAPI_AENC_CHN_STARTED &&
        ...
        s32Ret = memcpy_s(&stOutAStream, sizeof(AUDIO_STREAM_S), &pstStream->stAencData, sizeof(AUDIO_STREAM_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_AENC, "memcpy_s", s32Ret);
...
static HI_VOID MAPI_AENC_CondTimeWait(pthread_cond_t *pCond, pthread_mutex_t *pMutex, HI_U64 u64delayUs)
{
    struct timespec ts;
...
HI_VOID *ProcAStreamThread(HI_VOID *arg)
{
    ...
    while (g_stAencContext.astProcADataThread[hAencChn].bProcADataThdStartFlg == HI_TRUE) {
        ...
        if (g_stAencContext.astAencChn[hAencChn].enAencChnState != MAPI_AENC_CHN_STARTED ||
            ...
            MAPI_AENC_CondTimeWait(&g_stAencContext.astProcADataThread[hAencChn].c_ProcADataThd_cond,
...
static HI_S32 MAPI_AENC_OpenDatafifoReader(HI_HANDLE AencHdl)
{
    ...
    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
...
HI_S32 MAPI_AENC_ClientInit()
{
    ...
    s32Ret = memset_s(&g_stAencContext, sizeof(MAPI_AENC_CONTEXT_S), 0, sizeof(MAPI_AENC_CONTEXT_S));
...
HI_S32 HI_MAPI_AENC_Init(HI_HANDLE AencHdl, const HI_MAPI_AENC_ATTR_S *pstAencAttr)
{
    ...
    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
    CHECK_MAPI_AENC_NULL_PTR_RET(pstAencAttr);
    ...
    s32Ret = memcpy_s(&stAencAttr, sizeof(HI_MAPI_AENC_ATTR_S), pstAencAttr, sizeof(HI_MAPI_AENC_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_AENC, "memcpy_s", s32Ret, g_aencFuncLock[AencHdl]);
    s32Ret = memcpy_s(stMsgPriData.as32PrivData, sizeof(MAPI_PRIV_DATA_S), pstAencAttr, sizeof(HI_MAPI_AENC_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_AENC, "memcpy_s", s32Ret, g_aencFuncLock[AencHdl]);
    ...
    s32Ret = memcpy_s(&g_stAencContext.astAencChn[AencHdl].stAencChnAttr, sizeof(HI_MAPI_AENC_ATTR_S), pstAencAttr,
        sizeof(HI_MAPI_AENC_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_AENC, "memcpy_s", s32Ret, g_aencFuncLock[AencHdl]);
    for (i = 0; i < HI_MAPI_AENC_CHN_CALLBACK_MAX_NUM; i++) {
...
HI_S32 HI_MAPI_AENC_Deinit(HI_HANDLE AencHdl)
{
    ...
    CHECK_MAPI_AENC_CHECK_INIT_RET(g_stAencContext.bAencInited);
    if (g_stAencContext.astAencChn[AencHdl].enAencChnState == MAPI_AENC_CHN_STARTED) {
    ...
    MUTEX_DESTROY(g_stAencContext.astAencChn[AencHdl].m_AencChn_lock);
    s32Ret = memset_s(&g_stAencContext.astAencChn[AencHdl].stAencChnAttr, sizeof(HI_MAPI_AENC_ATTR_S), 0,
        sizeof(HI_MAPI_AENC_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_AENC, "memset_s", s32Ret, g_aencFuncLock[AencHdl]);
    for (i = 0; i < HI_MAPI_AENC_CHN_CALLBACK_MAX_NUM; i++) {
        g_stAencContext.astAencChn[AencHdl].astAencRegCallBacks[i].pfnDataCB = HI_NULL;
        g_stAencContext.astAencChn[AencHdl].astAencRegCallBacks[i].pPrivateData = HI_NULL;
    }
...
HI_S32 HI_MAPI_AENC_SetMute(HI_HANDLE AencHdl, HI_BOOL bEnable)
{
    HI_S32 s32Ret;
    ...
    HI_U32 u32ModFd = MODFD(HI_MAPI_MOD_AENC, 0, AencHdl);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_AENC_SET_MUTE, &bEnable, sizeof(HI_BOOL), HI_NULL);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "MSG_CMD_AENC_SET_MUTE fail,s32Ret:%x\n", s32Ret);
        return s32Ret;
    }

    MAPI_DEBUG_TRACE(HI_MAPI_MOD_AENC, "HI_MAPI_AENC_SetMute\n");
    return HI_SUCCESS;
}

HI_S32 HI_MAPI_AENC_GetMute(HI_HANDLE AencHdl, HI_BOOL *pbEnable)
{
    HI_S32 s32Ret;

    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
    CHECK_MAPI_AENC_NULL_PTR_RET(pbEnable);
    CHECK_MAPI_AENC_CHECK_INIT_RET(g_stAencContext.bAencInited);
    ...
    HI_U32 u32ModFd = MODFD(HI_MAPI_MOD_AENC, 0, AencHdl);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_AENC_GET_MUTE, pbEnable, sizeof(HI_BOOL), HI_NULL);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "MSG_CMD_AENC_SET_MUTE fail,s32Ret:%x\n", s32Ret);
        return s32Ret;
    }
...
HI_S32 HI_MAPI_AENC_BindACap(HI_HANDLE AcapHdl, HI_HANDLE AcapChnHdl, HI_HANDLE AencHdl)
{
    ...
    /* check attr and state */
    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
    CHECK_MAPI_AENC_ACAPHANDLE_RANGE_RET(AcapHdl);
    CHECK_MAPI_AENC_ACAPCHNHANDLE_RANGE_RET(AcapChnHdl);
    CHECK_MAPI_AENC_CHECK_INIT_RET(g_stAencContext.bAencInited);
    if (g_stAencContext.astAencChn[AencHdl].enAencChnState == MAPI_AENC_CHN_UNINITED) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "aenc is uninited ,can not bind acap !\n");
        return HI_MAPI_AENC_ESTATEERR;
...
HI_S32 HI_MAPI_AENC_RegisterCallback(HI_HANDLE AencHdl, const HI_MAPI_AENC_CALLBACK_S *pstAencCB)
{
    ...
    /* check attr and state */
    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
    CHECK_MAPI_AENC_NULL_PTR_RET(pstAencCB);
    CHECK_MAPI_AENC_CHECK_INIT_RET(g_stAencContext.bAencInited);
    if (g_stAencContext.astAencChn[AencHdl].enAencChnState == MAPI_AENC_CHN_UNINITED) {
...
HI_S32 HI_MAPI_AENC_SendFrame(HI_HANDLE AencHdl, const AUDIO_FRAME_S *pstFrm)
{
    ...
    /* check attr and state */
    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
    CHECK_MAPI_AENC_NULL_PTR_RET(pstFrm);
    CHECK_MAPI_AENC_CHECK_INIT_RET(g_stAencContext.bAencInited);
