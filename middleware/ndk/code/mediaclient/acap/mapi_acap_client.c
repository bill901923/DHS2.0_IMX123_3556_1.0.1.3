...
HI_S32 HI_MAPI_ACAP_Init(HI_HANDLE AcapHdl, const HI_MAPI_ACAP_ATTR_S *pstACapAttr)
{
    ...
    CHECK_MAPI_ACAP_CHECK_INIT_RET(MAPI_GetMediaInitStatus());

    /* send ipmsg */
    s32Ret = memcpy_s(&stACapAttr, sizeof(HI_MAPI_ACAP_ATTR_S), pstACapAttr, sizeof(HI_MAPI_ACAP_ATTR_S));
                             ...
                             AEC_FRAME_S *pstAecFrm)
{
    ...
    s32Ret = MAPI_ACAP_GetChnNum(g_astAcapContext[AcapHdl].stAcapAttr.enSoundmode, &s32ChnNum);
    if (s32Ret != HI_SUCCESS) {
    ...
    s32Ret = memcpy_s(pstFrm, sizeof(AUDIO_FRAME_S), &(stAcapFrm.stFrm), sizeof(AUDIO_FRAME_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_ACAP, "memcpy_s", s32Ret);
    ...
    if (stAcapFrm.stAecFrm.bValid == HI_TRUE) {
        s32Ret = memcpy_s(pstAecFrm,sizeof(AEC_FRAME_S), &(stAcapFrm.stAecFrm), sizeof(AEC_FRAME_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_ACAP, "memcpy_s", s32Ret);
