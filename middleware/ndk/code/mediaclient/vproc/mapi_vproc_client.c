...
static PORT_DUMP_YUV_INFO_S g_astVpssDumpYuvInfo[HI_MAPI_VPSS_MAX_NUM][HI_MAPI_PHY_VPORT_MAX_NUM +
    HI_MAPI_EXT_VPORT_MAX_NUM];

#ifdef SUPPORT_PHOTO_POST_PROCESS
/* redefine a param to diff from vpss dump YUV */
static VPROC_PHOTO_DUMP_INFO_S g_astPhotoDumpInfo[HI_MAPI_VPSS_MAX_NUM][HI_MAPI_PHY_VPORT_MAX_NUM];
#endif
...
static HI_VOID MAPI_VPROC_ClientStitchReset(HI_VOID)
{
    HI_U32 i, j;
...
static HI_VOID MAPI_VPROC_ClientVpssReset(HI_VOID)
{
    HI_U32 i, j;
    for (i = 0; i < HI_MAPI_VPSS_MAX_NUM; i++) {
        ...
        for (j = 0; j < HI_MAPI_EXT_VPORT_MAX_NUM; j++) {
            g_astVpssDumpYuvInfo[i][j + HI_MAPI_PHY_VPORT_MAX_NUM].bDumpYuvEnable = HI_FALSE;
            g_astVpssDumpYuvInfo[i][j + HI_MAPI_PHY_VPORT_MAX_NUM].pthreadDumpYUV = -1;
            g_astVpssDumpYuvInfo[i][j + HI_MAPI_PHY_VPORT_MAX_NUM].stCallbackFun.pfunVProcYUVProc = HI_NULL;
            g_astVpssDumpYuvInfo[i][j + HI_MAPI_PHY_VPORT_MAX_NUM].bDumpYuvStartd = HI_FALSE;
            g_astVpssDumpYuvInfo[i][j + HI_MAPI_PHY_VPORT_MAX_NUM].stCallbackFun.pPrivateData = HI_NULL;
        }
...
HI_S32 MAPI_VPROC_ClientInit(HI_VOID)
{
    ...
    MAPI_VPROC_ClientVpssReset();
#ifdef SUPPORT_STITCH
    MAPI_VPROC_ClientStitchReset();
#endif

    return HI_SUCCESS;
}

HI_S32 MAPI_VPROC_ClientDeinit(HI_VOID)
{
...
HI_S32 HI_MAPI_VPROC_StartRecvVcapYuv(HI_HANDLE VpssHdl, HI_HANDLE VcapPipeHdl, VI_YUV_DATA_SEQ_E enYuvSeq)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VPROC, VpssHdl, 0);
    stPrivData.as32PrivData[0] = VcapPipeHdl;
    stPrivData.as32PrivData[1] = enYuvSeq;

    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VPROC_START_RECV_VCAP, HI_NULL, 0, &stPrivData);
...
HI_S32 HI_MAPI_VPROC_StopRecvVcapYuv(HI_HANDLE VpssHdl)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VPROC, VpssHdl, 0);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VPROC_STOP_RECV_VCAP, HI_NULL, 0, HI_NULL);
...
HI_S32 HI_MAPI_VPROC_SetExtPortAttr(HI_HANDLE VpssHdl, HI_HANDLE VPortHdl, HI_MAPI_EXT_VPORT_ATTR_S *pstExtVPortAttr)
{
    ...
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VPROC_SET_EXT_VPORT_ATTR, pstExtVPortAttr,
        sizeof(HI_MAPI_EXT_VPORT_ATTR_S), HI_NULL);
...
HI_S32 HI_MAPI_VPROC_GetExtPortAttr(HI_HANDLE VpssHdl, HI_HANDLE VPortHdl,
    HI_MAPI_EXT_VPORT_ATTR_S *pstExtVPortAttr)
{
    HI_S32 s32Ret;
    HI_U32 u32ModFd;
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VPROC, VpssHdl, VPortHdl);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VPROC_GET_EXT_VPORT_ATTR, pstExtVPortAttr, 0, HI_NULL);
...
HI_S32 HI_MAPI_VPROC_SetPortCrop(HI_HANDLE VpssHdl, HI_HANDLE VPortHdl, const VPSS_CROP_INFO_S *pstCropInfo)
{
    ...
    s32Ret = memcpy_s(&stCropInfo, sizeof(VPSS_CROP_INFO_S), pstCropInfo, sizeof(VPSS_CROP_INFO_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VPROC, "memcpy_s", s32Ret);

    u32ModFd = MODFD(HI_MAPI_MOD_VPROC, VpssHdl, VPortHdl);

    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VPROC_SET_VPORT_CROP, &stCropInfo, sizeof(VPSS_CROP_INFO_S),
                           HI_NULL);
...
HI_S32 HI_MAPI_VPROC_GetPortCrop(HI_HANDLE VpssHdl, HI_HANDLE VPortHdl, VPSS_CROP_INFO_S *pstCropInfo)
{
    ...
    u32ModFd = MODFD(HI_MAPI_MOD_VPROC, VpssHdl, VPortHdl);
    s32Ret = MAPI_SendSync(u32ModFd, MSG_CMD_VPROC_GET_VPORT_CROP, pstCropInfo, 0, HI_NULL);
                                    ...
                                    HI_PHOTO_DUMP_CALLBACK_FUNC_S *pstDumpCB)
{
    ...
    if (pstDumpCB->stBNRRawCB.pfunVProcBNRProc != NULL) {
        s32Ret = memcpy_s(&g_astPhotoDumpInfo[VpssHdl][VPortHdl].stBNRRawCB, sizeof(HI_DUMP_BNRRAW_CALLBACK_FUNC_S),
            &pstDumpCB->stBNRRawCB, sizeof(HI_DUMP_BNRRAW_CALLBACK_FUNC_S));
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VPROC, "memcpy_s", s32Ret,
            g_astPhotoDumpInfo[VpssHdl][VPortHdl].PhotoDump_Lock);
        g_astPhotoDumpInfo[VpssHdl][VPortHdl].bPhotoDump = HI_TRUE;
    }

    if (pstDumpCB->stYUVDataCB.pfunVProcYUVProc != NULL) {
        s32Ret = memcpy_s(&g_astPhotoDumpInfo[VpssHdl][VPortHdl].stYUVDataCB, sizeof(HI_DUMP_YUV_CALLBACK_FUNC_S),
...
HI_S32 HI_MAPI_VPROC_UnRegPhotoDumpCB(HI_HANDLE VpssHdl, HI_HANDLE VPortHdl)
{
    ...
    MUTEX_LOCK(g_astPhotoDumpInfo[VpssHdl][VPortHdl].PhotoDump_Lock);
    s32Ret = memset_s(&g_astPhotoDumpInfo[VpssHdl][VPortHdl].stBNRRawCB, sizeof(HI_DUMP_BNRRAW_CALLBACK_FUNC_S), 0x00,
        sizeof(HI_DUMP_BNRRAW_CALLBACK_FUNC_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VPROC, "memset_s", s32Ret, g_astPhotoDumpInfo[VpssHdl][VPortHdl].PhotoDump_Lock);
    s32Ret = memset_s(&g_astPhotoDumpInfo[VpssHdl][VPortHdl].stYUVDataCB, sizeof(HI_DUMP_YUV_CALLBACK_FUNC_S), 0x00,
...
HI_S32 HI_MAPI_VPROC_PhotoProcess(HI_HANDLE VpssHdl, HI_HANDLE VPortHdl)
{
    ...
    if (g_astPhotoDumpInfo[VpssHdl][VPortHdl].bPhotoDump == HI_FALSE) {
        ...
        s32Ret = memset_s(&stPhotoDump, sizeof(VPROC_PHOTO_DUMP_DATA_S), 0x00, sizeof(VPROC_PHOTO_DUMP_DATA_S));
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VPROC, "memset_s", s32Ret,
            g_astPhotoDumpInfo[VpssHdl][VPortHdl].PhotoDump_Lock);
        ...
        for (i = 0; i < s32FrameCnt; i++) {
            s32Ret = memset_s(&stPhotoDump.stYUVData, sizeof(HI_MAPI_FRAME_DATA_S), 0, sizeof(HI_MAPI_FRAME_DATA_S));
            MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VPROC, "memset_s", s32Ret,
                g_astPhotoDumpInfo[VpssHdl][VPortHdl].PhotoDump_Lock);
...
HI_S32 HI_MAPI_VPROC_SetPhotoAlgCoef(PHOTO_ALG_TYPE_E enAlgType, const PHOTO_ALG_COEF_S *pstAlgCoef)
{
    ...
    s32Ret = memcpy_s(&stAlgCoef, sizeof(PHOTO_ALG_COEF_S), pstAlgCoef, sizeof(PHOTO_ALG_COEF_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VPROC, "memcpy_s", s32Ret);
                                      ...
                                      HI_DUMP_YUV_CALLBACK_FUNC_S *pstCallbackFun)
{
    ...
    VPROC_CHECK_NULL_PTR_RET(pstCallbackFun->pPrivateData);
    ...
    s32Ret = memset_s(&stDumpYUVAttr, sizeof(HI_MAPI_DUMP_YUV_ATTR_S), 0x00, sizeof(HI_MAPI_DUMP_YUV_ATTR_S));
