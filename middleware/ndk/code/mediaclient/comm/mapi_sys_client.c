...
HI_S32 HI_MAPI_Media_Deinit()
{
    ...
    s32Ret = MAPI_VPROC_ClientDeinit();
...
HI_S32 HI_MAPI_Sys_SetDatafifoNodeNum(HI_MAPI_DATAFIFO_CONFIG_S *pstDatafifoCfg)
{
    ...
    MAPI_VENC_ClientSetDatafifoNodeNum(pstDatafifoCfg->u32VencDatafifeNodeNum);
                           ...
                           HI_U32 u32Len, const HI_CHAR *pstrName)
{
    ...
    s32Ret = memset_s(stAllocBuffer.aszstrName, SYS_STRNAME_LEN, 0, SYS_STRNAME_LEN);
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_SYS, "memset_s", s32Ret, g_mediaInitLock);

    if (SYS_STRNAME_LEN > strlen(pstrName)) {
        s32Ret = memcpy_s(stAllocBuffer.aszstrName, SYS_STRNAME_LEN, pstrName, strlen(pstrName));
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_SYS, "memcpy_s", s32Ret, g_mediaInitLock);
    } else {
