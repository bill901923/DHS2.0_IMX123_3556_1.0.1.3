...
static HI_S32 MAPI_VENC_GetVStream(HI_HANDLE VencHdl)
{
    ...
    */
    ...
    s32Ret = memcpy_s(&stOutVStream, sizeof(HI_MAPI_VENC_DATA_S), &pstStream->stVencData, sizeof(HI_MAPI_VENC_DATA_S));
...
HI_VOID MAPI_VENC_ClientSetDatafifoNodeNum(HI_U32 u32DatafifoNodeNum)
{
    g_stVencContext.u32DatafifeNodeNum = u32DatafifoNodeNum;
}

static HI_S32 MAPI_VENC_OpenDatafifoReader(HI_HANDLE VencHdl)
{
    HI_S32 s32Ret;
...
static HI_S32 MAPI_VENC_DatafifoReaderClose(HI_HANDLE VencHdl)
{
    HI_S32 s32Ret;
...
HI_S32 MAPI_VENC_ClientInit(HI_VOID)
{
    HI_U32 i;
    HI_S32 s32Ret = HI_SUCCESS;
...
HI_S32 MAPI_VENC_ClientDeinit(HI_VOID)
{
    HI_U32 i;
    HI_S32 s32Ret = HI_SUCCESS;
...
HI_S32 HI_MAPI_VENC_Init(HI_HANDLE VencHdl, const HI_MAPI_VENC_ATTR_S *pstVencAttr)
{
    ...
    s32Ret = MAPI_VENC_OpenDatafifoReader(VencHdl);

    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "MAPI_VENC_OpenDatafifoReader fail,s32Ret:%x\n", s32Ret);
        return s32Ret;
    }
...
HI_S32 HI_MAPI_VENC_Deinit(HI_HANDLE VencHdl)
{
    ...
    s32Ret = MAPI_VENC_DatafifoReaderClose(VencHdl);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "MAPI_VENC_DatafifoReaderClose error,s32Ret:%x\n", s32Ret);
        return s32Ret;
    }

    MUTEX_LOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
    s32Ret = memset_s(&g_stVencContext.astVDatafifo[VencHdl], sizeof(MAPI_VENC_DATAFIFO_S), 0,
        sizeof(MAPI_VENC_DATAFIFO_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
...
HI_S32 HI_MAPI_VENC_RegisterCallback(HI_HANDLE VencHdl, HI_MAPI_VENC_CALLBACK_S *pstVencCB)
{
    ...
    CHECK_MAPI_VENC_CHECK_PREINIT_RET(g_stVencContext.bVencInited);
    CHECK_MAPI_VENC_CHECK_INIT_RET(g_stVencContext.astVencChn[VencHdl].bInited);
...
HI_S32 HI_MAPI_VENC_Start(HI_HANDLE VencHdl, HI_S32 s32FrameCnt)
{
    ...
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, " venc[%d] start failed! s32Ret:%x\n", VencHdl, s32Ret);
        return s32Ret;
    }
...
HI_S32 HI_MAPI_VENC_Stop(HI_HANDLE VencHdl)
{
    ...
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, " venc[%d] stop failed! s32Ret:%x\n", VencHdl, s32Ret);
        return s32Ret;
    }
...
HI_S32 HI_MAPI_VENC_SetAttrEx(HI_HANDLE VencHdl, HI_MAPI_VENC_CMD_E enCMD, HI_VOID *pAttr, HI_U32 U32Len)
{
    ...
    s32Ret = memcpy_s(&stMsgPrivData, sizeof(MAPI_PRIV_DATA_S), &enCMD, sizeof(HI_MAPI_VENC_CMD_E));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "memcpy_s", s32Ret);
