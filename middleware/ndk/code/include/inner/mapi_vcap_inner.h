...
#define SERDES_DEV_FILE_MAX_LEN (128)
#endif
...
#define CHECK_MAPI_VCAP_MIN_VAL_RET(paraname, value, min)                                                       \
    do {                                                                                                    \
        if (value < min) {                                                                                  \
...
#define CHECK_MAPI_VCAP_HANDLE_RET(hdlname, value, max)                                             \
    do {                                                                                        \
...
#define CHECK_MAPI_VCAP_TRUE_RET(value, str, ret)                             \
    do {                                                                  \
        if ((value) != HI_TRUE) {                                           \
...
#define CHECK_MEDIA_VCAP_ALIGN_RET(value, align)                                              \
    do {                                                                                  \
        if ((value % align) != 0) {                                                       \
...
typedef struct tagMAPI_GLOB_PIPE_ATTR_S {
    ...
    HI_HANDLE BindVcHdl; /* <only use for ahd */
...
typedef struct tagMAPI_VCAP_DEFAULT_CONFIG_ATTR_S {
...
} MAPI_VCAP_DEFAULT_CONFIG_ATTR_S;

typedef enum tagMAPI_PIPE_SUPPORT_PIXFMT_E {
    MAPI_PIPE_PIXFMT_RAW_YUV,
    MAPI_PIPE_PIXFMT_RAW,
    MAPI_PIPE_PIXFMT_YUV,
    MAPI_PIPE_PIXFMT_BUTT
} MAPI_PIPE_SUPPORT_PIXFMT_E ;

#ifdef SUPPORT_SERDES
typedef struct HAL_SERDES_DEV_S {
    HI_BOOL bEnableSerdesModule;
    WDR_MODE_E enWdrMode;
} MAPI_HAL_SERDES_DEV_S;
#endif
...
HI_HAL_AHD_DEV_S *MAPI_VCAP_GetAhdDevInfo(HI_HANDLE VcapDevHdl);
MAPI_GLOB_DEV_ATTR_S *MAPI_VCAP_GetDevAttr(HI_HANDLE VcapDevHdl);

#ifdef SUPPORT_SERDES
HI_S32 MAPI_VCAP_GetSerdesWdrMode(HI_HANDLE VcapDevHdl, WDR_MODE_E *penWdrMode);
HI_S32 MAPI_VCAP_SetSerdesWdrMode(HI_HANDLE VcapDevHdl, const WDR_MODE_E* penWdrMode);
HI_S32 MAPI_VCAP_GetSerdesModuleStatus(HI_HANDLE VcapDevHdl, HI_BOOL *pbSerdesModuleStatus);
HI_S32 MAPI_VCAP_SetSerdesModuleStatus(HI_HANDLE VcapDevHdl, const HI_BOOL *pbSerdesModuleStatus);
#endif
