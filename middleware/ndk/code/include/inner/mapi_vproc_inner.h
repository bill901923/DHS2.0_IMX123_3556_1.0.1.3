...
#define VCAPPIPE_CHECK_HANDLE_RET(Hdl)                                                                            \
    do {                                                                                                      \
...
#define VCAPCHN_CHECK_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                    \
...
#define VPSS_CHECK_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                 \
...
#define VPORT_CHECK_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                  \
        if (Hdl >= HI_MAPI_PHY_VPORT_MAX_NUM + HI_MAPI_EXT_VPORT_MAX_NUM) {                               \
...
#define VPORT_CHECK_PHY_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                      \
        if (Hdl >= HI_MAPI_PHY_VPORT_MAX_NUM) {                                                               \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VPROC, "Phy VPort handle(%d) is invalid, only support [0, %d]\n", Hdl, \
                           HI_MAPI_PHY_VPORT_MAX_NUM - 1);                                                    \
            return HI_MAPI_VPROC_EINVALID_CHNID;                                                              \
        }                                                                                                     \
    } while (0)

#define VPORT_CHECK_EXT_HANDLE_RET(Hdl)                                                                                \
    do {                                                                                                           \
        if ((Hdl >= HI_MAPI_PHY_VPORT_MAX_NUM + HI_MAPI_EXT_VPORT_MAX_NUM) || (Hdl < HI_MAPI_PHY_VPORT_MAX_NUM)) { \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VPROC, "Extern VPort handle(%d) is invalid, only support [%d, %d]\n", Hdl,  \
                           HI_MAPI_PHY_VPORT_MAX_NUM, HI_MAPI_PHY_VPORT_MAX_NUM + HI_MAPI_EXT_VPORT_MAX_NUM - 1);  \
            return HI_MAPI_VPROC_EINVALID_CHNID;                                                                   \
        }                                                                                                          \
    } while (0)

#define STITCH_CHECK_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                   \
...
#define STITCHPORT_CHECK_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                       \
...
#define VPROC_OSD_CHECK_HANDLE_RET(Hdl)                                                                           \
    do {                                                                                                      \
...
#define VPROC_CHECK_NULL_PTR_RET(pstPtr)                              \
    do {                                                          \
        if (NULL == (pstPtr)) {                                   \
...
typedef struct tagVPSS_PHYPORT_S {
    HI_BOOL bVPortStart;
...
typedef struct tagVPSS_EXTPORT_S {
    HI_BOOL bVPortStart;
    HI_BOOL bExtVportStart;
    HI_BOOL bDumpYuvEnable;
    HI_DUMP_YUV_CALLBACK_FUNC_S stCallbackFun;
    pthread_t pthreadDumpYUV;
    HI_BOOL bVpssDumpYUVStarted;
...
typedef struct tagVPROC_VPSS_S {
    ...
    VPSS_PHYPORT_ATTR_S astVpssPhyPortAttr[HI_MAPI_PHY_VPORT_MAX_NUM];
    VPSS_EXTPORT_ATTR_S astVpssExtPortAttr[HI_MAPI_EXT_VPORT_MAX_NUM];
} VPROC_VPSS_S;
...
HI_VOID MAPI_VPROC_IveDeInit(HI_HANDLE VpssHdl);
