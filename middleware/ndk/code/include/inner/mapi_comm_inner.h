...
#define MAPI_CHECK_PRINT(Module, str, ret)                                    \
    do {                                                                      \
        if (ret != HI_SUCCESS) {                                              \
            MAPI_ERR_TRACE(Module, "%s fail and return:[%#x]!!\n", str, ret); \
        }                                                                     \
    } while (0)

#define MAPI_CHECK_RET(Module, str, ret)                                      \
    do {                                                                      \
        if (ret != HI_SUCCESS) {                                              \
            MAPI_ERR_TRACE(Module, "%s fail and return:[%#x]!!\n", str, ret); \
...
#define MAPI_CHECK_RET_UNLOCK(Module, str, ret, mLock)                        \
    do {                                                                      \
        if (ret != HI_SUCCESS) {                                              \
            MAPI_ERR_TRACE(Module, "%s fail and return:[%#x]!!\n", str, ret); \
            MUTEX_UNLOCK(mLock);                                              \
