...
#define MAPI_VENC_THREAD_NAME_MAX_LEN 16
...
#define CHECK_MAPI_VENC_NULL_PTR_RET(ptr)                            \
    do {                                                             \
        if (ptr == NULL) {                                           \
...
#define CHECK_MAPI_VENC_CHECK_PREINIT_RET(state)                                       \
    do {                                                                               \
        if (!state) {                                                                  \
...
#define CHECK_MAPI_VENC_CHECK_PREINIT_UNLOCK_RET(state)                                \
    do {                                                                               \
        if (!state) {                                                                  \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "media not init yet,venc no pre init\n"); \
...
#define CHECK_MAPI_VENC_CHECK_INIT_RET(state)                               \
    do {                                                                    \
        if (!state) {                                                       \
...
#define CHECK_MAPI_VENC_CHECK_INIT_UNLOCK_RET(state)                        \
    do {                                                                    \
        if (!state) {                                                       \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "VENC has not been inited\n"); \
            MUTEX_UNLOCK(g_Venc_FuncLock[VencHdl]);                         \
...
#define CHECK_MAPI_VCAPPIPE_HANDLE_RET(handle)                                                                 \
    do {                                                                                                       \
        if (handle >= HI_MAPI_VCAP_MAX_PIPE_NUM) {                                                             \
...
#define CHECK_MAPI_PIPECHN_HANDLE_RET(handle)                                                                 \
    do {                                                                                                      \
        if (handle >= HI_MAPI_PIPE_MAX_CHN_NUM) {                                                             \
...
#define CHECK_MAPI_VPSS_HANDLE_RET(handle)                                                                 \
    do {                                                                                                   \
        if (handle >= HI_MAPI_VPSS_MAX_NUM) {                                                              \
...
#define CHECK_MAPI_VPORT_HANDLE_RET(handle)                                                                 \
    do {                                                                                                    \
        if (handle >= (HI_MAPI_PHY_VPORT_MAX_NUM + HI_MAPI_EXT_VPORT_MAX_NUM)) {                            \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "VPort handle(%d) is invalid, only support [0, %d]\n", handle, \
                           HI_MAPI_PHY_VPORT_MAX_NUM + HI_MAPI_EXT_VPORT_MAX_NUM - 1);                      \
            return HI_MAPI_VENC_EHANDLE_ILLEGAL;                                                            \
        }                                                                                                   \
    } while (0)

#define CHECK_MAPI_STITCH_HANDLE_RET(handle)                                                                 \
    do {                                                                                                     \
        if (handle >= HI_MAPI_STITCH_MAX_NUM) {                                                              \
...
#define CHECK_MAPI_STITCHPORT_HANDLE_RET(handle)                                                                 \
    do {                                                                                                         \
        if (handle >= HI_MAPI_STITCH_PORT_MAX_NUM) {                                                             \
...
#define CHECK_MAPI_VENC_HANDLE_RANGE_RET(handle)                            \
    do {                                                                    \
        if (handle >= HI_MAPI_VENC_MAX_CHN_NUM) {                           \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "VENC handle out of range\n"); \
            return HI_MAPI_VENC_EHANDLE_ILLEGAL;                            \
        }                                                                   \
    } while (0)

#define CHECK_MAPI_VENC_FRAMECNT_RANGE_RET(framecnt)                          \
    do {                                                                      \
        if (framecnt < HI_MAPI_VENC_LIMITLESS_FRAME_COUNT || framecnt == 0) { \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "frame count out of range\n");   \
            return HI_MAPI_VENC_EILLEGAL_PARAM;                               \
        }                                                                     \
    } while (0)

#define CHECK_MAPI_VENC_OSD_HANDLE_RANGE_RET(handle)                            \
    do {                                                                        \
        if (handle >= HI_MAPI_VENC_OSD_MAX_NUM) {                               \
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "VENC OSD handle out of range\n"); \
            return HI_MAPI_VENC_EHANDLE_ILLEGAL;                                \
        }                                                                       \
    } while (0)

#define CHECK_MAPI_VENC_MAX_VAL_RET(paraname, atc, max)                                                     \
    do {                                                                                                    \
        if (atc > max) {                                                                                    \
...
#define CHECK_MAPI_VENC_MIN_VAL_RET(paraname, atc, min)                                                   \
    do {                                                                                                  \
        if (atc < min) {                                                                                  \
...
#define CHECK_MEDIA_VENC_ALIGN_RET(value, align)                                          \
    do {                                                                                  \
        if ((value % align) != 0) {                                                       \
...
HI_VOID MAPI_VENC_ClientSetDatafifoNodeNum(HI_U32 u32DatafifeNodeNum);
