...
typedef struct hiMAPI_APERTURE_INFO_S {
    /* <RW, Range:[F32.0, F1.0], Max F number of Piris's aperture, it's related to the specific iris */
    ISP_IRIS_F_NO_E enMaxIrisFNOTarget;

    /* <RW, Range:[F32.0, F1.0], Min F number of Piris's aperture, it's related to the specific iris */
    ISP_IRIS_F_NO_E enMinIrisFNOTarget;

    /* <RW, use equivalent gain to present FNO or not */
    HI_BOOL bFNOExValid;

    /* <RW, Range:[1, 1024], Max equivalent gain of F number of Piris's aperture, only used
        when bFNOExValid is true, it's related to the specific iris */
    HI_U32 u32MaxIrisFNOTarget;

    /* <RW, Range:[1, 1024], Min equivalent gain of F number of Piris's aperture, only used
        when bFNOExValid is true, it's related to the specific iris */
...
typedef struct hiMAPI_SNAP_EXIF_INFO_S {
    ...
    HI_U8 au8Software[HI_MAPI_EXIF_DRSCRIPTION_LENGTH];         /* <Shows firmware (internal software of
                                                                    digital cameras) version number */
...
typedef struct hiMAPI_VCAP_DEV_ATTR_S {
    SIZE_S stBasSize;     /* <CAP dev0 expected scale resolution, 1/2, 1/3, only when
                              PIPE0 binding Dev0 BAS to take effect, ISP's width and
                              height needs to be set to the BAS output width and height */
    VI_WDR_ATTR_S stWDRAttr;
    VI_INTF_MODE_E enIntfMode;  /* <Interface mode */
    HI_U32 au32ComponentMask[VI_COMPMASK_NUM]; /* <Component mask */
    VI_YUV_DATA_SEQ_E enDataSeq; /* <Input data sequence (only the YUV format is supported) */
    VI_DATA_TYPE_E      enInputDataType; /* <Input data type */
} HI_MAPI_VCAP_DEV_ATTR_S;

typedef struct hiMAPI_PIPE_CHN_ATTR_S {
    SIZE_S stDestSize;              /* <dynamic attribute, channel Output resolution setting,
...
typedef struct hiMAPI_INPUT_DEV_ATTR_S {
    input_mode_t inputMode;
    data_type_t inputDataType;
    HI_BOOL bInputSerdes;
}HI_MAPI_INPUT_DEV_ATTR_S;

typedef struct hiMAPI_SENSOR_ATTR_S {
    HI_U32 u32SnsMode;
    WDR_MODE_E enWdrMode;
    SIZE_S stSize;
    HI_MAPI_INPUT_DEV_ATTR_S stInputDevAttr;
...
typedef struct hiMAPI_SNAP_NORMAL_ATTR_S {
    /* <Snap frame number [0,0xFFFFFFFF] */
    HI_U32 u32FrameCnt;

    /* <Set PIPE to send first frame repeatedly when offline(Set 2 when defog, set 1 for the others) */
...
typedef enum hiMAPI_VCAP_CMD_E {
    ...
    HI_MAPI_VCAP_CMD_ISP_PipeDiffAttr,  /* <ISP Expand attributes:Multiple Splicing Differences
                                            ISP Pipe Discrepancy Attributes */
    HI_MAPI_VCAP_CMD_ISP_WDR,           /* <ISP Expand attributes:WDR Attributes, You can set
                                            the synthesis method andits parameters,Related to
                                            ghosting problems, indoor frequency flicker problem,
    ...
    HI_MAPI_VCAP_CMD_LDCV3,            /* <CHN Expand attributes:LDC */
...
typedef struct hiMAPI_VCAP_METERINGMODE_S {
    /* RW; Range:[0x0, 0xF]; Format:4.0; AE weighting table */
    HI_U8 au8Weight[HI_MAPI_VCAP_AE_ZONE_ROW][HI_MAPI_VCAP_AE_ZONE_COLUMN];
