...
static HI_BOOL VENC_GetDatafifoStatus(HI_HANDLE VencHdl)
{
    return g_astVDatafifo[VencHdl].bUsed;
}

static HI_S32 VENC_ReleaseVStream(HI_HANDLE VencHdl,
                                        MAPI_VENC_HISILINK_STREAM_S *pstOutLINK_VStream)
{
    HI_S32 s32Ret;
    CHECK_MAPI_VENC_NULL_PTR_RET(pstOutLINK_VStream);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstOutLINK_VStream->pPriv);
...
static HI_S32 VENC_DatafifoDataProc(HI_HANDLE VencHdl, MAPI_VENC_HISILINK_STREAM_S *pVDDCStreamData)
{
    HI_S32 s32Ret;
...
static HI_VOID VENC_DatafifoReleaseStream(MAPI_VENC_HISILINK_STREAM_S *pstLinkStream)
{
    if (pstLinkStream != HI_NULL) {
        HI_S32 s32Ret = VENC_ReleaseVStream(pstLinkStream->handle, pstLinkStream);
...
static HI_S32 VENC_LinkVStreamProcess(MAPI_VENC_HISILINK_STREAM_S *pstVencLinkStream,
...
static HI_VOID VENC_LinkConfigVStreamInfo(HI_HANDLE VencHdl,
...
HI_VOID MAPI_VENC_OSInit(HI_VOID)
{
    ...
    for (i = 0; i < HI_MAPI_VENC_MAX_CHN_NUM; i++) {
        s32Ret = memset_s(&g_astVDatafifo[i], sizeof(MAPI_VENC_DATAFIFO_S), 0, sizeof(MAPI_VENC_DATAFIFO_S));
        if (s32Ret != HI_SUCCESS) {
                                    ...
                                    MAPI_VENC_DATAFIFO_S *pstVEncDatafifo)
{
    ...
    CHECK_MAPI_VENC_NULL_PTR_RET(pstVEncDatafifo);

    if (VENC_GetDatafifoStatus(VencHdl) == HI_TRUE) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VENC, "datafifo has been opened\n");
        return HI_SUCCESS;
    }

    s32Ret = memset_s(&stStreamBufInfo, sizeof(VENC_STREAM_BUF_INFO_S), 0, sizeof(VENC_STREAM_BUF_INFO_S));
    ...
    memcpy_s(pstVEncDatafifo, sizeof(MAPI_VENC_DATAFIFO_S), &g_astVDatafifo[VencHdl], sizeof(MAPI_VENC_DATAFIFO_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memcpy_s", s32Ret, g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
    g_astVDatafifo[VencHdl].stVLinkDataInfo.u32LinkFrmNum = 0;
    g_astVDatafifo[VencHdl].stVLinkDataInfo.u32LastFrmSeq = 0;
    g_astVDatafifo[VencHdl].pFnVencHisiLinkDataProc = VENC_DatafifoDataProc;
...
{
    /*
       datafifo is not support reentry, if pFnVencHisiLinkDataProc(,NULL)
       occur reentry, it may be cause the same frame released twice.
    */
    MUTEX_LOCK(g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
    HI_S32 s32Ret;

    if (g_astVDatafifo[VencHdl].pFnVencHisiLinkDataProc != HI_NULL
        && VENC_GetDatafifoStatus(VencHdl) == HI_TRUE) {
...
HI_S32 MAPI_VENC_ClearDatafifoFrame(HI_HANDLE VencHdl)
{
    ...
    if (g_astVDatafifo[VencHdl].stUnhandleStream.pPriv != NULL) {
        s32Ret = VENC_ReleaseVStream(VencHdl, &g_astVDatafifo[VencHdl].stUnhandleStream);
        if (s32Ret != HI_SUCCESS) {
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC,
                           "VENC_ReleaseVStream fail,VencHdl:%d,s32Ret:%x\n", VencHdl, s32Ret);
            MUTEX_UNLOCK(g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
            return s32Ret;
        }
        s32Ret = memset_s(&g_astVDatafifo[VencHdl].stUnhandleStream, sizeof(MAPI_VENC_HISILINK_STREAM_S), 0,
            sizeof(MAPI_VENC_HISILINK_STREAM_S));
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
    }
...
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 s32SafeFuncRet = HI_SUCCESS;
    ...
    if (g_astVDatafifo[VencHdl].stUnhandleStream.pPriv == NULL) {
        ...
        VENC_LinkConfigVStreamInfo(VencHdl, &stOutLINK_VStream, stVencChnAttr.stVencAttr.enType, pstVStream,
                                   &stVencChnStat);
        s32Ret = VENC_LinkVStreamProcess(&stOutLINK_VStream, pstVencChn);

        if (s32Ret != HI_SUCCESS) {
            s32SafeFuncRet = memcpy_s(&g_astVDatafifo[VencHdl].stUnhandleStream,  sizeof(MAPI_VENC_HISILINK_STREAM_S),
                &stOutLINK_VStream, sizeof(MAPI_VENC_HISILINK_STREAM_S));
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "VENC_LinkVStreamProcess error,s32Ret:%x\n", s32Ret);
            MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memcpy_s", s32SafeFuncRet,
                g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
            MUTEX_UNLOCK(g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
            return s32Ret;
        }
    } else {
        /* there is unhandled stream last time */
        s32Ret = VENC_LinkVStreamProcess(&g_astVDatafifo[VencHdl].stUnhandleStream, pstVencChn);
    ...
    if (g_astVDatafifo[VencHdl].stUnhandleStream.pPriv != NULL) {
        s32SafeFuncRet = memset_s(&g_astVDatafifo[VencHdl].stUnhandleStream, sizeof(MAPI_VENC_HISILINK_STREAM_S), 0,
            sizeof(MAPI_VENC_HISILINK_STREAM_S));
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32SafeFuncRet, g_astVDatafifo[VencHdl].m_DatafifoChn_lock);
    }
