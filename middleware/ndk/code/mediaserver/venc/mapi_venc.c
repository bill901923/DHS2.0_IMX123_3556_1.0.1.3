...
#include "mapi_venc_os.h"
#include "mapi_venc_arch.h"
...
HI_VOID *ProcVDataThread(HI_VOID *arg)
{
    ...
    HI_CHAR szThreadName[MAPI_VENC_THREAD_NAME_MAX_LEN] = {0};
    fd_set read_fds;
    struct timeval stTimeoutVal;
    stTimeoutVal.tv_sec = 0;
    stTimeoutVal.tv_usec = VENC_GET_FRAME_THREAD_SLEECT_TIMEOUT_USEC;

    s32Ret = snprintf_s(szThreadName, MAPI_VENC_THREAD_NAME_MAX_LEN, MAPI_VENC_THREAD_NAME_MAX_LEN, "Hi_pTVData_Chn%u",
        VencHdl);
    if (s32Ret == HI_FAILURE)
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "snprintf_s fail and return:[%#x]!!\n", s32Ret);
        return HI_NULL;
    }
                                          ...
                                          VENC_RC_ATTR_S *pstRcChnAttr)
{
    ...
    CHECK_MAPI_VENC_NULL_PTR_RET(pstRcChnAttr);
...
static HI_S32 MAPI_VENC_ConfigAttr(const HI_MAPI_VENC_ATTR_S *pstVencAttr, VENC_CHN_ATTR_S *pstVencChnAttr)
{
    HI_U32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    CHECK_MAPI_VENC_NULL_PTR_RET(pstVencAttr);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstVencChnAttr);
...
HI_S32 MAPI_VENC_Init(const HI_MAPI_VENC_PARAM_MOD_S *pstVencModPara)
{
    ...
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "memset_s", s32Ret);
    s32Ret = memset_s(&g_stVencContext, sizeof(MAPI_VENC_CONTEXT_S), 0, sizeof(MAPI_VENC_CONTEXT_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "memset_s", s32Ret);
...
HI_S32 MAPI_VENC_Deinit(HI_VOID)
{
    ...
    for (i = 0; i < HI_MAPI_VENC_MAX_CHN_NUM; i++) {
        ...
        g_stVencContext.astVencChn[i].s32StartFrameCount = HI_MAPI_VENC_LIMITLESS_FRAME_COUNT;
        s32Ret = memset_s(g_stVencContext.astVencChn[i].astVencRegisterCallbacks,
            sizeof(HI_MAPI_VENC_CALLBACK_S) * HI_MAPI_VENC_CALLBACK_MAX_NUM, 0,
            sizeof(HI_MAPI_VENC_CALLBACK_S) * HI_MAPI_VENC_CALLBACK_MAX_NUM);
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_stVencContext.astVencChn[i].m_VencChn_lock);
        s32Ret = memset_s(g_stVencContext.astVencChn[i].abOsdStatus,
            sizeof(VENC_OSD_STATUS_S) * HI_MAPI_VENC_OSD_MAX_NUM, 0,
            sizeof(VENC_OSD_STATUS_S) * HI_MAPI_VENC_OSD_MAX_NUM);
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_stVencContext.astVencChn[i].m_VencChn_lock);
        MUTEX_UNLOCK(g_stVencContext.astVencChn[i].m_VencChn_lock);
    ...
    s32Ret = memset_s(g_stVencContext.astProcVDataThread,
        sizeof(MAPI_VENC_PROCVDATA_THREAD_S) * HI_MAPI_VENC_MAX_CHN_NUM, 0,
        sizeof(MAPI_VENC_PROCVDATA_THREAD_S) * HI_MAPI_VENC_MAX_CHN_NUM);
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "memset_s", s32Ret);
...
HI_S32 HI_MAPI_VENC_Init(HI_HANDLE VencHdl, const HI_MAPI_VENC_ATTR_S *pstVencAttr)
{
    ...
    s32Ret = memset_s(&stVencChnAttr, sizeof(VENC_CHN_ATTR_S), 0, sizeof(VENC_CHN_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "memset_s", s32Ret);
    s32Ret = memset_s(&stVencParamChn, sizeof(VENC_CHN_PARAM_S), 0, sizeof(VENC_CHN_PARAM_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "memset_s", s32Ret);
    stVencChnAttr.stGopAttr.stNormalP.s32IPQpDelta = 2; /* Recommended value from PQ tool debugging */

    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstVencAttr);

    MUTEX_LOCK(g_Venc_FuncLock[VencHdl]);

    CHECK_MAPI_VENC_CHECK_PREINIT_UNLOCK_RET(g_stVencContext.bVencInited);
    ...
    s32Ret = HAL_MAPI_VENC_ConfigReEncodeTimes(VencHdl, pstVencAttr->stVencPloadTypeAttr.enType,
        pstVencAttr->stRcAttr.enRcMode);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "HAL_MAPI_VENC_ConfigReEncodeTimes error,s32Ret:%x\n", s32Ret);
    ...
    MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "1. stVencParamChn.stCropCfg.bEnable%d\n", stVencParamChn.stCropCfg.bEnable);
    MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "stVencParamChn.stCropCfg  %d %d %d %d \n", 
    stVencParamChn.stCropCfg.stRect.s32X,
    stVencParamChn.stCropCfg.stRect.s32Y,
    stVencParamChn.stCropCfg.stRect.u32Height,
    stVencParamChn.stCropCfg.stRect.u32Width
    );
    MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "HI_MPI_VENC_DestroyChn error,s32Ret2:%x\n", stVencParamChn.stCropCfg.bEnable);
    ...
    MUTEX_LOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
    s32Ret = memcpy_s(&g_stVencContext.astVencChn[VencHdl].stVencAttr, sizeof(HI_MAPI_VENC_ATTR_S), pstVencAttr,
        sizeof(HI_MAPI_VENC_ATTR_S));
    if (s32Ret != HI_SUCCESS)
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
        MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
        MUTEX_UNLOCK(g_Venc_FuncLock[VencHdl]);
...
HI_S32 HI_MAPI_VENC_Deinit(HI_HANDLE VencHdl)
{
    ...
    CHECK_MAPI_VENC_CHECK_PREINIT_RET(g_stVencContext.bVencInited);
    ...
    MUTEX_LOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
    s32Ret = memset_s(&g_stVencContext.astVencChn[VencHdl].stVencAttr, sizeof(HI_MAPI_VENC_ATTR_S), 0,
        sizeof(HI_MAPI_VENC_ATTR_S));
    if (s32Ret != HI_SUCCESS)
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
        MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
        MUTEX_UNLOCK(g_Venc_FuncLock[VencHdl]);
        return HI_MAPI_SYS_ESAFEFUNC_OPERATE_FAIL;
    }
...
HI_S32 HI_MAPI_VENC_RegisterCallback(HI_HANDLE VencHdl, HI_MAPI_VENC_CALLBACK_S *pstVencCB)
{
    ...
    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstVencCB);

    MUTEX_LOCK(g_Venc_FuncLock[VencHdl]);

    CHECK_MAPI_VENC_CHECK_PREINIT_UNLOCK_RET(g_stVencContext.bVencInited);
    CHECK_MAPI_VENC_CHECK_INIT_UNLOCK_RET(g_stVencContext.astVencChn[VencHdl].bInited);
...
HI_S32 HI_MAPI_VENC_Start(HI_HANDLE VencHdl, HI_S32 s32FrameCnt)
{
    ...
    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VENC_FRAMECNT_RANGE_RET(s32FrameCnt);
...
HI_S32 HI_MAPI_VENC_BindVProc(HI_HANDLE VProcHdl, HI_HANDLE VPortHdl, HI_HANDLE VencHdl, HI_BOOL bStitch)
{
    ...
    if (bStitch == HI_TRUE) {
        CHECK_MAPI_STITCH_HANDLE_RET(VProcHdl);
        CHECK_MAPI_STITCHPORT_HANDLE_RET(VPortHdl);
    } else {
        CHECK_MAPI_VPSS_HANDLE_RET(VProcHdl);
        CHECK_MAPI_VPORT_HANDLE_RET(VPortHdl);
    }
    ...
    s32Ret = memset_s(&stSrcTempChn, sizeof(MPP_CHN_S), HI_INVALID_HANDLE, sizeof(MPP_CHN_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_Venc_FuncLock[VencHdl]);
...
HI_S32 HI_MAPI_VENC_BindVCap(HI_HANDLE VcapPipeHdl, HI_HANDLE PipeChnHdl, HI_HANDLE VencHdl)
{
    HI_S32 s32Ret = HI_SUCCESS;
    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VCAPPIPE_HANDLE_RET(VcapPipeHdl);
    CHECK_MAPI_PIPECHN_HANDLE_RET(PipeChnHdl);
...
HI_S32 HI_MAPI_VENC_SetAttr(HI_HANDLE VencHdl, const HI_MAPI_VENC_ATTR_S *pstStreamAttr)
{
    HI_S32 s32Ret;
    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstStreamAttr);
    ...
    s32Ret = memset_s(&stChnAttr, sizeof(VENC_CHN_ATTR_S), 0x0, sizeof(VENC_CHN_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_Venc_FuncLock[VencHdl]);
    ...
    s32Ret = HAL_MAPI_VENC_ConfigReEncodeTimes(VencHdl, pstStreamAttr->stVencPloadTypeAttr.enType,
        pstStreamAttr->stRcAttr.enRcMode);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "HAL_MAPI_VENC_ConfigReEncodeTimes error,s32Ret:%x\n", s32Ret);
        MUTEX_UNLOCK(g_Venc_FuncLock[VencHdl]);
        return s32Ret;
    ...
    MUTEX_LOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
    s32Ret = memcpy_s(&g_stVencContext.astVencChn[VencHdl].stVencAttr, sizeof(HI_MAPI_VENC_ATTR_S), pstStreamAttr,
        sizeof(HI_MAPI_VENC_ATTR_S));
    if (s32Ret != HI_SUCCESS)
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
...
HI_S32 HI_MAPI_VENC_GetAttr(HI_HANDLE VencHdl, HI_MAPI_VENC_ATTR_S *pstStreamAttr)
{
    ...
    CHECK_MAPI_VENC_CHECK_PREINIT_UNLOCK_RET(g_stVencContext.bVencInited);
    CHECK_MAPI_VENC_CHECK_INIT_UNLOCK_RET(g_stVencContext.astVencChn[VencHdl].bInited);
    s32Ret = memcpy_s(pstStreamAttr, sizeof(HI_MAPI_VENC_ATTR_S), &g_stVencContext.astVencChn[VencHdl].stVencAttr,
        sizeof(HI_MAPI_VENC_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memcpy_s", s32Ret, g_Venc_FuncLock[VencHdl]);
...
HI_S32 HI_MAPI_VENC_GetAttrEx(HI_HANDLE VencHdl, HI_MAPI_VENC_CMD_E enCMD, HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VENC_NULL_PTR_RET(pAttr);
    ...
    switch (enCMD) {
        ...
        case HI_MAPI_VENC_CMD_MJPEG_RC_ATTR_EX: {
            ...
            s32Ret = memset_s(&stRcParam, sizeof(VENC_RC_PARAM_S), 0, sizeof(VENC_RC_PARAM_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stRcParam, sizeof(VENC_RC_PARAM_S));
            if (s32Ret != HI_SUCCESS) {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
                MUTEX_UNLOCK(g_Venc_FuncLock[VencHdl]);
        ...
        case HI_MAPI_VENC_CMD_GOP_MODE: {
            ...
            s32Ret = memset_s(&stVencGopAttr, sizeof(HI_MAPI_VENC_GOP_ATTR_S), 0x0, sizeof(HI_MAPI_VENC_GOP_ATTR_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stVencGopAttr, sizeof(HI_MAPI_VENC_GOP_ATTR_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
        ...
        case HI_MAPI_VENC_CMD_INTRA_REFRESH: {
            ...
            s32Ret = memset_s(&stVencParamIntraRefresh, sizeof(VENC_INTRA_REFRESH_S), 0x0,
                sizeof(VENC_INTRA_REFRESH_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stVencParamIntraRefresh, sizeof(VENC_INTRA_REFRESH_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
        ...
        case HI_MAPI_VENC_CMD_FRAME_RATE: {
            ...
            VENC_CHN_PARAM_S stVencParamChn;
            s32Ret = memset_s(&stVencFrameRate, sizeof(VENC_FRAME_RATE_S), 0x0, sizeof(VENC_FRAME_RATE_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
            ...
            s32Ret = memset_s(&stVencParamChn, sizeof(VENC_CHN_PARAM_S), 0x0, sizeof(VENC_CHN_PARAM_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
            ...
            s32Ret = HI_MPI_VENC_GetChnParam(VencHdl, &stVencParamChn);
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "HI_MPI_VENC_GetChnParam\n");
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "2. stVencParamChn.stCropCfg.bEnable %d\n", stVencParamChn.stCropCfg.bEnable);
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "stVencParamChn.stCropCfg  %d %d %d %d \n", 
            stVencParamChn.stCropCfg.stRect.s32X,
            stVencParamChn.stCropCfg.stRect.s32Y,
            stVencParamChn.stCropCfg.stRect.u32Height,
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stVencFrameRate, sizeof(VENC_FRAME_RATE_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
        ...
        case HI_MAPI_VENC_CMD_HIERARCHICAL_QP: {
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stVencHierarchicalQp, sizeof(VENC_HIERARCHICAL_QP_S));
            if (s32Ret != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memcpy_s fail and return:[%#x]\n", s32Ret);
                MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
        ...
        case HI_MAPI_VENC_CMD_FRAMELOST_STRATEGY: {
            ...
            if (g_stVencContext.astVencChn[VencHdl].stVencAttr.stVencPloadTypeAttr.enType == HI_MAPI_PAYLOAD_TYPE_H264
                ...
                s32Ret = memset_s(&stFrmLostParam, sizeof(VENC_FRAMELOST_S), 0, sizeof(VENC_FRAMELOST_S));
                if (s32Ret != HI_SUCCESS)
                {
                    MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "memset_s fail and return:[%#x]\n", s32Ret);
                    MUTEX_UNLOCK(g_stVencContext.astVencChn[VencHdl].m_VencChn_lock);
...
HI_S32 HI_MAPI_VENC_SetAttrEx(HI_HANDLE VencHdl, HI_MAPI_VENC_CMD_E enCMD, HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    switch (enCMD) {
        ...
        MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "3. stVencParamChn.stCropCfg.bEnable%d\n", stVencParamChn.stCropCfg.bEnable);
            MAPI_ERR_TRACE(HI_MAPI_MOD_VENC, "stVencParamChn.stCropCfg  %d %d %d %d \n", 
            stVencParamChn.stCropCfg.stRect.s32X,
            stVencParamChn.stCropCfg.stRect.s32Y,
            stVencParamChn.stCropCfg.stRect.u32Height,
...
HI_S32 HI_MAPI_VENC_SetOSDAttr(HI_HANDLE VencHdl, HI_HANDLE OSDHdl, HI_MAPI_OSD_ATTR_S *pstOSDAttr)
{
    ...
    CHECK_MAPI_VENC_HANDLE_RANGE_RET(VencHdl);
    CHECK_MAPI_VENC_OSD_HANDLE_RANGE_RET(OSDHdl);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstOSDAttr);
    CHECK_MAPI_VENC_MIN_VAL_RET("stBitmapAttr.enPixelFormat", pstOSDAttr->stBitmapAttr.enPixelFormat,
        PIXEL_FORMAT_ARGB_1555);
    CHECK_MAPI_VENC_MAX_VAL_RET("stBitmapAttr.enPixelFormat", pstOSDAttr->stBitmapAttr.enPixelFormat,
        PIXEL_FORMAT_ARGB_4444);
    CHECK_MAPI_VENC_NULL_PTR_RET(pstOSDAttr->stBitmapAttr.pData);
    CHECK_MAPI_VENC_MIN_VAL_RET("stBitmapAttr.u32Width", pstOSDAttr->stBitmapAttr.u32Width, HI_MAPI_MIN_OSD_BMP_W);
    CHECK_MAPI_VENC_MIN_VAL_RET("stBitmapAttr.u32Height", pstOSDAttr->stBitmapAttr.u32Height, HI_MAPI_MIN_OSD_BMP_H);
    CHECK_MAPI_VENC_MAX_VAL_RET("stBitmapAttr.u32Width", pstOSDAttr->stBitmapAttr.u32Width, HI_MAPI_MAX_OSD_BMP_W);
    CHECK_MAPI_VENC_MAX_VAL_RET("stBitmapAttr.u32Height", pstOSDAttr->stBitmapAttr.u32Height, HI_MAPI_MAX_OSD_BMP_H);
    CHECK_MAPI_VENC_MAX_VAL_RET("stOsdDisplayAttr.u32BgAlpha", pstOSDAttr->stOsdDisplayAttr.u32BgAlpha,
        HI_MAPI_MAX_OSD_ALPHA);
    CHECK_MAPI_VENC_MAX_VAL_RET("stOsdDisplayAttr.u32FgAlpha", pstOSDAttr->stOsdDisplayAttr.u32FgAlpha,
        HI_MAPI_MAX_OSD_ALPHA);
    CHECK_MAPI_VENC_MIN_VAL_RET("stOsdDisplayAttr.s32RegionX", pstOSDAttr->stOsdDisplayAttr.s32RegionX,
        HI_MAPI_MIN_OSD_REGION_X);
    CHECK_MAPI_VENC_MIN_VAL_RET("stOsdDisplayAttr.s32RegionY", pstOSDAttr->stOsdDisplayAttr.s32RegionY,
        HI_MAPI_MIN_OSD_REGION_Y);
    CHECK_MAPI_VENC_MAX_VAL_RET("stOsdDisplayAttr.s32RegionX", pstOSDAttr->stOsdDisplayAttr.s32RegionX,
        HI_MAPI_MAX_OSD_REGION_X);
    CHECK_MAPI_VENC_MAX_VAL_RET("stOsdDisplayAttr.s32RegionY", pstOSDAttr->stOsdDisplayAttr.s32RegionY,
        HI_MAPI_MAX_OSD_REGION_Y);
    CHECK_MAPI_VENC_MIN_VAL_RET("u32CanvasNum", pstOSDAttr->u32CanvasNum, HI_MAPI_RGN_BUF_MIN_NUM);
    CHECK_MAPI_VENC_MAX_VAL_RET("u32CanvasNum", pstOSDAttr->u32CanvasNum, HI_MAPI_RGN_BUF_MAX_NUM);
    CHECK_MEDIA_VENC_ALIGN_RET(pstOSDAttr->stBitmapAttr.u32Width, HI_MAPI_OSD_BMP_W_ALIGN);
    CHECK_MEDIA_VENC_ALIGN_RET(pstOSDAttr->stBitmapAttr.u32Height, HI_MAPI_OSD_BMP_H_ALIGN);
    CHECK_MEDIA_VENC_ALIGN_RET(pstOSDAttr->stOsdDisplayAttr.s32RegionX, HI_MAPI_OSD_REGION_X_ALIGN);
    CHECK_MEDIA_VENC_ALIGN_RET(pstOSDAttr->stOsdDisplayAttr.s32RegionY, HI_MAPI_OSD_REGION_Y_ALIGN);
    ...
    if (g_stVencContext.astVencChn[VencHdl].abOsdStatus[OSDHdl].bOsdStart == HI_TRUE) {
        ...
        s32Ret = memset_s(&stRgnChnAttr, sizeof(RGN_CHN_ATTR_S), 0x0, sizeof(RGN_CHN_ATTR_S));
        MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_VENC, "memset_s", s32Ret, g_Venc_FuncLock[VencHdl]);
...
HI_S32 HI_MAPI_VENC_StopOSD(HI_HANDLE VencHdl, HI_HANDLE OSDHdl)
{
    ...
    s32Ret = memset_s(&g_stVencContext.astVencChn[VencHdl].astOsdAttr[OSDHdl], sizeof(HI_MAPI_OSD_ATTR_S), 0x0,
