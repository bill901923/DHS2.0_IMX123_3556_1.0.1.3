...
HI_VOID *ProcAStreamThread(HI_VOID *arg)
{
    ...
    while (g_stAencContext.astProcADataThread[ChnHdl].bProcADataThdStartFlg == HI_TRUE) {
        ...
        if (g_stAencContext.astAencChn[ChnHdl].enAencChnState != MAPI_AENC_CHN_STARTED) {
            ...
            MAPI_AENC_CondTimeWait(&g_stAencContext.astProcADataThread[ChnHdl].c_ProcADataThd_cond,
...
HI_S32 MAPI_AENC_Init(HI_VOID)
{
    ...
    s32Ret = memset_s(&g_stAencContext, sizeof(MAPI_AENC_CHN_CONTEXT_S), 0, sizeof(MAPI_AENC_CHN_CONTEXT_S));
...
HI_S32 MAPI_AENC_Deinit(HI_VOID)
{
    ...
    MAPI_AENC_OSDeinit();
    for (i = 0; i < HI_MAPI_AENC_CHN_MAX_NUM; i++) {
        s32Ret = memset_s(&g_stAencContext.astAencChn[i], sizeof(MAPI_AENC_CHN_S), 0, sizeof(MAPI_AENC_CHN_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_AENC, "memset_s", s32Ret);
        g_stAencContext.astAencChn[i].s32AiChn = HI_INVALID_HANDLE;
        g_stAencContext.astAencChn[i].s32AiDev = HI_INVALID_HANDLE;
...
HI_S32 HI_MAPI_AENC_Init(HI_HANDLE AencHdl, const HI_MAPI_AENC_ATTR_S *pstAencAttr)
{
    ...
    s32Ret = memset_s(&stAencChnAttr, sizeof(AENC_CHN_ATTR_S), 0, sizeof(AENC_CHN_ATTR_S));
    ...
    CHECK_MAPI_AENC_NULL_PTR_RET(pstAencAttr->pValue);
...
HI_S32 HI_MAPI_AENC_Deinit(HI_HANDLE AencHdl)
{
    ...
    MUTEX_DESTROY(g_stAencContext.astAencChn[AencHdl].m_AencChn_lock);
    s32Ret = memset_s(&g_stAencContext.astAencChn[AencHdl].stAencChnAttr, sizeof(HI_MAPI_AENC_ATTR_S), 0x0,
        sizeof(HI_MAPI_AENC_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_AENC, "memset_s", s32Ret, g_aencFuncLock[AencHdl]);
    for (i = 0; i < HI_MAPI_AENC_CHN_CALLBACK_MAX_NUM; i++) {
        g_stAencContext.astAencChn[AencHdl].astAencRegCallBacks[i].pfnDataCB = HI_NULL;
...
HI_S32 HI_MAPI_AENC_SetMute(HI_HANDLE AencHdl, HI_BOOL bEnable)
{
    ...
    if (g_stAencContext.astAencChn[AencHdl].enAencChnState == MAPI_AENC_CHN_UNINITED) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "aenc is deinited ,can not be SetMute!\n");
        MUTEX_UNLOCK(g_aencFuncLock[AencHdl]);
        return HI_MAPI_AENC_ESTATEERR;
    }
    ...
    HI_S32 s32Ret;
    s32Ret = HI_MPI_AENC_SetMute(AencHdl, bEnable);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "HI_MPI_AENC_SetMute err:0x%x\n", s32Ret);
        MUTEX_UNLOCK(g_aencFuncLock[AencHdl]);
        return s32Ret;
    }
#else
    MAPI_UNUSED(bEnable);
    MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "the vension is not support HI_MAPI_AENC_SetMute\n");
#endif
    MUTEX_UNLOCK(g_aencFuncLock[AencHdl]);
...
#ifdef __HI3559V200__
    HI_S32 s32Ret;
    s32Ret = HI_MPI_AENC_GetMute(AencHdl, pbEnable);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "HI_MPI_AENC_SetMute err:0x%x\n", s32Ret);
        MUTEX_UNLOCK(g_aencFuncLock[AencHdl]);
        return s32Ret;
