...
HI_VOID MAPI_AENC_OSInit(HI_VOID)
{
    ...
    for (i = 0; i < HI_MAPI_AENC_CHN_MAX_NUM; i++) {
        s32Ret = memset_s(&g_astADatafifo[i], sizeof(MAPI_AENC_DATAFIFO_S), 0, sizeof(MAPI_AENC_DATAFIFO_S));
        MAPI_CHECK_PRINT(HI_MAPI_MOD_AENC, "memset_s", s32Ret);
...
HI_U32 MAPI_AENC_GetLinkDataInfo(HI_HANDLE AencHdl, MAPI_AENC_LINKDATA_INFO_S *pstLinkDataInfo)
{
    ...
    CHECK_MAPI_AENC_NULL_PTR_RET(pstLinkDataInfo);
...
HI_S32 MAPI_AENC_GetAStream(HI_HANDLE AencHdl, const MAPI_AENC_CHN_S *pstAencChn)
{
    ...
    if (g_astADatafifo[AencHdl].stUnhandleStream.pPriv == HI_NULL) {
        ...
        /* process frame */
        s32SafeFuncRet = memcpy_s(&stAencOutLinkStream.stAencData, sizeof(AUDIO_STREAM_S), pstAencStream,
            sizeof(AUDIO_STREAM_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_AENC, "memcpy_s", s32SafeFuncRet);
        ...
        if (s32Ret != HI_SUCCESS) {
            s32SafeFuncRet = memcpy_s(&g_astADatafifo[AencHdl].stUnhandleStream, sizeof(MAPI_AENC_HISILINK_STREAM_S),
                &stAencOutLinkStream, sizeof(MAPI_AENC_HISILINK_STREAM_S));
            if (s32SafeFuncRet != HI_SUCCESS)
            {
                MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "memcpy_s err,s32Ret:%x\n", s32SafeFuncRet);
            }
    ...
    if (g_astADatafifo[AencHdl].stUnhandleStream.pPriv != HI_NULL) {
        s32SafeFuncRet = memset_s(&g_astADatafifo[AencHdl].stUnhandleStream, sizeof(MAPI_AENC_HISILINK_STREAM_S), 0,
            sizeof(MAPI_AENC_HISILINK_STREAM_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_AENC, "memset_s", s32SafeFuncRet);
    }
...
HI_VOID MAPI_AENC_ReleaseUnhandleStream(HI_HANDLE AencHdl)
{
    ...
    if (g_astADatafifo[AencHdl].stUnhandleStream.pPriv != HI_NULL) {
        ...
        s32Ret = memset_s(&g_astADatafifo[AencHdl].stUnhandleStream, sizeof(MAPI_AENC_HISILINK_STREAM_S), 0,
            sizeof(MAPI_AENC_HISILINK_STREAM_S));
        if (s32Ret != HI_SUCCESS)
        {
            MAPI_ERR_TRACE(HI_MAPI_MOD_AENC, "memset_s err,s32Ret:%x\n", s32Ret);
        }
    }
}
...
HI_S32 MAPI_AENC_OpenDatafifoWriter(HI_HANDLE AencHdl, MAPI_AENC_DATAFIFO_S *pstAencDatafifo)
{
    ...
    CHECK_MAPI_AENC_HANDLE_RANGE_RET(AencHdl);
    CHECK_MAPI_AENC_NULL_PTR_RET(pstAencDatafifo);
    ...
    s32Ret = memcpy_s(pstAencDatafifo, sizeof(MAPI_AENC_DATAFIFO_S), &g_astADatafifo[AencHdl],
        sizeof(MAPI_AENC_DATAFIFO_S));
