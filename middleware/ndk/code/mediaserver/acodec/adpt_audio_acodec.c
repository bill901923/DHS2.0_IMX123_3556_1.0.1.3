...
HI_S32 ADPT_ACODEC_MuteAI()
{
    ...
    /* vol_ctrl_mute, 1:mute , 0:unmute */
    ACODEC_VOL_CTRL acodecCtl;
    if (ioctl(g_s32AcodecFd, ACODEC_GET_ADCR_VOL, &acodecCtl))
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_ACAP, "ACODEC_GET_ADCR_VOL failed\n");
        return HI_FAILURE;
    }

    acodecCtl.vol_ctrl_mute = 0x1;
    if (ioctl(g_s32AcodecFd, ACODEC_SET_ADCR_VOL, &acodecCtl))
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_ACAP, "ACODEC_SET_ADCR_VOL failed\n");
        return HI_FAILURE;
    }

    if (ioctl(g_s32AcodecFd, ACODEC_GET_ADCL_VOL, &acodecCtl))
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_ACAP, "ACODEC_GET_ADCL_VOL failed\n");
        return HI_FAILURE;
    }

    acodecCtl.vol_ctrl_mute = 0x1;
    if (ioctl(g_s32AcodecFd, ACODEC_SET_ADCL_VOL, &acodecCtl))
    {
        MAPI_ERR_TRACE(HI_MAPI_MOD_ACAP, "ACODEC_SET_ADCL_VOL failed\n");
        return HI_FAILURE;
    }
