...
static HI_VOID MAPI_ACAP_DeInitResampleFun(HI_VOID)
{
    ...
    if (g_stResampleFun.pLibHandle != HI_NULL) {
        ...
        s32Ret = memset_s(&g_stResampleFun, sizeof(MAPI_RESAMPLE_FUN_S), 0, sizeof(MAPI_RESAMPLE_FUN_S));
        if (s32Ret != HI_SUCCESS) {
            MAPI_ERR_TRACE(HI_MAPI_MOD_ACAP, "memset_s fail and return:[%#x]!!\n", s32Ret);
        }
    }
...
static HI_S32 MAPI_ACAP_InitResampleFun(HI_VOID)
{
    ...
    s32Ret = memset_s(&stResampleFun, sizeof(MAPI_RESAMPLE_FUN_S), 0, sizeof(MAPI_RESAMPLE_FUN_S));
...
;
    ...
    s32Ret = memcpy_s(&g_stResampleFun, sizeof(MAPI_RESAMPLE_FUN_S), &stResampleFun, sizeof(MAPI_RESAMPLE_FUN_S));
...
HI_VOID *HAL_MAPI_ACAP_ResamplerCreate(HI_S32 s32Inrate, HI_S32 s32Outrate, HI_S32 s32Chans)
{
    HI_S32 s32Ret;
...
HI_VOID HAL_MAPI_ACAP_ResamplerDestroy(HI_VOID *inst)
{
    if (g_stResampleFun.pLibHandle != NULL) {
...
HI_S32 HAL_MAPI_ACAP_ResamplerProcess(HI_VOID *inst, HI_S16 *s16Inbuf, HI_S32 s32Insamps, HI_S16 *s16Outbuf)
{
    if (g_stResampleFun.pLibHandle == NULL) {
