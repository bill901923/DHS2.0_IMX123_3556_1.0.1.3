...
#define MAPI_LIST_HEAD_INIT(name) \
    {                        \
        &(name), &(name)     \
    }

#define MAPI_LIST_HEAD(name) \
    struct cbb_list_head name = MAPI_LIST_HEAD_INIT(name)
#define MAPI_INIT_LIST_HEAD(ptr)  \
    do {                     \
        (ptr)->next = (ptr); \
        (ptr)->prev = (ptr); \
    } while (0)
