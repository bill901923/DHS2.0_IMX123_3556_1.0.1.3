...
MAPI_PIPE_SUPPORT_PIXFMT_E HAL_MAPI_VCAP_GetPipeSupportPixFmt(HI_HANDLE VcapPipeHdl)
{
    if (VcapPipeHdl == 2) {
        return MAPI_PIPE_PIXFMT_RAW_YUV;
    } else {
        return MAPI_PIPE_PIXFMT_RAW;
    }
}

HI_S32 HAI_MAPI_VCAP_LoadMipiConfig(combo_dev_attr_t *pComboDevAttr, const HI_MAPI_MIPI_INTF_S *pstSensorInf,
                                    const HI_MAPI_SENSOR_ATTR_S *pstSensorAttr, short aLaneId[])
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 i;

    switch (pstSensorAttr->stInputDevAttr.inputMode) {
        case INPUT_MODE_LVDS:

            s32Ret = memcpy_s(&pComboDevAttr->lvds_attr, sizeof(lvds_dev_attr_t), &pstSensorInf->lvds_attr,
                sizeof(lvds_dev_attr_t));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);

            for (i = 0; i < LVDS_LANE_NUM; i++) {
                pComboDevAttr->lvds_attr.lane_id[i] = aLaneId[i];
            }
            pComboDevAttr->lvds_attr.input_data_type = pstSensorAttr->stInputDevAttr.inputDataType;
            break;

        case INPUT_MODE_MIPI:

            s32Ret = memcpy_s(&pComboDevAttr->mipi_attr, sizeof(mipi_dev_attr_t), &pstSensorInf->mipi_attr,
                sizeof(mipi_dev_attr_t));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);

            for (i = 0; i < MIPI_LANE_NUM; i++) {
                pComboDevAttr->mipi_attr.lane_id[i] = aLaneId[i];
            }
            pComboDevAttr->mipi_attr.input_data_type = pstSensorAttr->stInputDevAttr.inputDataType;
            break;
        ...
        default:
        {
            MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "sensor input mode:%d not support,please check!\n",
                pstSensorAttr->stInputDevAttr.inputMode);
