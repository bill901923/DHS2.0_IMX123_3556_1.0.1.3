...
HI_S32 MAPI_VCAP_SetMotionSensor(const HI_MAPI_MOTIONSENSOR_INFO_S *pstMotionAttr)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstMotionAttr);
...
static HI_S32 MAPI_VCAP_InitMotionSensor(HI_MAPI_MOTIONSENSOR_INFO_S *pstMotionAttr)
{
    ...
    s32Ret = memset_s(&stMotionBufAttr, sizeof(MSENSOR_BUF_ATTR_S), 0x00, sizeof(MSENSOR_BUF_ATTR_S));
    ...
    s32Ret = memset_s((HI_VOID *)stMotionBufAttr.pVirAddr, u32BufSize, 0, u32BufSize);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    s32Ret = memset_s(&stMotionSet, sizeof(MSENSOR_PARAM_S), 0, sizeof(MSENSOR_PARAM_S));
    ...
    s32Ret = memcpy_s(&stMotionSet.stMSensorBufAttr, sizeof(MSENSOR_BUF_ATTR_S), &stMotionBufAttr,
        sizeof(MSENSOR_BUF_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);

    s32Ret = ioctl(fd, MSENSOR_CMD_INIT, &stMotionSet);
    if (s32Ret != HI_SUCCESS) {
