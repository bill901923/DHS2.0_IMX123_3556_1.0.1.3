...
HI_S32 HI_MPI_AE_Ctrl(ALG_LIB_S *pstAeLib, HI_U32 u32Cmd, HI_VOID *pValue);
HI_S32 HI_MPI_AWB_Ctrl(ALG_LIB_S *pstAwbLib, HI_U32 u32Cmd, HI_VOID *pValue);
...
static HI_VOID MAPI_VCAP_ResetStatus(HI_VOID)
{
    ...
    for (i = 0; i < HI_MAPI_VCAP_MAX_DEV_NUM; i++) {
        ...
        g_stVcapGlobAttr.astGlobDevAttr[i].bSensorReset = HI_FALSE;
    ...
    for (i = 0; i < HI_MAPI_VCAP_MAX_PIPE_NUM; i++) {
        g_stVcapGlobAttr.astGlobPipeAttr[i].bIspInit = HI_FALSE;
        ...
        g_stVcapGlobAttr.astGlobPipeAttr[i].BindVcHdl = HI_INVALID_HANDLE;
        g_stVcapGlobAttr.astGlobPipeAttr[i].bSnapPipeEnable = HI_FALSE;
        g_stVcapGlobAttr.astGlobPipeAttr[i].ISPHdl = VCAP_PIPE_0 + i;

        s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[i].stVcapISPInitAttr, sizeof(VCAP_ISP_INIT_ATTR_S), 0,
            sizeof(VCAP_ISP_INIT_ATTR_S));
        ...
        s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[i].stPipeAttr, sizeof(PIPE_ATTR_S), 0, sizeof(PIPE_ATTR_S));
        MAPI_CHECK_PRINT(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);

        s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[i].stDumpRawInfo, sizeof(VCAP_DUMP_RAWDATA_INFO_S), 0,
            sizeof(VCAP_DUMP_RAWDATA_INFO_S));
        MAPI_CHECK_PRINT(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
        s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[i].stSnapAttr, sizeof(HI_MAPI_VCAP_SNAP_ATTR_S), 0,
            sizeof(HI_MAPI_VCAP_SNAP_ATTR_S));
        MAPI_CHECK_PRINT(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
        ...
        for (j = 0; j < HI_MAPI_PIPE_MAX_CHN_NUM; j++) {
            ...
            s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[i].astPipeChnAttr[j].stChnAttr,
                sizeof(HI_MAPI_PIPE_CHN_ATTR_S), 0, sizeof(HI_MAPI_PIPE_CHN_ATTR_S));
            MAPI_CHECK_PRINT(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
            ...
            for (k = 0; k < HI_MAPI_VCAP_OSD_MAX_NUM; k++) {
                g_stVcapGlobAttr.astGlobPipeAttr[i].astPipeChnAttr[j].astVcapOSDAttr[k].enOSDStatus =
                    VCAP_STATUS_NOTINIT;
                s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[i].astPipeChnAttr[j].astVcapOSDAttr[k].stOSDAttr,
                    sizeof(HI_MAPI_OSD_ATTR_S), 0, sizeof(HI_MAPI_OSD_ATTR_S));
                MAPI_CHECK_PRINT(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
            }
        }
    }
...
HI_S32 MAPI_ISPModParam_Init(const HI_MAPI_ISP_PARAM_MOD_S *pstIspModParam)
{
#ifndef __HI3559AV100__
    /* current 59av100 is not support */
    HI_S32 s32Ret;
    ISP_MOD_PARAM_S stIspModPara;

    CHECK_MAPI_VCAP_NULL_PTR_RET(pstIspModParam);

    if (pstIspModParam->bIspSetModPara != HI_TRUE) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "IspSetModPara not need\n");
        return HI_SUCCESS;
    }

    s32Ret = memset_s(&stIspModPara, sizeof(stIspModPara), 0x00, sizeof(stIspModPara));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);

    s32Ret = HI_MPI_ISP_GetModParam(&stIspModPara);
    CHECK_MAPI_VCAP_RET(s32Ret, "HI_MPI_ISP_GetModParam fail\n");

    stIspModPara.u32QuickStart = pstIspModParam->bQuickStart;
    stIspModPara.u32IntBotHalf = pstIspModParam->bIntBotHalf;
    s32Ret = HI_MPI_ISP_SetModParam(&stIspModPara);
    CHECK_MAPI_VCAP_RET(s32Ret, "HI_MPI_ISP_SetModParam fail\n");
#else
    MAPI_UNUSED(pstIspModParam);
#endif
...
HI_S32 MAPI_VCAP_Init(HI_VOID)
{
    ...
    s32Ret = memset_s(&g_stVcapGlobAttr, sizeof(MAPI_VCAP_GLOB_ATTR_S), 0, sizeof(MAPI_VCAP_GLOB_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    ...
    if (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].enSensorScene != HI_MAPI_SENSOR_SCENE_NORMAL) {
        ...
        if (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.enWdrMode != pstSensorAttr->enWdrMode) {
            ...
            if ((g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stSize.u32Width !=
                ...
                || (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.u32SnsMode != pstSensorAttr->u32SnsMode)) {
                                                ...
                                                const HI_MAPI_SENSOR_ATTR_S *pstSensorAttr, HI_S32 *ps32CurrSnsSeq)
{
    ...
    for (i = 0; i < s32SensorModeCnt; i++) {
        if ((HI_U32)pstSensorAttrs[i].s32Width == pstSensorAttr->stSize.u32Width
            ...
            && pstSensorAttrs[i].u32SnsMode == pstSensorAttr->u32SnsMode
...
static HI_S32 MAPI_VCAP_SetSerdes(HI_HANDLE VcapDevHdl, HI_MAPI_SENSOR_ATTR_S *pstSensorAttr, HI_BOOL bEnableSerdes)
{
    HI_S32 s32Ret;
    HI_U32 u32LaneNum;
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSensorAttr);
    ...
    CHECK_MAPI_VCAP_RET(s32Ret, "no this dev[%d] type:%d \n", VcapDevHdl, g_as32SensorType[VcapDevHdl]);

    /* judge the serdes mode */
    u32LaneNum = MAPI_VCAP_GetLaneNum(VcapDevHdl, &stSensorCfg);

    if ((u32LaneNum == 4) && (pstSensorAttr->enWdrMode == WDR_MODE_NONE)) {
        stSerdesAttr.serdes_mode = SERDES_MODE_4LANE_LINEAR;
    } else if ((u32LaneNum == 4) && (pstSensorAttr->enWdrMode != WDR_MODE_NONE)) {
        stSerdesAttr.serdes_mode = SERDES_MODE_4LANE_WDR;
    } else if ((u32LaneNum == 2) && (pstSensorAttr->enWdrMode == WDR_MODE_NONE)) {
        stSerdesAttr.serdes_mode = SERDES_MODE_2LANE_LINEAR;
    } else if ((u32LaneNum == 2) && (pstSensorAttr->enWdrMode != WDR_MODE_NONE)) {
        stSerdesAttr.serdes_mode = SERDES_MODE_2LANE_WDR;
    } else {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "laneNum(%d) WdrMode(%d) is not support.\n", u32LaneNum, pstSensorAttr->enWdrMode);
        return HI_MAPI_VCAP_ENOTSUPPORT;
    }

    stSerdesAttr.devno = (serdes_dev_t)VcapDevHdl;
    stSerdesAttr.sendes_en = bEnableSerdes;
    stSerdesAttr.sensor_i2c_addr = stSensorCfg.sensor_i2c_addr;

    s32Ret = snprintf_s(acDevFile, SERDES_DEV_FILE_MAX_LEN, SERDES_DEV_FILE_MAX_LEN, "/dev/serdes%d", VcapDevHdl);
...
static HI_S32 MAPI_VCAP_StartMipi(HI_HANDLE VcapDevHdl, const combo_dev_attr_t *pstComboDevAttr, HI_MAPI_SENSOR_ATTR_S *pstSensorAttr, HI_BOOL bResetSensor)
{
    HI_S32 fd;
    HI_S32 i;
...
#ifdef SUPPORT_SERDES
    if (pstSensorAttr->stInputDevAttr.bInputSerdes == HI_TRUE) {
        s32Ret = MAPI_VCAP_SetSerdes(VcapDevHdl, pstSensorAttr, HI_TRUE);
        if (s32Ret != HI_SUCCESS) {
            MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "start serdes failed\n");
            return s32Ret;
        }
    }
#else
    MAPI_UNUSED(pstSensorAttr);
    MAPI_INFO_TRACE(HI_MAPI_MOD_VCAP, "serdes is not support\n");
#endif
...
static HI_S32 MAPI_VCAP_StopMipi(HI_HANDLE VcapDevHdl, HI_BOOL bResetSensor)
{
    HI_S32 fd;
    HI_S32 s32Ret;
    HI_MAPI_COMBO_DEV_ATTR_S stSensorCfg;
...
#ifdef SUPPORT_SERDES
    /* 5.stop serdes */
    if (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.bInputSerdes == HI_TRUE) {
        s32Ret = MAPI_VCAP_SetSerdes(VcapDevHdl, &g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr, HI_FALSE);
        if (s32Ret != HI_SUCCESS) {
            MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "stop serdes failed\n");
            goto CLOSE_MIPI_DEV_NODE_ERR;
        }
    }
#else
                                       ...
                                       combo_dev_attr_t *pComboDevAttr)
{
    ...
    s32Ret = MAPI_SENSOR_GetConfig(g_as32SensorType[VcapDevHdl], &stSensorCfg);
    CHECK_MAPI_VCAP_RET(s32Ret, "no this sensor[%d] type:%d \n", VcapDevHdl, g_as32SensorType[VcapDevHdl]);
    ...
    pComboDevAttr->input_mode = pstSensorAttr->stInputDevAttr.inputMode;
    if (g_as32SensorType[VcapDevHdl] != AHD) {
        /*
            dc sensor used from sensor cfg temporary
            this case will be removed later
        */
        pComboDevAttr->input_mode = stSensorCfg.input_mode;
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "dc sensor's input_mode get from sensor cfg temporary \n");
    }

    s32Ret = HAI_MAPI_VCAP_LoadMipiConfig(pComboDevAttr, &pstSenIntf[s32SnsModeSeq], pstSensorAttr,
    ...
    if (g_as32SensorType[VcapDevHdl] != AHD && pComboDevAttr->input_mode == INPUT_MODE_MIPI) {
        /*
            dc sensor used from sensor cfg temporary
            this case will be removed later
        */
        pComboDevAttr->mipi_attr.input_data_type = pstSenIntf[s32SnsModeSeq].mipi_attr.input_data_type;
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "dc sensor's input_data_type get from sensor cfg temporary \n");
    }
...
static HI_BOOL MAPI_VCAP_IsDevAttrChange(HI_HANDLE VcapDevHdl, const HI_MAPI_VCAP_DEV_ATTR_S *pstDevAttr)
{
    ...
    if (pstGlobDevAttr->stWDRAttr.enWDRMode != pstDevAttr->stWDRAttr.enWDRMode
...
static HI_S32 MAPI_VCAP_ISP_SnsRegisterCallback(HI_HANDLE VcapPipeHdl, const ISP_SNS_OBJ_S *pstSnsObj)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSnsObj);
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSnsObj->pfnRegisterCallback);
    ...
    s32Ret = strncpy_s(stAeLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AE_LIB_NAME, ALG_LIB_NAME_SIZE_MAX);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "strncpy_s", s32Ret);
    s32Ret = strncpy_s(stAwbLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AWB_LIB_NAME, ALG_LIB_NAME_SIZE_MAX);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "strncpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_ISP_3A_Register(HI_HANDLE VcapPipeHdl)
{
    ...
    stLib.s32Id = ViPipe;
    s32Ret = strncpy_s(stLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AE_LIB_NAME, ALG_LIB_NAME_SIZE_MAX);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "strncpy_s", s32Ret);
    ...
    stLib.s32Id = ViPipe;
    s32Ret = strncpy_s(stLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AWB_LIB_NAME, ALG_LIB_NAME_SIZE_MAX);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "strncpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_AdaptPipePixFmtFromSensorCfg(HI_HANDLE VcapPipeHdl,
                                                     VI_PIPE_ATTR_S *pstPipeAttr)
{
    VI_DEV ViDev;
    HI_MAPI_SENSOR_ATTR_S *pstSensorAttr;

    ViDev = (VI_DEV)g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].BindDevHdl;
    pstSensorAttr = &g_stVcapGlobAttr.astGlobDevAttr[ViDev].stSensorAttr;
    if (g_as32SensorType[ViDev] != AHD) {
        switch (pstSensorAttr->stInputDevAttr.inputDataType) {
            ...
            default:
                MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "sensor not support input data type:%d error!\n",
                    pstSensorAttr->stInputDevAttr.inputDataType);
                return  HI_FAILURE;
        }
    ...
    pstPipeAttr->enBitWidth = DATA_BITWIDTH_8;
    MAPI_PIPE_SUPPORT_PIXFMT_E enSupportPixFmt = HAL_MAPI_VCAP_GetPipeSupportPixFmt(VcapPipeHdl);
    if (g_stVcapGlobAttr.astGlobDevAttr[ViDev].stSensorAttr.stInputDevAttr.inputMode ==  INPUT_MODE_BT656) {
        pstPipeAttr->bYuvSkip = HI_TRUE;
        if (enSupportPixFmt == MAPI_PIPE_PIXFMT_RAW) {
            /* caution: the pipe u32MaxW should double, eg sensor input 1920*1080, then u32MaxW is 1920*2 */
            pstPipeAttr->enPixFmt = PIXEL_FORMAT_RGB_BAYER_8BPP;
        } else {
            pstPipeAttr->enPixFmt = PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        }

    } else if (g_stVcapGlobAttr.astGlobDevAttr[ViDev].stSensorAttr.stInputDevAttr.inputMode == INPUT_MODE_MIPI) {
        if (enSupportPixFmt == MAPI_PIPE_PIXFMT_RAW) {
            pstPipeAttr->enPixFmt = PIXEL_FORMAT_RGB_BAYER_16BPP;
        } else {
            pstPipeAttr->enPixFmt = PIXEL_FORMAT_YVU_SEMIPLANAR_420;
        ...
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "not support this input mode:%d when the sensor is ahd!\n",
            g_stVcapGlobAttr.astGlobDevAttr[ViDev].stSensorAttr.stInputDevAttr.inputMode);
        return  HI_FAILURE;
    }
...
static HI_S32 MAPI_VCAP_ISP_SetCommParam(HI_HANDLE VcapPipeHdl, const ISP_PUB_ATTR_S *pstPubAttr)
{
    ...
    /* set isp pubattr in isp running status, call HI_MPI_ISP_QueryInnerStateInfo
           make sure the set operation is finished. */
    if ((g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].enSensorScene == HI_MAPI_SENSOR_SCENE_STANDBY)
        ...
        s32Ret = memset_s(&stInnerStateInfo, sizeof(ISP_INNER_STATE_INFO_S), 0x00, sizeof(ISP_INNER_STATE_INFO_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
                                      ...
                                      HI_MAPI_COMBO_DEV_ATTR_S *pstSensorCfg)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSensorCfg);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].bSensorInit, "sensor has not been inited",
    ...
    /* Transformate from u8 to u32 may cause data missing , but it depends on sdk */
    stPubAttr.u8SnsMode = (HI_U8)pSensorMode[s32CurrSnsSeq].u32SnsMode;
...
static HI_S32 MAPI_VCAP_ResetISP(HI_HANDLE VcapDevHdl, HI_HANDLE VcapPipeHdl)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].bSensorInit, "sensor has not been inited",
        HI_MAPI_VCAP_ENOT_INITED);
    ...
    s32Ret = memcpy_s(&stISPInitAttr, sizeof(ISP_INIT_ATTR_S),
        &g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].stVcapISPInitAttr.stISPInitAttr, sizeof(ISP_INIT_ATTR_S));
...
static HI_VOID *MAPI_VCAP_ISPRunProc(HI_VOID *pPipeHandl)
{
    ...
    s32Ret = snprintf_s(aszThreadName, HI_MAPI_PTHREAD_NAME_LEN, HI_MAPI_PTHREAD_NAME_LEN, "Hi_pTIspRun%d", ViPipe);
    if (s32Ret == HI_FAILURE) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "snprintf_s error:[%#x]", s32Ret);
        return HI_NULL;
    }
...
static HI_S32 MAPI_VCAP_GetMasterISPId(HI_HANDLE VcapDevHdl)
{
    ...
    s32Ret = memcpy_s(stDevBindPipe.PipeId, sizeof(VI_PIPE) * VI_MAX_PIPE_NUM,
        g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].aPipeIdBind, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_DynamicSetPipeAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_ATTR_S *pstVCapAttr)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstPipeAttr);
    CHECK_MAPI_VCAP_TRUE_RET(!MAPI_VCAP_IsPipeStaticAttrChange(VcapPipeHdl, pstPipeAttr),
...
static HI_S32 MAPI_VCAP_DynamicSetISPAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_ATTR_S *pstVCapAttr)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(psIspAttr);
    CHECK_MAPI_VCAP_TRUE_RET(!MAPI_VCAP_IsISPStaticPubAttrChange(VcapPipeHdl, psIspAttr),
        "don't support modify ISP pub static Attributes in the dev running status", HI_MAPI_VCAP_ENOT_PERM);
                                          ...
                                          HI_MAPI_VCAP_ATTR_S *pstVCapAttr)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstChnAttr);

    if (g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].astPipeChnAttr[PipeChnHdl].bChnEnable == HI_FALSE) {
        s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].astPipeChnAttr[PipeChnHdl].stChnAttr,
            sizeof(HI_MAPI_PIPE_CHN_ATTR_S), pstChnAttr, sizeof(HI_MAPI_PIPE_CHN_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        return HI_SUCCESS;
    }

    CHECK_MAPI_VCAP_TRUE_RET(!MAPI_VCAP_IsChnStaticAttrChange(VcapPipeHdl, PipeChnHdl, pstChnAttr),
...
static HI_VOID MAPI_VCAP_LoadParam(MAPI_VCAP_DEFAULT_CONFIG_ATTR_S *pstVcap, const HI_S32 s32SensorType[])
{
    HI_U32 i;

    for (i = 0; i <= HI_MAPI_VCAP_MAX_DEV_NUM; i++) {
        pstVcap->astDevAttr[i].as32AdChnId[0] = HI_INVALID_HANDLE;
        pstVcap->astDevAttr[i].as32AdChnId[1] = HI_INVALID_HANDLE;
        pstVcap->astDevAttr[i].as32AdChnId[2] = HI_INVALID_HANDLE;
        pstVcap->astDevAttr[i].as32AdChnId[3] = HI_INVALID_HANDLE;
        ...
        pstVcap->astDevAttr[i].stSynCfg.stTimingBlank.u32VsyncVbact = 0;
        pstVcap->astDevAttr[i].bDataReverse = HI_FALSE;
        pstVcap->astDevAttr[i].enScanMode = VI_SCAN_PROGRESSIVE;
        pstVcap->astDevAttr[i].enWorkMode = VI_WORK_MODE_1Multiplex;
        pstVcap->astDevAttr[i].stBasAttr.stRephaseAttr.enHRephaseMode = VI_REPHASE_MODE_NONE;
        pstVcap->astDevAttr[i].stBasAttr.stRephaseAttr.enVRephaseMode = VI_REPHASE_MODE_NONE;

        if (s32SensorType[i] == AHD) {
            pstVcap->astDevAttr[i].stSynCfg.enVsync = VI_VSYNC_PULSE;
            pstVcap->astDevAttr[i].stSynCfg.enVsyncNeg = VI_HSYNC_NEG_HIGH;
            pstVcap->astDevAttr[i].stSynCfg.enHsync = VI_HSYNC_VALID_SINGNAL;
            pstVcap->astDevAttr[i].stSynCfg.enHsyncNeg = VI_HSYNC_NEG_HIGH;
            pstVcap->astDevAttr[i].stSynCfg.enVsyncValid = VI_VSYNC_PULSE;
            pstVcap->astDevAttr[i].stSynCfg.enVsyncValidNeg = VI_VSYNC_VALID_NEG_HIGH;

        } else {
            pstVcap->astDevAttr[i].stSynCfg.enVsync = VI_VSYNC_PULSE;
            pstVcap->astDevAttr[i].stSynCfg.enVsyncNeg = VI_VSYNC_NEG_LOW;
            pstVcap->astDevAttr[i].stSynCfg.enHsync = VI_HSYNC_VALID_SINGNAL;
            pstVcap->astDevAttr[i].stSynCfg.enHsyncNeg = VI_HSYNC_NEG_HIGH;
            pstVcap->astDevAttr[i].stSynCfg.enVsyncValid = VI_VSYNC_VALID_SINGAL;
            pstVcap->astDevAttr[i].stSynCfg.enVsyncValidNeg = VI_VSYNC_VALID_NEG_HIGH;
        }
    }
...
static HI_S32 MAPI_VCAP_StaticSetAttr(HI_HANDLE VcapDevHdl, const HI_MAPI_VCAP_ATTR_S *pstVCapAttr)
{
    ...
    MAPI_VCAP_DEFAULT_CONFIG_ATTR_S stVcapCfg;
    HI_MAPI_COMBO_DEV_ATTR_S stSensorCfg;
    ...
    s32Ret = memset_s(&stVcapCfg, sizeof(MAPI_VCAP_DEFAULT_CONFIG_ATTR_S), 0, sizeof(MAPI_VCAP_DEFAULT_CONFIG_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    MAPI_VCAP_LoadParam(&stVcapCfg, g_as32SensorType);
    s32Ret = memcpy_s(&stDevAttr, sizeof(VI_DEV_ATTR_S), &stVcapCfg.astDevAttr[ViDev], sizeof(VI_DEV_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    stDevAttr.stWDRAttr.enWDRMode = pstVCapAttr->stVcapDevAttr.stWDRAttr.enWDRMode;
    stDevAttr.stWDRAttr.u32CacheLine = pstVCapAttr->stVcapDevAttr.stWDRAttr.u32CacheLine;  // check at SDK
    ...
    stDevAttr.enIntfMode = pstVCapAttr->stVcapDevAttr.enIntfMode;
    stDevAttr.enDataSeq = pstVCapAttr->stVcapDevAttr.enDataSeq;
    stDevAttr.enInputDataType = pstVCapAttr->stVcapDevAttr.enInputDataType;
    for (i = 0; i < VI_COMPMASK_NUM; i++) {
        stDevAttr.au32ComponentMask[i] = pstVCapAttr->stVcapDevAttr.au32ComponentMask[i];
    }
 ...
 /*    printf("stDevAttr enIntfMode =%d \r\n",stDevAttr.enIntfMode);
     printf("stDevAttr enWorkMode =%d \r\n",stDevAttr.enWorkMode);

     printf("stDevAttr VI_COMPMASK_NUM =%ld\r\n",VI_COMPMASK_NUM);*/
 /*   for (i = 0; i < VI_COMPMASK_NUM; i++) {
     printf("stDevAttr au32ComponentMask[%d] =%d\r\n",i,stDevAttr.au32ComponentMask[i]);
    }*/
/*     printf("stDevAttr enDataSeq =%d \r\n",stDevAttr.enDataSeq);
    printf("stDevAttr stSynCfg enVsync =%d \r\n",stDevAttr.stSynCfg.enVsync);
    printf("stDevAttr stSynCfg enVsyncNeg =%d \r\n",stDevAttr.stSynCfg.enVsyncNeg);
    printf("stDevAttr stSynCfg enHsync =%d \r\n",stDevAttr.stSynCfg.enHsync);
    printf("stDevAttr stSynCfg enHsyncNeg =%d \r\n",stDevAttr.stSynCfg.enHsyncNeg);
    printf("stDevAttr stSynCfg enVsyncValid =%d \r\n",stDevAttr.stSynCfg.enVsyncValid);*/
 /*   printf("stDevAttr stSynCfg enVsyncValidNeg =%d \r\n",stDevAttr.stSynCfg.enVsyncValidNeg);
    printf("stDevAttr stSynCfg stTimingBlank u32HsyncHfb=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32HsyncHfb);
    printf("stDevAttr stSynCfg stTimingBlank u32HsyncAct=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32HsyncAct);
    printf("stDevAttr stSynCfg stTimingBlank u32HsyncHbb=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32HsyncHbb);
    printf("stDevAttr stSynCfg stTimingBlank u32VsyncVfb=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32VsyncVfb);
    printf("stDevAttr stSynCfg stTimingBlank u32VsyncVact=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32VsyncVact);
    printf("stDevAttr stSynCfg stTimingBlank u32VsyncVbb=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32VsyncVbb);
    printf("stDevAttr stSynCfg stTimingBlank u32VsyncVbfb=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32VsyncVbfb);
    printf("stDevAttr stSynCfg stTimingBlank u32VsyncVbact=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32VsyncVbact);
    printf("stDevAttr stSynCfg stTimingBlank u32VsyncVbbb=%d \r\n",stDevAttr.stSynCfg.stTimingBlank.u32VsyncVbbb);
     printf("stDevAttr enInputDataType =%d\r\n",stDevAttr.enInputDataType);

     printf("stDevAttr bDataReverse =%d\r\n",stDevAttr.bDataReverse);

     printf("stDevAttr u32Width =%d \r\n",stDevAttr.stSize.u32Width);
     printf("stDevAttr u32Height =%d \r\n",stDevAttr.stSize.u32Height);

     printf("stDevAttr enWDRMode =%d \r\n",stDevAttr.stWDRAttr.enWDRMode);
     printf("stDevAttr u32CacheLine =%d \r\n",stDevAttr.stWDRAttr.u32CacheLine);  // check at SDK

     printf("stDevAttr stBasSize u32Width =%d \r\n",stDevAttr.stBasAttr.stSacleAttr.stBasSize.u32Width);
     printf("stDevAttr stBasSize u32Height =%d \r\n",stDevAttr.stBasAttr.stSacleAttr.stBasSize.u32Height);  // check at SDK
     printf("stDevAttr stBasSize enHRephaseMode =%d \r\n",stDevAttr.stBasAttr.stRephaseAttr.enHRephaseMode);
     printf("stDevAttr stBasSize enVRephaseMode =%d \r\n",stDevAttr.stBasAttr.stRephaseAttr.enVRephaseMode);
*/
    /* set vi dev attr */
    printf("set vi dev attr \r\n");
    ...
    s32Ret = memcpy_s(g_stVcapGlobAttr.astGlobDevAttr[ViDev].aPipeIdBind, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM,
        pstVCapAttr->aPipeIdBind, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobDevAttr[ViDev].stDevAttr, sizeof(HI_MAPI_VCAP_DEV_ATTR_S),
        &pstVCapAttr->stVcapDevAttr, sizeof(HI_MAPI_VCAP_DEV_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
#ifndef __HI3559AV100__
        s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stPipeAttr.stFrameIntAttr,
            sizeof(FRAME_INTERRUPT_ATTR_S), &pstVCapAttr->astVcapPipeAttr[ViPipe].stFrameIntAttr,
            sizeof(FRAME_INTERRUPT_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
#endif
        s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stPipeAttr.stFrameRateCtrl, sizeof(FRAME_RATE_CTRL_S),
               &pstVCapAttr->astVcapPipeAttr[ViPipe].stFrameRate, sizeof(FRAME_RATE_CTRL_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stIspPubAttr, sizeof(HI_MAPI_PIPE_ISP_ATTR_S),
               &pstVCapAttr->astVcapPipeAttr[ViPipe].stIspPubAttr, sizeof(HI_MAPI_PIPE_ISP_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);

        /* 2. save chn user attr and flag */
        for (j = 0; j < HI_MAPI_PIPE_MAX_CHN_NUM; j++) {
            s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[j].stChnAttr,
                sizeof(HI_MAPI_PIPE_CHN_ATTR_S), &pstVCapAttr->astVcapPipeAttr[ViPipe].astPipeChnAttr[j],
...
{
    CHECK_MAPI_VCAP_NULL_PTR_RET(penDstType);
...
static HI_S32 MAPI_VCAP_SetExposure(HI_HANDLE VcapPipeHdl, const HI_MAPI_VCAP_EXPOSURE_ATTR_S *pstExposure)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstExposure);
...
static HI_S32 MAPI_VCAP_GetExposure(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_EXPOSURE_ATTR_S *pstExposure)
{
    ...
    s32Ret = memset_s(&stExposure, sizeof(ISP_EXPOSURE_ATTR_S), 0, sizeof(ISP_EXPOSURE_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
...
static HI_S32 MAPI_VCAP_SetWB(HI_HANDLE VcapPipeHdl, const HI_MAPI_VCAP_WB_ATTR_S *pstWBAttr)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstWBAttr);

    ViPipe = (VI_PIPE)VcapPipeHdl;

    s32Ret = memset_s(&stWbAttr, sizeof(ISP_WB_ATTR_S), 0, sizeof(ISP_WB_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    ...
    if (pstWBAttr->enOpType == HI_MAPI_VCAP_OP_TYPE_MANUAL) {
        ...
        g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stVcapISPWbAttr.stWBMode.u32ColorTemp = 0xFFFF;
    }
                                    ...
                                    HI_BOOL bBNRraw)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstFrameData);
                                ...
                                HI_BOOL bBNRraw)
{
    ...
    s32Ret = memcpy_s(&pstFrameData->stVideoSupplement, sizeof(VIDEO_SUPPLEMENT_S), &stVideoFrame.stVFrame.stSupplement,
                                ...
                                HI_BOOL bBNRraw)
{
    ...
    for (i = 0; i < u32DumpPipeCnt; i++) {
        for (j = i + 1; j < u32DumpPipeCnt; j++) {
            if (u32WdrRawVcNum[i] < u32WdrRawVcNum[j]) {
                s32Ret = memcpy_s(&stTempWdrRaw, sizeof(HI_MAPI_FRAME_DATA_S), &astOneGroupWdrRaw[i],
                    sizeof(HI_MAPI_FRAME_DATA_S));
                MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
                s32Ret = memcpy_s(&astOneGroupWdrRaw[i], sizeof(HI_MAPI_FRAME_DATA_S), &astOneGroupWdrRaw[j],
                    sizeof(HI_MAPI_FRAME_DATA_S));
                MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
                s32Ret = memcpy_s(&astOneGroupWdrRaw[j], sizeof(HI_MAPI_FRAME_DATA_S), &stTempWdrRaw,
                    sizeof(HI_MAPI_FRAME_DATA_S));
                MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
            }
        }
    ...
    s32Ret = memcpy_s(pstFrameData, sizeof(HI_MAPI_FRAME_DATA_S) * u32DumpPipeCnt, astOneGroupWdrRaw,
        sizeof(HI_MAPI_FRAME_DATA_S) * u32DumpPipeCnt);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
                                 ...
                                 HI_S32 s32DataNum, HI_BOOL bBNRraw)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstFrameData);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].bPipeCreate,
        "please start vcap device first", HI_MAPI_VCAP_ENOT_PERM);

    if (bBNRraw) {
        CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].bSnapPipeEnable,
    ...
    if (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stDevAttr.stWDRAttr.enWDRMode == WDR_MODE_NONE) {
                             ...
                             HI_S32 *ps32FrameCnt, HI_BOOL bBNRraw)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstFrameData);
    CHECK_MAPI_VCAP_NULL_PTR_RET(ps32FrameCnt);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].bPipeCreate,
        "please start vcap device first", HI_MAPI_VCAP_ENOT_PERM);
...
static HI_S32 MAPI_VCAP_SetDumpRawAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_DUMP_ATTR_S *pstDumpAttr)
{
    ...
    s32Ret = memset_s(&stPipeAttr, sizeof(VI_PIPE_ATTR_S), 0, sizeof(VI_PIPE_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    ...
    if (pstDumpAttr->stDumpAttr.bEnable || pstDumpAttr->stDumpBNRAttr.bEnable) {
        ...
        s32Ret = MAPI_VCAP_AdaptPipePixFmtFromSensorCfg(VcapPipeHdl, &stPipeAttr);
    ...
    s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stDumpRawInfo.stVcapDumpAttr,
        sizeof(HI_MAPI_DUMP_ATTR_S), pstDumpAttr, sizeof(HI_MAPI_DUMP_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_SetLDC(HI_HANDLE VcapPipeHdl, HI_HANDLE PipeChnHdl, const VI_LDC_ATTR_S *pstLdcAttr)
{
    ...
    // para check
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].bChnEnable,
        "chn has not been start", HI_MAPI_VCAP_ENOT_PERM);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32CenterXOffset", pstLdcAttr->stAttr.s32CenterXOffset,
        HI_MAPI_VCAP_MIN_LDC_CENTERX_OFFSET);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32CenterXOffset", pstLdcAttr->stAttr.s32CenterXOffset,
        HI_MAPI_VCAP_MAX_LDC_CENTERX_OFFSET);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32CenterYOffset", pstLdcAttr->stAttr.s32CenterYOffset,
        HI_MAPI_VCAP_MIN_LDC_CENTERY_OFFSET);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32CenterYOffset", pstLdcAttr->stAttr.s32CenterYOffset,
        HI_MAPI_VCAP_MAX_LDC_CENTERY_OFFSET);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32XYRatio", pstLdcAttr->stAttr.s32XYRatio, HI_MAPI_VCAP_MIN_LDC_RATIO_XY);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32XYRatio", pstLdcAttr->stAttr.s32XYRatio, HI_MAPI_VCAP_MAX_LDC_RATIO_XY);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32XRatio", pstLdcAttr->stAttr.s32XRatio, HI_MAPI_VCAP_MIN_LDC_RATIO_X);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32XRatio", pstLdcAttr->stAttr.s32XRatio, HI_MAPI_VCAP_MAX_LDC_RATIO_X);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32YRatio", pstLdcAttr->stAttr.s32YRatio, HI_MAPI_VCAP_MIN_LDC_RATIO_Y);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32YRatio", pstLdcAttr->stAttr.s32YRatio, HI_MAPI_VCAP_MAX_LDC_RATIO_Y);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32YRatio", pstLdcAttr->stAttr.s32DistortionRatio, HI_MAPI_VCAP_MIN_LDC_RATIO_DISTORTION);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32YRatio", pstLdcAttr->stAttr.s32DistortionRatio, HI_MAPI_VCAP_MAX_LDC_RATIO_DISTORTION);
                                    ...
                                    HI_MAPI_VCAP_CMD_E enCMD)
{
    ...
    switch (enCMD) {
        case HI_MAPI_VCAP_CMD_ISP_SetAEDebug:
            s32Ret = strcpy_s(stLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AE_LIB_NAME);
            MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strcpy_s", s32Ret);
        ...
        case HI_MAPI_VCAP_CMD_ISP_SetAWBDebug:
            s32Ret = strcpy_s(stLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AWB_LIB_NAME);
            MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strcpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_SetLuma(HI_HANDLE VcapPipeHdl, const HI_MAPI_VCAP_LUMA_S *pstLuma)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstLuma);
    CHECK_MAPI_VCAP_MAX_VAL_RET("pstLuma->u8Luma", pstLuma->u8Luma, HI_MAPI_VCAP_MAX_LUMA);
...
static HI_S32 MAPI_VCAP_SetSaturation(HI_HANDLE VcapPipeHdl, const HI_MAPI_VCAP_SATURATION_S *pstSaturation)
{
    ...
    ViPipe = (VI_PIPE)VcapPipeHdl;
    // para check
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSaturation);
    CHECK_MAPI_VCAP_MAX_VAL_RET("Saturation value", pstSaturation->u8Saturation, HI_MAPI_VCAP_MAX_SATURATION);
...
static HI_S32 MAPI_VCAP_SetSharpen(HI_HANDLE VcapPipeHdl, const HI_MAPI_VCAP_SHARPEN_S *pstSharpen)
{
    ...
    switch (pstSharpen->enOpType) {
        ...
        case HI_MAPI_VCAP_OP_TYPE_MANUAL:
            ...
            s32Ret = memcpy_s(stIspShpAttr.stManual.au16EdgeStr, sizeof(HI_U16) * ISP_SHARPEN_GAIN_NUM,
                pstSharpen->stSharpenManualAttr.au16EdgeStr, sizeof(HI_U16) * ISP_SHARPEN_GAIN_NUM);
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
            break;
...
static HI_S32 MAPI_VCAP_GetSharpen(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_SHARPEN_S *pstSharpen)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSharpen);
    ...
    if (!stIspShpAttr.bEnable) {
        ...
        s32Ret = memcpy_s(pstSharpen->stSharpenManualAttr.au16EdgeStr, sizeof(HI_U16) * ISP_SHARPEN_GAIN_NUM,
            stIspShpAttr.stManual.au16EdgeStr, sizeof(HI_U16) * ISP_SHARPEN_GAIN_NUM);
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        return HI_SUCCESS;
    }
                                        ...
                                        const HI_MAPI_VCAP_METERINGMODE_S *pstMeteringMode)
{
    ...
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstMeteringMode);
    ...
    s32Ret = memcpy_s(stStatCfg.stAECfg.au8Weight, sizeof(HI_U8) * AE_ZONE_ROW * AE_ZONE_COLUMN,
        pstMeteringMode->au8Weight, sizeof(HI_U8) * AE_ZONE_ROW * AE_ZONE_COLUMN);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
                                        ...
                                        HI_MAPI_VCAP_METERINGMODE_S *pstMeteringMode)
{
    ...
    s32Ret = memcpy_s(pstMeteringMode->au8Weight, sizeof(HI_U8) * AE_ZONE_ROW * AE_ZONE_COLUMN,
        stStatCfg.stAECfg.au8Weight, sizeof(HI_U8) * AE_ZONE_ROW * AE_ZONE_COLUMN);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_InitISP(HI_HANDLE VcapPipeHdl)
{
    ...
    ViPipe = (VI_PIPE)VcapPipeHdl;
    s32Ret = memset_s(&stIspCtrlParam, sizeof(ISP_CTRL_PARAM_S), 0x00, sizeof(ISP_CTRL_PARAM_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
...
static HI_S32 MAPI_VCAP_EnterStandBy(HI_HANDLE VcapPipeHdl)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "vcap has not been inited", HI_MAPI_VCAP_ENOT_INITED);
...
#ifndef __HI3559AV100__
/* VI_LDCV3_ATTR_S is not defined in hi3559av100, fix in the next version */
static HI_S32 MAPI_VCAP_SetLDCv3(HI_HANDLE VcapPipeHdl, HI_HANDLE PipeChnHdl, const VI_LDCV3_ATTR_S *pstLDCV3Attr)
{
    HI_S32 s32Ret;
    VI_PIPE ViPipe;
    VI_CHN ViChn;
    ...
    HI_U32 u32Width = g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].stChnAttr.stDestSize.u32Width;
    HI_U32 u32Height = g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].stChnAttr.stDestSize.u32Height;

    CHECK_MAPI_VCAP_MIN_VAL_RET("u32Width", u32Width, 640);
    CHECK_MAPI_VCAP_MAX_VAL_RET("u32Width", u32Width, 3072);
    CHECK_MAPI_VCAP_MIN_VAL_RET("u32Height", u32Height, 480);
    CHECK_MAPI_VCAP_MAX_VAL_RET("u32Height", u32Height, 1944);
    CHECK_MAPI_VCAP_MIN_VAL_RET("u32Width * u32Height", (u32Width * u32Height), (640 * 480));
    CHECK_MAPI_VCAP_MAX_VAL_RET("u32Width * u32Height", (u32Width * u32Height), (3072 * 1728));
    CHECK_MAPI_VCAP_MIN_VAL_RET("enViewType", pstLDCV3Attr->stAttr.enViewType, LDC_VIEW_TYPE_ALL);
    CHECK_MAPI_VCAP_MAX_VAL_RET("enViewType", pstLDCV3Attr->stAttr.enViewType, LDC_VIEW_TYPE_CROP);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32CenterXOffset", pstLDCV3Attr->stAttr.s32CenterXOffset,
                               (HI_S32)(- 1 * MIN(u32Width * 8 / 100, 128)));
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32CenterXOffset", pstLDCV3Attr->stAttr.s32CenterXOffset, MIN(u32Width * 8 / 100,
                                128));
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32CenterYOffset", pstLDCV3Attr->stAttr.s32CenterYOffset,
                               (HI_S32)(- 1 * MIN(u32Height * 8 / 100, 128)));
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32CenterYOffset", pstLDCV3Attr->stAttr.s32CenterYOffset, MIN(u32Height * 8 / 100,
                                128));
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32DistortionRatio", pstLDCV3Attr->stAttr.s32DistortionRatio, -300);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32DistortionRatio", pstLDCV3Attr->stAttr.s32DistortionRatio, 300);
    CHECK_MAPI_VCAP_MIN_VAL_RET("s32MinRatio", pstLDCV3Attr->stAttr.s32MinRatio, -300);
    CHECK_MAPI_VCAP_MAX_VAL_RET("s32MinRatio", pstLDCV3Attr->stAttr.s32MinRatio, 300);

    s32Ret = MAPI_VCAP_CheckViVpssWorkMode(VcapPipeHdl);
    CHECK_MAPI_VCAP_RET(s32Ret, "don't support LDC when the vpss work mode is online or parallel,VcapPipeHdl[%d]\n",
                        VcapPipeHdl);

    s32Ret = HI_MPI_VI_SetChnLDCV3Attr(ViPipe, ViChn, pstLDCV3Attr);
    CHECK_MAPI_VCAP_RET(s32Ret, "call HI_MPI_VI_SetChnLDCV3Attr failed,VcapPipeHdl[%d],PipeChn[%d]\n", VcapPipeHdl,
                        PipeChnHdl);
...
MAPI_GLOB_DEV_ATTR_S *MAPI_VCAP_GetDevAttr(HI_HANDLE VcapDevHdl)
{
                                     ...
                                     const HI_MAPI_VCAP_DIS_ATTR_S *pstDisAttr)
{
    ...
    /* 1.check param */
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_HANDLE_RET("PipeChnHdl", PipeChnHdl, (HI_MAPI_PIPE_MAX_CHN_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstDisAttr);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    ...
    if (pstDisAttr->stDISConfig.enMode == DIS_MODE_GYRO) {
        ...
        s32Ret = memset_s(&stLDCV2Attr, sizeof(VI_LDCV2_ATTR_S), 0, sizeof(VI_LDCV2_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
        s32Ret = memcpy_s(&stLDCV2Attr, sizeof(VI_LDCV2_ATTR_S), &pstDisAttr->stLDCV2Attr, sizeof(VI_LDCV2_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    s32Ret = memset_s(&stDISConfig, sizeof(DIS_CONFIG_S), 0, sizeof(DIS_CONFIG_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    s32Ret = memcpy_s(&stDISConfig, sizeof(DIS_CONFIG_S), &(pstDisAttr->stDISConfig), sizeof(DIS_CONFIG_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    s32Ret = memset_s(&stDISAttr, sizeof(DIS_ATTR_S), 0, sizeof(DIS_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    s32Ret = memcpy_s(&stDISAttr, sizeof(DIS_ATTR_S), &pstDisAttr->stDisAttr, sizeof(DIS_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
static HI_S32 MAPI_VCAP_SetFrameIntAttr(HI_HANDLE VcapPipeHdl, const FRAME_INTERRUPT_ATTR_S *pstFrameIntAttr)
{
    ...
    if (g_stVcapGlobAttr.astGlobDevAttr[ViDev].stDevAttr.stWDRAttr.enWDRMode != WDR_MODE_NONE
    ...
    if (stVIVPSSMode.aenMode[ViPipe] == VI_OFFLINE_VPSS_OFFLINE
        ...
        || stVIVPSSMode.aenMode[ViPipe] == VI_PARALLEL_VPSS_PARALLEL) {
        printf("pstFrameIntAttr->u32EarlyLine %d\r\n",pstFrameIntAttr->u32EarlyLine);
        s32Ret = HI_MPI_VI_SetPipeFrameInterruptAttr(ViPipe, pstFrameIntAttr);
        CHECK_MAPI_VCAP_RET(s32Ret, "call HI_MPI_VI_SetPipeFrameInterruptAttr fail,VcapPipeHdl[%d],aenMode[%d]\n", VcapPipeHdl,stVIVPSSMode.aenMode[ViPipe]);
    }
...
HI_S32 HI_MAPI_VCAP_InitSensor(HI_HANDLE VcapDevHdl, HI_MAPI_SENSOR_ATTR_S *pstSensorAttr, HI_BOOL bResetSensor)
{
    HI_S32 s32Ret;
    ...
    /* 4.Access to the mipi property settings of the sensor interface configuration file, get the current sequence,
           check the sensor attr passed over is correct. */
    ...
    g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].bSensorReset = bResetSensor;
    if (g_as32SensorType[VcapDevHdl] == AHD) {
        /* the ahd driver has done reseting sensor clock */
        bResetSensor = HI_FALSE;
    } else if (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].enSensorScene != HI_MAPI_SENSOR_SCENE_NORMAL) {
        /* no need to reset sensor no matter with input in standby flow */
        bResetSensor = HI_FALSE;
    }

    /* 5.Set mipi interface attrs */
    s32Ret = MAPI_VCAP_StartMipi(VcapDevHdl, &stComboDevAttr, pstSensorAttr, bResetSensor);
    CHECK_MAPI_VCAP_RET(s32Ret, "start mipi and sensor fail,sensor[%d]\n", VcapDevHdl);

    /* 6.set Peripheral attr */
    s32Ret = MAPI_PERIPHERALMNG_SetAttr(VcapDevHdl, g_as32SensorType[VcapDevHdl], pstSensorAttr);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "MAPI_PERIPHERALMNG_SetAttr failed\n");
        return HI_MAPI_VCAP_EOPERATE_FAIL;
    }
    ...
    printf("pstSensorAttr  enWdrMode = %d  u32SnsMode = %d  u32Width = %d  u32Height = %d  \r\n",pstSensorAttr->enWdrMode,pstSensorAttr->u32SnsMode,pstSensorAttr->stSize.u32Width,pstSensorAttr->stSize.u32Height);
    ...
    /* get inputDevAttr from sensor cfg temporary*/
     printf("stComboDevAttr  input_mode = %d\r\n", stComboDevAttr.input_mode);
    
    g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.inputMode = stComboDevAttr.input_mode;
    g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.bInputSerdes = pstSensorAttr->stInputDevAttr.bInputSerdes;
    if ((stComboDevAttr.input_mode == INPUT_MODE_MIPI) && (g_as32SensorType[VcapDevHdl] != AHD)) {

     printf("stComboDevAttr  input_data_type = %d\r\n", stComboDevAttr.mipi_attr.input_data_type);
        g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.inputDataType =
            stComboDevAttr.mipi_attr.input_data_type;
    } else {
        g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.inputDataType =
            pstSensorAttr->stInputDevAttr.inputDataType;
...
HI_S32 HI_MAPI_VCAP_DeinitSensor(HI_HANDLE VcapDevHdl)
{
    ...
    if (g_as32SensorType[VcapDevHdl] == AHD) {
        ...
        /* reset sensor depend on input in standby flow */
        bResetSensor = g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].bSensorReset;
    }

    /* 2. unreset mipi */
    s32Ret = MAPI_VCAP_StopMipi(VcapDevHdl, bResetSensor);
    CHECK_MAPI_VCAP_RET(s32Ret, "unreset mipi fail,Sensor[%u]\n", VcapDevHdl);

    s32Ret = memset_s(&g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr, sizeof(HI_MAPI_SENSOR_ATTR_S), 0,
...
HI_S32 HI_MAPI_VCAP_SetSensorFrameRate(HI_HANDLE VcapDevHdl, HI_FLOAT *pf32Framerate)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pf32Framerate);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].bSensorInit,
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("the id of isp which control sensor", ViPipe, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));

    s32Ret = memset_s(&stIspPubAttr, sizeof(ISP_PUB_ATTR_S), 0, sizeof(ISP_PUB_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    s32Ret = HI_MPI_ISP_GetPubAttr(ViPipe, &stIspPubAttr);
...
HI_S32 HI_MAPI_VCAP_GetSensorAttr(HI_HANDLE VcapDevHdl, HI_MAPI_SENSOR_ATTR_S *pstSensorAttr)
{
    ...
    pstSensorAttr->u32SnsMode = g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.u32SnsMode;
    ...
    pstSensorAttr->stInputDevAttr.inputMode = g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.inputMode;
    pstSensorAttr->stInputDevAttr.inputDataType = g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stSensorAttr.stInputDevAttr.inputDataType;
    return HI_SUCCESS;
}
...
HI_S32 HI_MAPI_VCAP_GetSensorModesCnt(HI_HANDLE VcapDevHdl, HI_S32 *ps32ModesCnt)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(ps32ModesCnt);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
                                      ...
                                      HI_MAPI_SENSOR_MODE_S *pstSensorModes)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSensorModes);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    ...
    for (i = 0; i < s32ModesCnt; i++) {
        ...
        pstSensorModes[i].u32SnsMode = pstAllSensorModes[i].u32SnsMode;
...
HI_S32 HI_MAPI_VCAP_SetAttr(HI_HANDLE VcapDevHdl, HI_MAPI_VCAP_ATTR_S *pstVCapAttr)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstVCapAttr);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].bSensorInit,
        "sensor has not been inited ,Operation not allowed", HI_MAPI_VCAP_ENOT_PERM);
    ...
    if (g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].enDevStatus == VCAP_STATUS_RUNNING) {
        /* check DEV */
        CHECK_MAPI_VCAP_TRUE_RET(!MAPI_VCAP_IsDevAttrChange(VcapDevHdl, &pstVCapAttr->stVcapDevAttr),
            "don't support modify vcap dev Attributes in the running status", HI_MAPI_VCAP_ENOT_PERM);
        /* check bind */
        CHECK_MAPI_VCAP_TRUE_RET(!MAPI_VCAP_IsDevPipeBindChange(VcapDevHdl, pstVCapAttr),
            "don't support modify vcap dev and pipebind in the running status", HI_MAPI_VCAP_ENOT_PERM);
...
HI_S32 HI_MAPI_VCAP_GetAttr(HI_HANDLE VcapDevHdl, HI_MAPI_VCAP_ATTR_S *pstVCapAttr)
{
    ...
    s32Ret = memcpy_s(pstVCapAttr->aPipeIdBind, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM,
        g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].aPipeIdBind, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    s32Ret = memcpy_s(&pstVCapAttr->stVcapDevAttr, sizeof(HI_MAPI_VCAP_DEV_ATTR_S),
        &g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].stDevAttr, sizeof(HI_MAPI_VCAP_DEV_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    for (i = 0; i < g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].u32PipeBindNum; i++) {
        ...
        s32Ret = memcpy_s(&pstVCapAttr->astVcapPipeAttr[VcapPipeId].stFrameRate, sizeof(FRAME_RATE_CTRL_S),
               &g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeId].stPipeAttr.stFrameRateCtrl, sizeof(FRAME_RATE_CTRL_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        s32Ret = memcpy_s(&pstVCapAttr->astVcapPipeAttr[VcapPipeId].stIspPubAttr, sizeof(HI_MAPI_PIPE_ISP_ATTR_S),
               &g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeId].stIspPubAttr, sizeof(HI_MAPI_PIPE_ISP_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);

        for (j = 0; j < HI_MAPI_PIPE_MAX_CHN_NUM; j++) {
            PipeChnId = PIPE_CHN_0 + j;
            s32Ret = memcpy_s(&pstVCapAttr->astVcapPipeAttr[VcapPipeId].astPipeChnAttr[PipeChnId],
                sizeof(HI_MAPI_PIPE_CHN_ATTR_S),
...
HI_S32 HI_MAPI_VCAP_StartDev(HI_HANDLE VcapDevHdl)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapDevHdl", VcapDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobDevAttr[ViDev].bSensorInit,
        "sensor has not been inited ,Operation not allowed", HI_MAPI_VCAP_ENOT_PERM);
    ...
    */
    ...
    s32Ret = memset_s(&stDevBindPipe, sizeof(VI_DEV_BIND_PIPE_S), 0, sizeof(VI_DEV_BIND_PIPE_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    ...
    for (i = 0; i < stDevBindPipe.u32Num; i++) {
        ...
        s32Ret = memcpy_s(&stPipeAttr, sizeof(VI_PIPE_ATTR_S), &stVcapCfg.astPipeAttr[ViPipe], sizeof(VI_PIPE_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        ...
        s32Ret = MAPI_VCAP_AdaptPipePixFmtFromSensorCfg(ViPipe, &stPipeAttr);
        if (s32Ret != HI_SUCCESS) {
        ...
        if(g_as32SensorType[ViDev] == AHD) {
            s32Ret = HI_MPI_VI_SetPipeVCNumber(ViPipe, i);
            if (s32Ret != HI_SUCCESS) {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "HI_MPI_VI_SetPipeVCNumber exec fail,pipe:%d vc:%d ,ret:%#x!\n",
                     ViPipe, i, s32Ret);
                goto VCAP_CREATE_PIPE_ERR;
            }
            g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].BindVcHdl = i;
        }
    }

    g_stVcapGlobAttr.astGlobDevAttr[ViDev].enDevStatus = VCAP_STATUS_RUNNING;

    s32Ret = MAPI_PERIPHERALMNG_Start(VcapDevHdl, g_as32SensorType[VcapDevHdl]);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "MAPI_PERIPHERALMNG_Start failed\n");
        goto VCAP_CREATE_PIPE_ERR;
    }
...
<2>Closing is done by closing all CHNs on the pipe-> closing all pipes bound to dev (VcapDevHdl) -> deactivating the specified
(VcapDevHdl)dev */
HI_S32 HI_MAPI_VCAP_StopDev(HI_HANDLE VcapDevHdl)
{
    ...
    for (i = 0; i < g_stVcapGlobAttr.astGlobDevAttr[VcapDevHdl].u32PipeBindNum; i++) {
        ...
        for (j = 0; j < HI_MAPI_PIPE_MAX_CHN_NUM; j++) {
            ...
            s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].stChnAttr,
                sizeof(HI_MAPI_PIPE_CHN_ATTR_S), 0, sizeof(HI_MAPI_PIPE_CHN_ATTR_S));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
#ifdef SUPPORT_DIS
            g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].stVcapDisAttr.enDisStatus = DIS_STATUS_STOP;
        ...
        g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].BindDevHdl = HI_INVALID_HANDLE;
        g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].BindVcHdl = HI_INVALID_HANDLE;
        g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bPipeCreate = HI_FALSE;
        s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stPipeAttr, sizeof(PIPE_ATTR_S), 0,
            sizeof(PIPE_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
        s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stSnapAttr, sizeof(HI_MAPI_VCAP_SNAP_ATTR_S), 0,
            sizeof(HI_MAPI_VCAP_SNAP_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    }
    ...
    s32Ret = memset_s(g_stVcapGlobAttr.astGlobDevAttr[ViDev].aPipeIdBind, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM,
        0, sizeof(HI_HANDLE) * HI_MAPI_VCAP_MAX_PIPE_NUM);
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    s32Ret = memset_s(&g_stVcapGlobAttr.astGlobDevAttr[ViDev].stDevAttr, sizeof(HI_MAPI_VCAP_DEV_ATTR_S), 0,
        sizeof(HI_MAPI_VCAP_DEV_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);

    s32Ret = MAPI_PERIPHERALMNG_Stop(VcapDevHdl, g_as32SensorType[VcapDevHdl]);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "MAPI_PERIPHERALMNG_Stop failed\n");
        return HI_MAPI_VCAP_EOPERATE_FAIL;
    }
...
HI_S32 HI_MAPI_VCAP_SetPipeVCNumber(HI_HANDLE VcapPipeHdl, HI_U32 u32VCNumber)
{
    HI_S32 s32Ret = HI_SUCCESS;
    VI_PIPE ViPipe;
    VI_DEV ViDev;
    ...
    if ((ViDev > (VI_DEV)(HI_MAPI_VCAP_MAX_DEV_NUM - 1))
        || (g_stVcapGlobAttr.astGlobDevAttr[ViDev].enDevStatus != VCAP_STATUS_RUNNING)) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "dot not permit,you must set Vcap Attrs and start dev[%d] first\n", ViDev);
        return HI_MAPI_VCAP_ENOT_PERM;
    }

    s32Ret = HI_MPI_VI_SetPipeVCNumber(ViPipe, u32VCNumber);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "HI_MPI_VI_SetPipeVCNumber exec fail,ret:%#x!\n", s32Ret);
        return s32Ret;
    }

    g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].BindVcHdl = u32VCNumber;
    return HI_SUCCESS;
}

HI_S32 HI_MAPI_VCAP_GetPipeVCNumber(HI_HANDLE VcapPipeHdl, HI_U32 *pu32VCNumber)
{
    HI_S32 s32Ret = HI_SUCCESS;
    VI_PIPE ViPipe;
    VI_DEV ViDev;
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pu32VCNumber);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    ...
    s32Ret = HI_MPI_VI_GetPipeVCNumber(ViPipe, pu32VCNumber);
    if (s32Ret != HI_SUCCESS) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "HI_MPI_VI_SetPipeVCNumber exec fail,ret:%#x!\n", s32Ret);
        return s32Ret;
    }
...
HI_S32 HI_MAPI_VCAP_StartPipe(HI_HANDLE VcapPipeHdl)
{
    printf("HI_MAPI_VCAP_StartPipe\r\n");
    HI_S32 s32Ret = HI_SUCCESS;
    VI_PIPE ViPipe;
    VI_DEV ViDev;
    ...
    if (g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bPipeStart == HI_FALSE) {
        if ((g_stVcapGlobAttr.astGlobDevAttr[ViDev].u32PipeBindNum == 1)
            ...
            s32Ret = MAPI_VCAP_SetFrameIntAttr(VcapPipeHdl,&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stPipeAttr.stFrameIntAttr);
            CHECK_MAPI_VCAP_RET(s32Ret, "Set Frame Int Attr fail,Pipe[%d]\n", VcapPipeHdl);

            s32Ret = HI_MPI_VI_StartPipe(ViPipe);
            CHECK_MAPI_VCAP_RET(s32Ret, "call HI_MPI_VI_StartPipe fail,VcapDevHdl[%d],VcapPipeHdl[%d]\n",ViDev, VcapPipeHdl);
            g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bPipeStart = HI_TRUE;

            s32Ret = MAPI_PERIPHERALMNG_StartChn(ViDev, g_as32SensorType[ViDev], (HI_U32)g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].BindVcHdl);
            if (s32Ret != HI_SUCCESS) {
                MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "MAPI_PERIPHERALMNG_StartChn failed\n");
                return HI_MAPI_VCAP_EOPERATE_FAIL;
            }
        }
    }
...
HI_S32 HI_MAPI_VCAP_StopPipe(HI_HANDLE VcapPipeHdl)
{
    HI_S32 s32Ret;
    VI_PIPE ViPipe;
    VI_DEV ViDev;
    ...
    if ((g_stVcapGlobAttr.astGlobDevAttr[ViDev].u32PipeBindNum == 1)
        ...
        s32Ret = MAPI_PERIPHERALMNG_StopChn(ViDev, g_as32SensorType[ViDev], (HI_U32)g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].BindVcHdl);
        if (s32Ret != HI_SUCCESS) {
            MAPI_ERR_TRACE(HI_MAPI_MOD_VCAP, "MAPI_PERIPHERALMNG_StopChn failed\n");
            return HI_MAPI_VCAP_EOPERATE_FAIL;
        }
    }
#endif
...
HI_S32 HI_MAPI_VCAP_StartChn(HI_HANDLE VcapPipeHdl, HI_HANDLE PipeChnHdl)
{
    ...
    MAPI_VCAP_LoadParam(&stVcapCfg, g_as32SensorType);
    s32Ret = memcpy_s(&stChnAttr, sizeof(VI_CHN_ATTR_S), &stVcapCfg.astChnAttr[ViChn], sizeof(VI_CHN_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_InitISP(HI_HANDLE VcapPipeHdl)
{
    VI_PIPE ViPipe;
    VI_DEV BindDevHdl;
    HI_S32 s32Ret;
    ...
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].bSensorInit,
        "sensor has not been inited ,Operation not allowed", HI_MAPI_VCAP_ENOT_PERM);

    /* 2.In WDR mode, only the isp of the master PIPE need to be init */
    if (g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].stDevAttr.stWDRAttr.enWDRMode != WDR_MODE_NONE
        && (HI_HANDLE) ViPipe != g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].aPipeIdBind[0]) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "[warning]:In WDR mode, only the isp of the master PIPE need to be init\n");
        return HI_SUCCESS;
    }

    if (g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bIspInit == HI_TRUE) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "[warning]:isp %d is already been inited\n", ViPipe);
        return HI_SUCCESS;
    }
    ...
    if (pstSnsObj->pfnSetInit && g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stVcapISPInitAttr.bISPInitAttr != HI_NULL) {
        ISP_INIT_ATTR_S stISPInitAttr;
        s32Ret = memcpy_s(&stISPInitAttr, sizeof(ISP_INIT_ATTR_S),
            &g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stVcapISPInitAttr.stISPInitAttr, sizeof(ISP_INIT_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bIspInit = HI_TRUE;

    return HI_SUCCESS;
}

HI_S32 HI_MAPI_VCAP_DeInitISP(HI_HANDLE VcapPipeHdl)
{
    VI_PIPE ViPipe;
    VI_DEV BindDevHdl;
    ALG_LIB_S stAeLib;
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_HANDLE_RET("PipeBindDev", BindDevHdl, (HI_MAPI_VCAP_MAX_DEV_NUM - 1));

    if (g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bIspRun == HI_TRUE) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "isp is still in start, stop it firstly.\n");
        return HI_MAPI_VCAP_ENOT_PERM;
    }

    if (g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bIspInit == HI_FALSE) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "isp is already deinit.\n");
        return HI_SUCCESS;
    }
    ...
    stAwbLib.s32Id = ViPipe;
    s32Ret = strncpy_s(stAeLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AE_LIB_NAME, ALG_LIB_NAME_SIZE_MAX);
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strncpy_s", s32Ret);
    s32Ret = strncpy_s(stAwbLib.acLibName, ALG_LIB_NAME_SIZE_MAX, HI_AWB_LIB_NAME, ALG_LIB_NAME_SIZE_MAX);
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strncpy_s", s32Ret);
    ...
    g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bIspInit = HI_FALSE;
...
HI_S32 HI_MAPI_VCAP_StartISP(HI_HANDLE VcapPipeHdl)
{
    ...
    /* 2.In WDR mode, only the isp of the master PIPE need to be start */
    if (g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].stDevAttr.stWDRAttr.enWDRMode != WDR_MODE_NONE
        && (HI_HANDLE) ViPipe != g_stVcapGlobAttr.astGlobDevAttr[BindDevHdl].aPipeIdBind[0]) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "[warning]:In WDR mode, only the isp of the master PIPE need to be start\n");
        return HI_SUCCESS;
    }

    if (g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bIspInit == HI_FALSE) {
        MAPI_WARN_TRACE(HI_MAPI_MOD_VCAP, "[warning]:isp %d need to be inited firstly\n", ViPipe);
        return HI_MAPI_VCAP_ENOT_PERM;
    }
...
HI_S32 HI_MAPI_VCAP_SetSnapAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_SNAP_ATTR_S *pstSnapAttr)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstSnapAttr);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].bPipeCreate,
        "please start vcap device first", HI_MAPI_VCAP_ENOT_PERM);
    ...
    if (pstSnapAttr->enSnapType == SNAP_TYPE_NORMAL) {
        ...
        if (pstSnapAttr->stProAttr.stProParam.enOperationMode == OPERATION_MODE_AUTO) {
            s32Ret = memcpy_s(&stSnapAttr.stProAttr.stProParam.stAutoParam, sizeof(SNAP_PRO_AUTO_PARAM_S),
                &pstSnapAttr->stProAttr.stProParam.stAutoParam, sizeof(SNAP_PRO_AUTO_PARAM_S));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        } else if (pstSnapAttr->stProAttr.stProParam.enOperationMode == OPERATION_MODE_MANUAL) {
            s32Ret = memcpy_s(&stSnapAttr.stProAttr.stProParam.stManualParam, sizeof(SNAP_PRO_MANUAL_PARAM_S),
                &pstSnapAttr->stProAttr.stProParam.stManualParam, sizeof(SNAP_PRO_MANUAL_PARAM_S));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].stSnapAttr, sizeof(HI_MAPI_VCAP_SNAP_ATTR_S),
        pstSnapAttr, sizeof(HI_MAPI_VCAP_SNAP_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_GetSnapAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_VCAP_SNAP_ATTR_S *pstSnapAttr)
{
    ...
    s32Ret = memcpy_s(pstSnapAttr, sizeof(HI_MAPI_VCAP_SNAP_ATTR_S),
        &g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].stSnapAttr, sizeof(HI_MAPI_VCAP_SNAP_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_StopTrigger(HI_HANDLE VcapPipeHdl)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].bPipeCreate, "please start vcap device first",
...
HI_S32 HI_MAPI_VCAP_SetStitchAttr(HI_HANDLE VcapStitchHdl, VI_STITCH_GRP_ATTR_S *pstStitchAttr)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapStitchHdl", VcapStitchHdl, (HI_MAPI_VCAP_STITCH_MAX_GROUP_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstStitchAttr);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
...
HI_S32 HI_MAPI_VCAP_GetStitchAttr(HI_HANDLE VcapStitchHdl, VI_STITCH_GRP_ATTR_S *pstStitchAttr)
{
    ...
    s32Ret = memcpy_s(pstStitchAttr, sizeof(VI_STITCH_GRP_ATTR_S), &stVistitchAttr, sizeof(VI_STITCH_GRP_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_StitchTrigger(HI_HANDLE VcapStitchHdl)
{
    ...
    for (i = 0; i < stVistitchAttr.u32PipeNum; i++) {
        ...
        CHECK_MAPI_VCAP_HANDLE_RET("PIPE ID of the stitch binding group", ViPipe, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
...
HI_S32 HI_MAPI_VCAP_SetExifInfo(HI_HANDLE VcapPipeHdl, HI_MAPI_SNAP_EXIF_INFO_S *pstExifInfo)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstExifInfo);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    ...
    s32Ret = strncpy_s((HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8ImageDescription, DCF_DRSCRIPTION_LENGTH,
    ...
    s32Ret = strncpy_s((HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Make, DCF_DRSCRIPTION_LENGTH,
        (HI_CHAR *)pstExifInfo->au8Make, strlen((HI_CHAR *)pstExifInfo->au8Make));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strncpy_s", s32Ret);
    stIspDCFInfo.stIspDCFConstInfo.au8Make[strlen((HI_CHAR *)pstExifInfo->au8Make)] = '\0';

    s32Ret = strncpy_s((HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Model, DCF_DRSCRIPTION_LENGTH,
        (HI_CHAR *)pstExifInfo->au8Model, strlen((HI_CHAR *)pstExifInfo->au8Model));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strncpy_s", s32Ret);
    stIspDCFInfo.stIspDCFConstInfo.au8Model[strlen((HI_CHAR *)pstExifInfo->au8Model)] = '\0';

    s32Ret = strncpy_s((HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Software, DCF_DRSCRIPTION_LENGTH,
        (HI_CHAR *)pstExifInfo->au8Software, strlen((HI_CHAR *)pstExifInfo->au8Software));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strncpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_GetExifInfo(HI_HANDLE VcapPipeHdl, HI_MAPI_SNAP_EXIF_INFO_S *pstExifInfo)
{
    ...
    s32Ret = strncpy_s((HI_CHAR *)pstExifInfo->au8ImageDescription, DCF_DRSCRIPTION_LENGTH,
    ...
    s32Ret = strncpy_s((HI_CHAR *)pstExifInfo->au8Make, DCF_DRSCRIPTION_LENGTH,
        (HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Make, strlen((HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Make));
    MAPI_CHECK_RET(HI_MAPI_MOD_VENC, "strncpy_s", s32Ret);
    pstExifInfo->au8Make[strlen((HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Make)] = '\0';

    s32Ret = strncpy_s((HI_CHAR *)pstExifInfo->au8Model, DCF_DRSCRIPTION_LENGTH,
        (HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Model,
    ...
    s32Ret = strncpy_s((HI_CHAR *)pstExifInfo->au8Software, DCF_DRSCRIPTION_LENGTH,
        (HI_CHAR *)stIspDCFInfo.stIspDCFConstInfo.au8Software,
                              ...
                              HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_MAX_VAL_RET("enCMD", enCMD, (HI_MAPI_VCAP_CMD_BUTT - 1));
    CHECK_MAPI_VCAP_HANDLE_RET("PipeChnHdl", PipeChnHdl, (HI_MAPI_PIPE_MAX_CHN_NUM - 1));
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    ...
    switch (enCMD) {
        ...
        case HI_MAPI_VCAP_CMD_ISP_Set3AInit: {
            s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].stVcapISPInitAttr.stISPInitAttr,
                sizeof(ISP_INIT_ATTR_S), (ISP_INIT_ATTR_S *)pAttr, sizeof(ISP_INIT_ATTR_S));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
        ...
        case HI_MAPI_VCAP_CMD_LDCV2: {
            ...
            CHECK_MAPI_VCAP_RET(s32Ret, "call HI_MAPI_VCAP_CMD_LDCV2 fail,VcapPipeHdl[%d],PipeChnHdl[%d]\n", VcapPipeHdl,
                                PipeChnHdl);
            break;
        }

        case HI_MAPI_VCAP_CMD_LDCV3: {
#ifndef __HI3559AV100__
            s32Ret = MAPI_VCAP_SetLDCv3(ViPipe, ViChn, (VI_LDCV3_ATTR_S *)pAttr);
            CHECK_MAPI_VCAP_RET(s32Ret, "call MAPI_VCAP_SetLDCv3 fail,VcapPipeHdl[%d],PipeChnHdl[%d]\n", VcapPipeHdl,
                                PipeChnHdl);
#endif
            break;
        }
                              ...
                              HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    switch (enCMD) {
        ...
        case HI_MAPI_VCAP_CMD_ISP_GetAwbInfo: {
            ...
            s32Ret = memcpy_s(pstWBInfoTemp, u32Len, &stWBInfo, sizeof(ISP_WB_INFO_S));
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
            s32Ret = memcpy_s(pstWBInfoTemp->au16CCM, sizeof(HI_U16) * CCM_MATRIX_SIZE, stWBInfo.au16CCM,
                sizeof(HI_U16) * CCM_MATRIX_SIZE);
            MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_GetDumpRawAttr(HI_HANDLE VcapPipeHdl, HI_MAPI_DUMP_ATTR_S *pstDumpAttr)
{
    ...
    s32Ret = memcpy_s(pstDumpAttr, sizeof(HI_MAPI_DUMP_ATTR_S),
        &g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].stDumpRawInfo.stVcapDumpAttr, sizeof(HI_MAPI_DUMP_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
                                  ...
                                  HI_MAPI_OSD_ATTR_S *pstOSDAttr)
{
    ...
    CHECK_MAPI_VCAP_HANDLE_RET("VcapPipeHdl", VcapPipeHdl, (HI_MAPI_VCAP_MAX_PIPE_NUM - 1));
    CHECK_MAPI_VCAP_HANDLE_RET("PipeChnHdl", PipeChnHdl, (HI_MAPI_PIPE_MAX_CHN_NUM - 1));
    CHECK_MAPI_VCAP_HANDLE_RET("OSDHdl", OSDHdl, (HI_MAPI_VCAP_OSD_MAX_NUM - 1));
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstOSDAttr);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.bVcapInited, "media has not been inited", HI_MAPI_VCAP_ENOT_INITED);
    CHECK_MAPI_VCAP_MIN_VAL_RET("stBitmapAttr.enPixelFormat", pstOSDAttr->stBitmapAttr.enPixelFormat,
        PIXEL_FORMAT_ARGB_1555);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stBitmapAttr.enPixelFormat", pstOSDAttr->stBitmapAttr.enPixelFormat,
        PIXEL_FORMAT_ARGB_8888);
    CHECK_MAPI_VCAP_NULL_PTR_RET(pstOSDAttr->stBitmapAttr.pData);
    CHECK_MAPI_VCAP_MIN_VAL_RET("stBitmapAttr.u32Width", pstOSDAttr->stBitmapAttr.u32Width, HI_MAPI_MIN_OSD_BMP_W);
    CHECK_MAPI_VCAP_MIN_VAL_RET("stBitmapAttr.u32Height", pstOSDAttr->stBitmapAttr.u32Height, HI_MAPI_MIN_OSD_BMP_H);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stBitmapAttr.u32Width", pstOSDAttr->stBitmapAttr.u32Width, HI_MAPI_MAX_OSD_BMP_W);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stBitmapAttr.u32Height", pstOSDAttr->stBitmapAttr.u32Height, HI_MAPI_MAX_OSD_BMP_H);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stOsdDisplayAttr.u32Albgha", pstOSDAttr->stOsdDisplayAttr.u32BgAlpha,
        HI_MAPI_MAX_OSD_ALPHA);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stOsdDisplayAttr.u32Alfgha", pstOSDAttr->stOsdDisplayAttr.u32FgAlpha,
        HI_MAPI_MAX_OSD_ALPHA);
    CHECK_MAPI_VCAP_MIN_VAL_RET("stOsdDisplayAttr.s32RegionX", pstOSDAttr->stOsdDisplayAttr.s32RegionX,
        HI_MAPI_MIN_OSD_REGION_X);
    CHECK_MAPI_VCAP_MIN_VAL_RET("stOsdDisplayAttr.s32RegionY", pstOSDAttr->stOsdDisplayAttr.s32RegionY,
        HI_MAPI_MIN_OSD_REGION_Y);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stOsdDisplayAttr.s32RegionX", pstOSDAttr->stOsdDisplayAttr.s32RegionX,
        HI_MAPI_MAX_OSD_REGION_X);
    CHECK_MAPI_VCAP_MAX_VAL_RET("stOsdDisplayAttr.s32RegionY", pstOSDAttr->stOsdDisplayAttr.s32RegionY,
        HI_MAPI_MAX_OSD_REGION_Y);
    CHECK_MAPI_VCAP_MAX_VAL_RET("u32CanvasNum", pstOSDAttr->u32CanvasNum, HI_MAPI_RGN_BUF_MAX_NUM);
    CHECK_MAPI_VCAP_MIN_VAL_RET("u32CanvasNum", pstOSDAttr->u32CanvasNum, HI_MAPI_RGN_BUF_MIN_NUM);
    CHECK_MEDIA_VCAP_ALIGN_RET(pstOSDAttr->stBitmapAttr.u32Width, HI_MAPI_OSD_BMP_W_ALIGN);
    CHECK_MEDIA_VCAP_ALIGN_RET(pstOSDAttr->stBitmapAttr.u32Height, HI_MAPI_OSD_BMP_H_ALIGN);
    CHECK_MEDIA_VCAP_ALIGN_RET(pstOSDAttr->stOsdDisplayAttr.s32RegionX, HI_MAPI_OSD_REGION_X_ALIGN);
    CHECK_MEDIA_VCAP_ALIGN_RET(pstOSDAttr->stOsdDisplayAttr.s32RegionY, HI_MAPI_OSD_REGION_Y_ALIGN);
    ...
    s32Ret = memcpy_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].astVcapOSDAttr[OSDHdl].stOSDAttr,
...
HI_S32 HI_MAPI_VCAP_StartChnOSD(HI_HANDLE VcapPipeHdl, HI_HANDLE PipeChnHdl, HI_HANDLE OSDHdl)
{
    ...
    s32Ret = memcpy_s(&stBitmap, sizeof(BITMAP_S), &pstOSDAttr->stBitmapAttr, sizeof(BITMAP_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
...
HI_S32 HI_MAPI_VCAP_StopChnOSD(HI_HANDLE VcapPipeHdl, HI_HANDLE PipeChnHdl, HI_HANDLE OSDHdl)
{
    ...
    s32Ret = memset_s(&g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].astVcapOSDAttr[OSDHdl].stOSDAttr,
        sizeof(HI_MAPI_OSD_ATTR_S), 0, sizeof(HI_MAPI_OSD_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memset_s", s32Ret);
    g_stVcapGlobAttr.astGlobPipeAttr[ViPipe].astPipeChnAttr[ViChn].astVcapOSDAttr[OSDHdl].enOSDStatus =
        VCAP_STATUS_NOTINIT;
                                  ...
                                  HI_U32 u32Len)
{
    ...
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].bPipeCreate,
        "please start vcap device first", HI_MAPI_VCAP_ENOT_PERM);
    CHECK_MAPI_VCAP_TRUE_RET(g_stVcapGlobAttr.astGlobPipeAttr[VcapPipeHdl].bIspRun, "please start ISP first",
        HI_MAPI_VCAP_ENOT_PERM);
                                  ...
                                  HI_MAPI_VCAP_DIS_ATTR_S *pstDisAttr)
{
    ...
    s32Ret = memcpy_s(&pstDisAttr->stDISConfig, sizeof(DIS_CONFIG_S), &stDISConfig, sizeof(DIS_CONFIG_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    CHECK_MAPI_VCAP_RET(s32Ret, "call HI_MPI_VI_GetChnDISAttr fail,VcapPipeHdl[%d],PipeChnHdl[%d]\n", VcapPipeHdl,
                        PipeChnHdl);
    s32Ret = memcpy_s(&pstDisAttr->stDisAttr, sizeof(DIS_ATTR_S), &stDISAttr, sizeof(DIS_ATTR_S));
    MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    ...
    if (MAPI_VCAP_GetMotionSensorStatus() == HI_TRUE) {
        ...
        s32Ret = memcpy_s(&pstDisAttr->stLDCV2Attr, sizeof(VI_LDCV2_ATTR_S), &stLDCV2Attr, sizeof(VI_LDCV2_ATTR_S));
        MAPI_CHECK_RET(HI_MAPI_MOD_VCAP, "memcpy_s", s32Ret);
    }
#endif
