...
static pthread_mutex_t g_dispFuncLock[HI_MAPI_DISP_MAX_NUM] = { PTHREAD_MUTEX_INITIALIZER };
...
HI_S32 HI_MAPI_DISP_Init(HI_HANDLE DispHdl, const HI_MAPI_DISP_ATTR_S *pstDispAttr)
{
    ...
    DISP_CHECK_HANDLE_RET(DispHdl);
    DISP_CHECK_NULL_PTR_RET(pstDispAttr);

    MUTEX_LOCK(g_dispFuncLock[DispHdl]);

    if (g_astDispCtx[DispHdl].bDispInited) {
        MAPI_ERR_TRACE(HI_MAPI_MOD_DISP, "Disp is already inited\n");
        MUTEX_UNLOCK(g_dispFuncLock[DispHdl]);
        return HI_SUCCESS;
    }

    s32Ret = memcpy_s(&stDispAttr, sizeof(HI_MAPI_DISP_ATTR_S), pstDispAttr, sizeof(HI_MAPI_DISP_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_DISP, "memcpy_s", s32Ret, g_dispFuncLock[DispHdl]);
    ...
    s32Ret = memcpy_s(&g_astDispCtx[DispHdl].stDispAttr, sizeof(HI_MAPI_DISP_ATTR_S), &stDispAttr,
        sizeof(HI_MAPI_DISP_ATTR_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_DISP, "memcpy_s", s32Ret, g_dispFuncLock[DispHdl]);
...
HI_S32 HI_MAPI_DISP_Deinit(HI_HANDLE DispHdl)
{
    ...
    DISP_CHECK_INIT_RET(g_astDispCtx[DispHdl].bDispInited);
...
HI_S32 HI_MAPI_DISP_SetAttrEx(HI_HANDLE DispHdl, HI_MAPI_DISP_CMD_E enCMD, HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    switch (enCMD) {
        case HI_MAPI_DISP_CMD_VIDEO_CSC: {
            DISP_CHECK_VIDEO_CSC_HANDLE_RET(DispHdl);
        ...
        case HI_MAPI_DISP_CMD_GRAPHIC_CSC: {
            DISP_CHECK_GRAPHIC_CSC_HANDLE_RET(DispHdl);
            if (u32Len != sizeof(HI_MAPI_DISP_CSCATTREX_S)) {
                MAPI_ERR_TRACE(HI_MAPI_MOD_DISP, "Disp_SETCSC u32Len:%x is not the size of HI_MAPI_DISP_CSCATTREX_S\n",
                               u32Len);
                MUTEX_UNLOCK(g_dispFuncLock[DispHdl]);
...
HI_S32 HI_MAPI_DISP_GetAttrEx(HI_HANDLE DispHdl, HI_MAPI_DISP_CMD_E enCMD, HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    switch (enCMD) {
        case HI_MAPI_DISP_CMD_VIDEO_CSC: {
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stDispAttrEx, sizeof(HI_MAPI_DISP_CSCATTREX_S));
            MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_DISP, "memcpy_s", s32Ret, g_dispFuncLock[DispHdl]);
                                    ...
                                    HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    switch (enCMD) {
        ...
        case HI_MAPI_DISP_WINDOW_CMD_ASPECTRATIO: {
            ...
            s32Ret = HAL_MAPI_DISP_SetWindowAspectRatioAttr(DispHdl, WndHdl, pstAspectRatioAttr);
            if (s32Ret != HI_SUCCESS) {
                                    ...
                                    HI_VOID *pAttr, HI_U32 u32Len)
{
    ...
    switch (enCMD) {
        ...
        case HI_MAPI_DISP_WINDOW_CMD_ASPECTRATIO: {
            ...
            s32Ret = HAL_MAPI_DISP_GetWindowAspectRatioAttr(DispHdl, WndHdl, &stAspectRatioAttr);
            if (s32Ret != HI_SUCCESS) {
            ...
            s32Ret = memcpy_s(pAttr, u32Len, &stAspectRatioAttr, sizeof(HI_MAPI_DISP_WINDOW_ASPECTRATIO_S));
            MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_DISP, "memcpy_s", s32Ret, g_dispFuncLock[DispHdl]);
                                  ...
                                  const HI_MAPI_DISP_WINDOW_ATTR_S *pstWndAttr)
{
    ...
    DISP_CHECK_HANDLE_RET(DispHdl);
    DISP_CHECK_WIN_ID_RET(WndHdl);
    DISP_CHECK_NULL_PTR_RET(pstWndAttr);
                               ...
                               HI_BOOL bStitch)
{
    ...
    DISP_CHECK_HANDLE_RET(DispHdl);
    DISP_CHECK_WIN_ID_RET(WndHdl);
    DISP_CHECK_VPSSDVE_HANDLE_RET(GrpHdl);
    DISP_CHECK_VPSSVPORT_HANDLE_RET(VPortHdl);
    ...
    DISP_CHECK_BIND_RET(g_astDispCtx[DispHdl].astWndAttr[WndHdl].bWindowBinded);
    ...
    HI_HANDLE WndHdl)
{
    ...
    DISP_CHECK_HANDLE_RET(DispHdl);
    DISP_CHECK_WIN_ID_RET(WndHdl);
    DISP_CHECK_VCAPPIPE_HANDLE_RET(VcapPipeHdl);
    DISP_CHECK_VCAPCHN_HANDLE_RET(PipeChnHdl);
...
HI_S32 HI_MAPI_DISP_SendFrame(HI_HANDLE DispHdl, HI_HANDLE WndHdl, const HI_MAPI_FRAME_DATA_S *pstFramedata)
{
    ...
    DISP_CHECK_HANDLE_RET(DispHdl);
    DISP_CHECK_WIN_ID_RET(WndHdl);
    DISP_CHECK_NULL_PTR_RET(pstFramedata);
    ...
    s32Ret = memcpy_s(&stVideoFrame.stVFrame.stSupplement,  sizeof(VIDEO_SUPPLEMENT_S),
        &pstFramedata->stVideoSupplement, sizeof(VIDEO_SUPPLEMENT_S));
    MAPI_CHECK_RET_UNLOCK(HI_MAPI_MOD_DISP, "memcpy_s", s32Ret, g_dispFuncLock[DispHdl]);
