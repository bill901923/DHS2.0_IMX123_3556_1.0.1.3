...
config_osdrv_mem()
{
	CONFIG_MEM_DDR_SIZE=$(awk 'BEGIN{printf("%#x",'$CONFIG_MEM_TOTAL_SIZE'*0x100000)}')
	...
	sed -i "s/^UBOOT_CFG_DDR_SIZE=.*0x.*$/UBOOT_CFG_DDR_SIZE=${CONFIG_MEM_DDR_SIZE}/g" ${1}
...
check()
{
	...
	echo HuaweiLiteMMZ: ${CONFIG_MEM_HUAWEILITE_MMZ_SIZE} $HUAWEILITE_MMZ_SIZE
	echo LinuzSys: ${CONFIG_MEM_LINUX_SYS_SIZE} $LINUX_SYS_MEM_SIZE
	echo LinuxMMZ: ${CONFIG_MEM_LINUX_MMZ_SIZE} $LINUX_MMZ_MEM_SIZE
	echo DDR Size: ${CONFIG_MEM_TOTAL_SIZE}M
}
