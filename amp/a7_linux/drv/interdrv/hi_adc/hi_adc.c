...
#define print_info_adc(fmt, arg...)   \
    do{  \
        printk("ADC:%s,%d "fmt, __func__,__LINE__, ##arg);  \
    }while(0)
...
static int lsadc_get_chn_value(int chn)
{
    ...
    else
    {
        ...
        print_info_adc("Pass chn:%d\n", chn);

            dev_err(NULL,"Pass chn:%d\n", chn);
    }
    

    printk(NULL,"lsadc_readl 0x%lx\n", (HISI_LSADC_CHNDATA + (chn << 2)));
    dev_err(NULL,"lsadc_readl 0x%lx\n", (HISI_LSADC_CHNDATA + (chn << 2)));
...
static long hiLsAdc_ioctl (unsigned int cmd, unsigned long arg, void* private_data)
{
    ...
    switch (cmd)
    {
        ...
        {
            param = *(int*)arg;
            print_info_adc("param:%d\n", param);
            dev_err(NULL,"param:%d\n", param);
...
int lsadc_init(void)
{
    ...
    if (!lsadc_reg)
    {
        ...
        if (!lsadc_reg)
        {
            print_info_adc("lsadc ioremap error.\n");
            return -1;
        }
    }
    ...
    if (ret != 0)
    {
        print_info_adc("lsadc request irq error.\n");
    ...
    if (ret != 0)
    {
        ...
        print_info_adc("lsadc register device error.\n");
