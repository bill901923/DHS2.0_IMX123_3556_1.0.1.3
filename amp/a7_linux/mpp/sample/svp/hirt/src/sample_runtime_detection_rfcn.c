        ...
        HI_RUNTIME_BLOB_S* pstSrcRFCNBlobs)
{
    ...
    s32Ret = SAMPLE_RUNTIME_ReadSrcFile(pcSrcFile, &(pstSrcRFCNBlobs[0]), (HI_U32)strlen(pcSrcFile));
    SAMPLE_CHK_GOTO(HI_SUCCESS != s32Ret, FAIL_0,
...
HI_S32 SAMPLE_RUNTIME_SetGroupDst_RFCN(HI_RUNTIME_BLOB_S* pstDstRFCNBlobs, const HI_U32 u32Length)
{
    if(0 == u32Length)
    {
        SAMPLE_LOG_INFO("pstDstRFCNBlobs is zero length!");
        return HI_FAILURE;
    }
...
HI_S32 SAMPLE_RUNTIME_ForwardGroup_RFCN(const HI_CHAR* pcSrcFile, HI_RUNTIME_GROUP_HANDLE hGroupHandle)
{
    ...
    s32Ret = SAMPLE_RUNTIME_SetGroupDst_RFCN(stDst, sizeof(stDst));
    SAMPLE_CHK_GOTO(HI_SUCCESS != s32Ret, FAIL_0,
    ...
    for (HI_U32 i = 0; i < 3; i++)
    {
        ...
        memset((HI_VOID*)(uintptr_t)(stDst[i].u64VirAddr), 0, u32BlobSize);
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (1 == i) { pcOutName = (HI_CHAR*)"cls_prob"; }

        if (2 == i) { pcOutName = (HI_CHAR*)"bbox_pred"; }
...
HI_S32 SAMPLE_RFCN()
{
    ...
    clock_gettime(0, &end);
    spend = ((long)end.tv_sec - (long)start.tv_sec) * 1000 + ((long)end.tv_nsec - (long)start.tv_nsec) / 1000000;
    ...
    spend = ((long)next.tv_sec - (long)end.tv_sec) * 1000 + ((long)next.tv_nsec - (long)end.tv_nsec) / 1000000;
