...
static HI_S32 createImageByIVE(IVE_IMAGE_S* pstImg, IVE_IMAGE_TYPE_E enType, HI_U32 u32Width, HI_U32 u32Height)
{
    ...
    switch (enType)
    {
        ...
        case IVE_IMAGE_TYPE_S8C1:
        {
            ...
            {
                SAMPLE_LOG_INFO("Mmz Alloc fail,Error(%#x)\n", s32Ret);
                return s32Ret;
            }
        }
        break;
...
static HI_S32 copyImageByIVE(IVE_IMAGE_S* pstSrcImage, IVE_IMAGE_S* pstDstImage, IVE_RECT_U16_S* pstDstRect)
{
    ...
    {
        SAMPLE_LOG_INFO("Invalid image type[%u]\n", pstDstImage->enType);
        return HI_FAILURE;
    }
...
static HI_S32 copyIveImage2Blob(const IVE_IMAGE_S* pstSrcImage, HI_RUNTIME_BLOB_S* pstDstBlobs, HI_U16 u16Index)
{
    ...
    HI_U8* pu8Dst = (HI_U8*)((uintptr_t)(pstDstBlobs->u64VirAddr + u16Index * u32DstHeight * u32DstStride * pstDstBlobs->unShape.stWhc.u32Chn));
    ...
    memcpy(pu8DstR, (HI_U8*)(uintptr_t)pstSrcImage->au64VirAddr[0], u32OneCSize);
    memcpy(pu8DstG, (HI_U8*)(uintptr_t)pstSrcImage->au64VirAddr[1], u32OneCSize);
    memcpy(pu8DstB, (HI_U8*)(uintptr_t)pstSrcImage->au64VirAddr[2], u32OneCSize);
...
static HI_S32 copyBlob2IveImage(const HI_RUNTIME_BLOB_S* pstSrcBlob, HI_U16 u16Index, IVE_IMAGE_S* pstImage)
{
    ...
    HI_U8* pu8Src = (HI_U8*)(uintptr_t)(pstSrcBlob->u64VirAddr + u16Index * u32SrcHeight * u32SrcStride * pstSrcBlob->unShape.stWhc.u32Chn);
    ...
    memcpy((HI_U8*)((uintptr_t)pstImage->au64VirAddr[0]), pu8SrcR, u32OneCSize);
    memcpy((HI_U8*)((uintptr_t)pstImage->au64VirAddr[1]), pu8SrcG, u32OneCSize);
    memcpy((HI_U8*)((uintptr_t)pstImage->au64VirAddr[2]), pu8SrcB, u32OneCSize);
    ...
    HI_DOUBLE* pEdgeX, HI_DOUBLE* pEdgeY)
{
    ...
    SAMPLE_CHK_RET_VOID(HI_SUCCESS != s32Ret, "rectImageByIVE fail!\n");
...
{
    SAMPLE_CHK_RET((IVE_IMAGE_TYPE_U8C3_PLANAR != pstSrcImage->enType), HI_FAILURE, "Not support image type[%u]\n", pstSrcImage->enType);
...
HI_VOID savePPMImage(const HI_CHAR* pszPicPath, IVE_IMAGE_S* pstImage)
{
    ...
    SAMPLE_CHK_RET_VOID((HI_NULL == pszPicPath || strlen(pszPicPath) > PATH_MAX), "invalid pic path(%s) is NULL or too long", pszPicPath);
    ...
    SAMPLE_CHK_RET_VOID((HI_NULL == pszDirName || strlen(pszDirName) <= 0), "dirname(%s) error", pszPicPath);
    pszBaseName = basename(aszPicPathForBaseName);
    SAMPLE_CHK_RET_VOID((HI_NULL == pszBaseName || strlen(pszBaseName) <= 0), "basename(%s) error", pszPicPath);
    ...
    SAMPLE_CHK_RET_VOID((strlen(aszAbsDirPath) + strlen(pszBaseName) + 1 > PATH_MAX), "aszAbsDirPath[%s%s] too long", aszAbsDirPath, pszBaseName);
    ...
    SAMPLE_CHK_RET_VOID(HI_NULL == ppmFile, "open file %s fail!\n", aszAbsFilePath);
    fprintf(ppmFile, "P6\n%d %d\n255\n", pstImage->u32Width, pstImage->u32Height);

    unsigned char * pucImageC0 = (unsigned char *)((uintptr_t)pstImage->au64VirAddr[0]);
    unsigned char * pucImageC1 = (unsigned char *)((uintptr_t)pstImage->au64VirAddr[1]);
    unsigned char * pucImageC2 = (unsigned char *)((uintptr_t)pstImage->au64VirAddr[2]);
...
HI_VOID drawImageRectByIVE(const HI_CHAR* pszPicPath, const HI_RUNTIME_BLOB_S* pstBlob,
                         HI_S32 as32Coord[], HI_U32 u32CoordCnt, HI_U32 u32CoordStride, const HI_U32 u32Length)
{
    IVE_IMAGE_S stSrcImage = {0};
    ...
    SAMPLE_CHK_GOTO(HI_SUCCESS != s32Ret, FAIL_0, "Picture path error!\n");
...
HI_VOID saveBlobByIVE(const HI_CHAR* pszPath, const HI_RUNTIME_BLOB_S* pstSrcBlob, HI_U16 u16Index, const HI_U32 u32Length)
{
    IVE_IMAGE_S stImage = {0};
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    SAMPLE_CHK_GOTO(HI_SUCCESS != s32Ret, FAIL_0, "Path error!\n");
