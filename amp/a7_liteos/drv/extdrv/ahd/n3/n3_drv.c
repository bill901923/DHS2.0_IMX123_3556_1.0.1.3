...
void N3_I2CWriteByte8(unsigned char chip_addr, unsigned char reg_addr, unsigned char value)
{
    HI_LITOS_I2C_Write(reg_addr,&value,1);
}

unsigned char N3_I2CReadByte8(unsigned char chip_addr, unsigned char reg_addr)
{
...
int mpu_ioctl(struct file* filp, int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case IOC_VDEC_START_CHNL:
            ...
            N3_video_chnl_start(chnl_control.devnum, chnl_control.chn);
            break;
        case IOC_VDEC_STOP_CHNL:
            if (copy_from_user(&chnl_control, argp, sizeof(video_chnl_control)))
            { printk("IOC_VDEC_STOP_CHNL error\n"); }

            N3_video_chnl_stop(chnl_control.devnum, chnl_control.chn);
            break;
...
void __exit n3_module_exit(void)
{
    ...
    HI_LITOS_I2C_Exit();
    printk("N3_drv : Test Driver Unloaded\n");
}
