...
extern unsigned char mclk;
extern unsigned char lane;
...
void embeded_data_enable(int dev_num)
{
    unsigned char val_20x1B;
    gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0xff, 0x20);
    val_20x1B = gpio_i2c_read(jaguar1_i2c_addr[dev_num], 0x1B) & 0xF0;
    gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x1B, val_20x1B | 0x03); //enable
    gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x1C, 0x55);  //embedded data in first line
    //gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x1C, 0xaa);  //embedded data in last line
    gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0xff, 0x21);
    gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x3C, 0x55);
}
...
void mipi_video_format_set(video_input_init* dev_ch_info)
{
    ...
    if (dev_ch_info->ch_mipiout_en != DISABLE)
    {
...
int mipi_datatype_set(unsigned char data_type)
{
    ...
    switch (data_type)
    {
        case VD_DATA_TYPE_YUV422 :
            mipi_dtype = 0x1E;
            arb_dtype = 0x00;
            printk("VD_DATA_TYPE_YUV422\n");
            break;

        case VD_DATA_TYPE_YUV420 :
            mipi_dtype = 0x18;
            arb_dtype = 0xAA;
            printk("VD_DATA_TYPE_YUV420\n");
            break;

        case VD_DATA_TYPE_LEGACY420 :
            mipi_dtype = 0x1A;
            arb_dtype = 0x55;
            printk("VD_DATA_TYPE_LEGACY420\n");
            break;
...
void mipi_tx_init(int dev_num)
{
    ...
    //#if defined(_MIPI_252MHZ_TEST_)
    switch (mclk)
    {
        case 3:
            printk("[DRV]_MIPI_1242MHZ_TEST_\n");
        ...
        case 1:
            printk("[DRV]_MIPI_594MHZ_TEST_\n");
			gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x40, 0xAC);  //05.15
            gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x41, 0x10);
            gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x42, 0x03);
            break;

        default:
            printk("[DRV]_MIPI_756MHZ_TEST_\n");
            gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x40, 0xDC);
            gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x41, 0x10);
            gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x42, 0x03);
            break;
