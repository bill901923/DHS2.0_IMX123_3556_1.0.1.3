...
void mipi_tx_drv_set_phy_cfg(combo_dev_cfg_t *dev_cfg)
{
    ...
    data_ths_trail = (actual_phy_data_rate * THS_TRAIL + 7999) / 8000+1;
    ...
    mipi_tx_regs_va->PHY_TMR_CFG.u32 = (data_ths_trail << 16) +
        (data_tpre_delay + data_tlpx + data_ths_prepare + data_ths_zero);
    /* clk lp2hs,hs2lp time */
    mipi_tx_regs_va->PHY_TMR_LPCLK_CFG.u32 = ((32 + data_ths_trail) << 16) +
        (clk_tlpx + clk_tclk_prepare + clk_tclk_zero);
    /* edpi_cmd_size */
