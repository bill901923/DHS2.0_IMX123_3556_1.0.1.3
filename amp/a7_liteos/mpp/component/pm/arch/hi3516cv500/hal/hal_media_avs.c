...
int  hal_media_get_voltage_accord_temp(int s32temp)
{
    ...
    if(u32Chip_ID == HI3559_V200_CHIP) {
      if ( s32temp <= -20) {
          s32volt_uv = 0;
      } else if ( -20 < s32temp && s32temp <= 50 ) {    /**Linear adjustment  10mv **/
          s32volt_uv = (10 * 1000 / 70) * (s32temp + 20);
      } else if  (100 >= s32temp && s32temp > 50) {     /**Linear adjustment 5mv **/
          s32volt_uv = (5 * 1000 / 50) * (s32temp - 50) + 10* 1000;
      } else if  (s32temp > 100) {
        ...
        if ( s32temp <= -20) {
            s32volt_uv = 0;
        } else if ( -20 < s32temp && s32temp <= 50 ) {
            s32volt_uv = (10 * 1000 / 70) * (s32temp + 20);  /**Linear adjustment 10mv **/
        } else if  (100 >= s32temp && s32temp > 50) {
            s32volt_uv = (10 * 1000 / 50) * (s32temp - 50) + 10* 1000; /**Linear adjustment 10mv **/
        } else if  (s32temp > 100) {
            s32volt_uv = 20 * 1000;     /**adjustment 20mv **/
        }
        unsigned int hpm = 0;
        READ_REG(0x12020098, &hpm);  /**Get hpm value **/
        hpm = hpm >> 16;
        PM_DEBUG("== The chip hpm value[0x%08x] \n",hpm);
        if (hpm < 230) {    /**hpm < 230 **/
            s32volt_uv = 0;
        }
    }
